...
dojo.declare(
  ...
  com.nuclearunicorn.core.Control,
  {
    /**
    ...
    registerMeta: function (type, meta, provider) {
      if (!type) {
        ...
        this.meta.push({
          meta: meta,
          provider: {
            getEffect: function (item, effect) {
	...
	*/
    ...
    updateEffectCached: function () {
      ...
      for (var name in this.effectsCachedExisting) {
        ...
        this.game.globalEffectsCached[name] =
          typeof this.game.globalEffectsCached[name] == "number"
            ? this.game.globalEffectsCached[name] + effect
            : effect;
      }
    },
...
*/
    ...
    loadMetadata: function (meta, saveMeta, metaId) {
      if (!saveMeta) {
        console.trace();
        console.warn(
          "Unable to load metadata table '" + metaId + "', save record is empty"
        );
        return;
      }
        ...
        if (savedMetaElem != null) {
          ...
          for (var fld in savedMetaElem) {
            ...
            if (savedMetaElem[fld] !== undefined) {
              if (
                savedMetaElem[fld] != null &&
                typeof savedMetaElem[fld] == "object"
              ) {
...
dojo.declare("com.nuclearunicorn.game.log.Console", null, {
  ...
  msg: function (message, type, tag, noBullet) {
    ...
    var logmsg = {
      ...
      seasonTitle: hasCalendarTech
        ? this.game.calendar.getCurSeasonTitle()
        : null,
      seasonTitleShorten: hasCalendarTech
        ? this.game.calendar.getCurSeasonTitleShorten()
        : null
    };
...
dojo.declare("com.nuclearunicorn.game.ui.ButtonController", null, {
  ...
  payPriceForUndoRefund: function (model) {
    ...
    if (!model.prices.length) {
      console.warn(
        "unable pay prices for undo refund a building, no prices specified in metadata :O"
      );
      return;
    }
    for (var i = 0; i < model.prices.length; i++) {
      ...
      if (res.isRefundable(this.game) && !price.isTemporary) {
        this.game.resPool.addResEvent(
          price.name,
          -price.val * model.refundPercentage
        );
      } else {
        // No refund to undo
      }
  ...
  refund: function (model) {
    if (!model.prices.length) {
      console.warn(
        "unable to refund building, no prices specified in metadata :O"
      );
      return;
    }
    for (var i = 0; i < model.prices.length; i++) {
...
dojo.declare(
  "com.nuclearunicorn.game.ui.Button",
  com.nuclearunicorn.core.Control,
  {
    model: null,
    controller: null,
    ...
    render: function (btnContainer) {
      ...
      this.buttonContent = dojo.create(
        "div",
        {
    ...
    onClick: function (event) {
      this.animate();
      var self = this;
      this.controller.buyItem(
        this.model,
        event,
        function (itemBought, extendedInfo) {
          if (
            typeof itemBought !== "boolean" ||
            typeof extendedInfo !== "object"
          ) {
      ...
      if (prices.length && !this.tooltip) {
        ...
        for (var i = 0; i < prices.length; i++) {
          ...
          var nameSpan = dojo.create(
            "span",
            { innerHTML: price.title, style: { float: "left" } },
            priceItemNode
          );
          var priceSpan = dojo.create(
            "span",
            { innerHTML: price.displayValue, style: { float: "right" } },
            priceItemNode
          );
    ...
    addLink: function (linkModel) {
      ...
      var link = dojo.create(
        "a",
        {
          ...
          className:
            longTitleClass +
            (linkModel.cssClass ? " " + linkModel.cssClass : ""),
          style: {
            paddingLeft: "2px",
            float: "right",
            cursor: "pointer"
      ...
      var linkHandler = dojo.connect(
        link,
        "onclick",
        this,
        dojo.partial(function (handler, event) {
...
dojo.declare(
  ...
  com.nuclearunicorn.game.ui.ButtonController,
  {
    defaults: function () {
    ...
    _createPriceLineModel: function (price, simplePrices, indent) {
      ...
      if (!hasRes && resPerTick > 0 && !simplePrices) {
        eta =
          (price.val - res.value) /
          (resPerTick * this.game.getTicksPerSecondUI());
        if (eta >= 1) {
        ...
        if (craft.unlocked) {
          ...
          for (var j in components) {
            ...
            var comp = {
              ...
              val: val,
              baseVal: components[j].val
            };

            var compResult = this._createPriceLineModel(
              comp,
              simplePrices,
              indent + 1
            );
    ...
    updateEffectModels: function (model) {
      ...
      var isEffectMultiplierEnabled =
        model.multiplyEffects && this.game.ui.isEffectMultiplierEnabled();
      ...
      for (var effectName in effectsList) {
        ...
        if (
          !isEffectMultiplierEnabled &&
          effectMeta.calculation !== "nonProportional"
        ) {
          var nextEffectValue = this.getNextEffectValue(model, effectName);
          if (nextEffectValue) {
            effectValue =
              nextEffectValue * (model.metadata.on + 1) -
              effectValue * model.metadata.on;
          }
        }

        var displayParams = this.game.getEffectDisplayParams(
          effectName,
          effectValue,
          false /*showIfZero*/
        );
...
ButtonModernHelper = {
  getTooltipHTML: function (controller, model) {
    ...
    if (
      model.metadata &&
      typeof model.metadata.isAutomationEnabled == "boolean"
    ) {
      //undefined or null don't count here
      dojo.create(
        "div",
        {
          innerHTML: model.metadata.isAutomationEnabled
            ? $I("btn.aon.tooltip")
            : $I("btn.aoff.tooltip"),
          className:
            "desc small" +
            (model.metadata.isAutomationEnabled ? " auto-on" : " auto-off")
        },
        tooltip
      );
    }
  ...
  renderEffects: function (tooltip, effectsList, hideTitle) {
    ...
    for (var i = 0; i < effectsList.length; i++) {
      ...
      var nameSpan = dojo.create(
        ...
        {
          innerHTML:
            effectModel.displayEffectName +
            ": " +
            effectModel.displayEffectValue,
...
dojo.declare(
  ...
  com.nuclearunicorn.game.ui.Button,
  {
    afterRender: function () {
      ...
      this.renderLinks();
      this.attachTooltip(
        dojo.partial(this.getTooltipHTML(), this.controller, this.model)
      );
    ...
    updateLink: function (buttonLink, modelLink) {
      if (buttonLink) {
        if (!modelLink) {
          //This only ever happens if I mess around with console commands
          dojo.destroy(buttonLink.link);
          return;
        }
        buttonLink.link.textContent = modelLink.title;
        if (modelLink.cssClass) {
          buttonLink.link.className = modelLink.cssClass;
        }
        if (modelLink.tooltip) {
          buttonLink.link.title = modelLink.tooltip;
        }
        dojo.style(
          buttonLink.link,
          "display",
          modelLink.visible === undefined || modelLink.visible ? "" : "none"
        );
      }
    },
...
dojo.declare(
  ...
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    ...
    fetchModel: function (options) {
      ...
      model.showSellLink =
        model.metadata && model.metadata.val && model.hasSellLink;
      var self = this;
      ...
      if (typeof model.metadata.isAutomationEnabled == "boolean") {
        model.toggleAutomationLink = {
          ...
          tooltip: model.metadata.isAutomationEnabled
            ? $I("btn.aon.tooltip")
            : $I("btn.aoff.tooltip"),
          //reserved for mobile
          enabled: model.metadata.val > 0,
    ...
    getNextEffectValue: function (model, effectName) {
      ...
      this.game.calendar.cycleEffectsBasics(
        underlying.effects,
        underlying.name
      );
    ...
    sell: function (event, model) {
      ...
      if (end > 0 && event && event.shiftKey) {
        //no need to confirm if selling just 1
        end = 0;
    ...
    sellInternal: function (model, end, requireSellLink) {
      ...
      if (typeof requireSellLink !== "boolean") {
        console.warn(
          'Boolean parameter "requireSellLink" was not specified, defaulting to false.'
        );
      ...
      while (building.val > end) {
        ...
        if (requireSellLink && !this.hasSellLink(model)) {
          //religion upgrades can't sell past 1
          break;
        }
      }
...
dojo.declare(
  ...
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    ...
    renderLinks: function () {
      ...
      //--------------- style -------------
      if (
        (building.val > 9 || building.name.length > 10) &&
        this.model.hasSellLink
      ) {
  ...
  com.nuclearunicorn.game.ui.BuildingBtnController,
  {
    ...
    getPrices: function (model) {
      ...
      var pricesDiscount = this.game.getLimitedDR(
        this.game.getEffect(meta.name + "CostReduction"),
        1
      );
      var priceModifier = 1 - pricesDiscount;

      for (var i = 0; i < meta.prices.length; i++) {
        var resPriceDiscount = this.game.getEffect(
          meta.prices[i].name + "CostReduction"
        );
    ...
    buyItem: function (model, event, callback) {
      ...
      if (
        ...
        this.game.ui.confirm(
          "",
          $I("iron.will.break.confirmation.msg"),
          function () {
          ...
          function () {
            callback(false /*itemBought*/, {
              reason:
                "player-denied" /*The player decided not to buy this after all.*/
            });
          }
        );
      } else {
    ...
    _buyItem_step2: function (model, event, callback) {
      ...
      if (!meta.noStackable && event.shiftKey) {
        ...
        if (this.game.opts.noConfirm) {
          ...
          var self = this;
          this.game.ui.confirm(
            $I("construct.all.confirmation.title"),
            $I("construct.all.confirmation.msg"),
            function () {
    ...
    build: function (model, maxBld) {
      ...
      if (meta.breakIronWill) {
        ...
        var zebraOutpostMeta =
          this.game.bld.getBuildingExt("zebraOutpost").meta;
    ...
    incrementValue: function (model) {
      ...
      if (
        (meta.togglableOnOff &&
          (allOff ||
            (meta.val == 1 && this.game.resPool.get("paragon").value == 0))) ||
...
dojo.declare(
  "com.nuclearunicorn.game.ui.BuildingResearchBtn",
  com.nuclearunicorn.game.ui.BuildingBtn,
  {}
);
...
dojo.declare("com.nuclearunicorn.game.ui.Spacer", null, {
  ...
  render: function (container) {
    dojo.create(
      "div",
      { innerHTML: this.title, className: "spacer" },
      container
    );
  },
...
dojo.declare(
  ...
  [com.nuclearunicorn.game.ui.ContentRowRenderer, mixin.IChildrenAware],
  {
    game: null,

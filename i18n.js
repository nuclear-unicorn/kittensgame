...
dojo.declare("com.nuclearunicorn.i18n.Lang", null, {
  ...
  _init: function (timestamp) {
    ...
    if (!lang || !this.isAvailable(lang)) {
      ...
      var defaultLocale =
        this.platformLocale || navigator.language || navigator.userLanguage;
      // find closes match
    ...
    $.getJSON("res/i18n/" + this.fallbackLocale + ".json?_=" + timestamp)
      .done(function (fallbackLocaleMessages) {
        ...
        if (self.language != self.fallbackLocale) {
          ...
          $.getJSON("res/i18n/" + lang + ".json?_=" + timestamp)
            .then(function (legacyLocaleMessages) {
              console.log(
                "loaded legacy locale for lang",
                lang,
                legacyLocaleMessages
              );
            ...
            .always(function () {
              ...
              $.getJSON("res/i18n/crowdin/" + lang + ".json?_=" + timestamp)
                .then(function (crowdinLocale) {
                  console.log(
                    "loaded crowdin locale for lang",
                    lang,
                    crowdinLocale
                  );
      ...
      .fail(function (error) {
        console.error(
          "Unable to load locale chain for  '" + lang + "', error:",
          error
        );
      });

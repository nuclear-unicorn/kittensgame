...
test("Winter Challenge--Effects should have correct values", () => {
  ...
  //Helper functions:
  const catnipModifier = function () {
    return calendar.getWeatherMod(catnipResource);
  };
  const calculateWhatIsRelevant = function () {
    game.upgrade({ challenges: ["winterIsComing"] });
  };
  ...
  expect(solarFarm.calculateEnergyProduction(game, 1 /*Summer*/)).toBeCloseTo(
    2.666667
  );
  expect(solarFarm.calculateEnergyProduction(game, 2 /*Autumn*/)).toBe(2);
  expect(solarFarm.calculateEnergyProduction(game, 3 /*Winter*/)).toBeCloseTo(
    1.5
  );
  ...
  calculateWhatIsRelevant();
  expect(solarFarm.calculateEnergyProduction(game, 0 /*Winter I*/)).toBeCloseTo(
    1.5
  );
  expect(
    solarFarm.calculateEnergyProduction(game, 1 /*Winter II*/)
  ).toBeCloseTo(1.5);
  expect(
    solarFarm.calculateEnergyProduction(game, 2 /*Winter III*/)
  ).toBeCloseTo(1.5);
  expect(
    solarFarm.calculateEnergyProduction(game, 3 /*Winter IV*/)
  ).toBeCloseTo(1.5);
  ...
  expect(solarFarm.calculateEnergyProduction(game, 0 /*Winter I*/)).toBeCloseTo(
    1.5
  ); //Cold harshness affects catnip, not solar power
  expect(
    solarFarm.calculateEnergyProduction(game, 1 /*Winter II*/)
  ).toBeCloseTo(1.5);
  expect(
  ...
  expect(solarFarm.calculateEnergyProduction(game, 0 /*Spring*/)).toBe(2);
  expect(solarFarm.calculateEnergyProduction(game, 1 /*Summer*/)).toBeCloseTo(
    (1 + game.getLimitedDR(5, 2)) * 2.666667
  );
  expect(solarFarm.calculateEnergyProduction(game, 2 /*Autumn*/)).toBe(2);
  expect(solarFarm.calculateEnergyProduction(game, 3 /*Winter*/)).toBeCloseTo(
    1.5
...
test("Anarchy Challenge--Effects should have correct values", () => {
  ...
  const calculateWhatIsRelevant = function () {
    game.upgrade({ challenges: ["anarchy"] });
  };
...
test("Energy Challenge--Effects should have correct values", () => {
  ...
  //Helper function:
  const calculateWhatIsRelevant = function () {
    game.upgrade({ challenges: ["energy"] });
  };
  ...
  expect(resPool.getEnergyConsumptionRatio()).toBeCloseTo(
    1 + game.getLimitedDR(-0.76, 1)
  );
...
test("Atheism Challenge--Solar Revolution limit should have correct value", () => {
  ...
  //Helper functions:
  const getSR = function () {
    return religionManager.getSolarRevolutionRatio();
  };
  const calculateWhatIsRelevant = function () {
    game.upgrade({ challenges: ["atheism"] });
  };
...
test("Black Sky Challenge--Corruption bonus should have correct value", () => {
  ...
  expect(markers.effects["corruptionRatio"]).toBeCloseTo(
    1.1 * CORRUPTION_FROM_1_MARKER
  );
  ...
  //Beginnings of LDR:
  expect(markers.effects["corruptionRatio"]).toBeCloseTo(
    (1 + game.getLimitedDR(1.6, 2)) * CORRUPTION_FROM_1_MARKER
  );
  ...
  //Well into LDR:
  expect(markers.effects["corruptionRatio"]).toBeCloseTo(
    (1 + game.getLimitedDR(9.9, 2)) * CORRUPTION_FROM_1_MARKER
  );
...
test("Pacifism Challenge--Weapon upgrades should have correct values", () => {
  ...
  const weaponUpgrades = [
    game.workshop.get("compositeBow"),
    game.workshop.get("crossbow"),
    game.workshop.get("railgun")
  ];

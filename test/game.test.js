...
test("Reset should assign a correct ammount of paragon and preserve certain upgrades", () => {
  ...
  expect(hgProduction).toBeLessThanOrEqual(
    baselineProduction * (1 + paragonProductionRatio) * 100
  );
  //-----------------------------------------------------
...
test("Safe infinity tests", () => {
  ...
  for (const tv of nearlyInfinity) {
    expect(game.getUnlimitedDR(tv, stripe)).toBeLessThan(
  ...
  for (const tv of halfInfinity) {
    expect(game.getInverseUnlimitedDR(tv, stripe)).toBeLessThan(
      Number.MAX_VALUE
    );
...
test("Explored biome should produce rewards", () => {
  ...
  /*
    ...
    */
  var fuzzBuffer = 0.95;
  expect(amt).toBeGreaterThanOrEqual(
    rewardSpec.value * (1 - rewardSpec.width) * fuzzBuffer
  );
  expect(amt).toBeLessThanOrEqual(
    rewardSpec.value * (1 + rewardSpec.width) * fuzzBuffer
  );
  ...
  var amt = rewards["catnip"];
  expect(amt).toBeGreaterThanOrEqual(
    rewardSpec.value * (1 - rewardSpec.width) * multiplier * fuzzBuffer
  );
  expect(amt).toBeLessThanOrEqual(
    rewardSpec.value * (1 + rewardSpec.width) * multiplier * fuzzBuffer
  );
...
test("Spaceports should be unlocked correctly and have a custom price logic applied", () => {
  ...
  expect(
    _get("warehouse").prices.find((price) => price.name == "starchart").val
  ).toBe(100000);
  ...
  expect(
    Math.round(
      _get("warehouse").prices.find((price) => price.name == "titanium").val
    )
  ).toBe(40456);
...
test("buyItem internals should work properly for Fix Cryochamber", () => {
  ...
  for (const res of usedCryochambers.fixPrices) {
    game.resPool.addResEvent(
      res.name,
      res.val + 1 /*Give us a little extra to check the price is correct*/
    );
  }
...
test("buyItem internals should work properly for items with no cost", () => {
  ...
  const handlerFunction = function (model) {
    handlerResult = model.name;
  };

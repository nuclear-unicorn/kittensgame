dojo.declare(
  "classes.managers.ResourceManager",
  com.nuclearunicorn.core.TabManager,
  {
    ...
    addRes: function (
      res,
      addedValue,
      allowDuringParadoxes,
      preventLimitCheck
    ) {
      if (
        (this.game.calendar.day < 0 && !allowDuringParadoxes) ||
        addedValue == 0
      ) {
        return 0;
      }
      ...
      if (res.name == "karma") {
        this.game.karmaKittens = Math.round(
          this.game.getInverseUnlimitedDR(res.value, 5)
        );
    ...
    update: function () {
      ...
      for (var i in this.resources) {
        ...
        maxValue = Math.min(
          this.addResMaxRatios(res, maxValue),
          Number.MAX_VALUE
        );
        ...
        if (challengeEffect) {
          ...
          challengeEffect = this.game.getLimitedDR(
            this.addResMaxRatios(res, challengeEffect),
            maxValue - 1 - game.bld.effectsBase[res.name + "Max"] || 0
          );
        ...
        if (game.loadingSave) {
          //hack to stop production before game.calculateAllEffects after manual import
          continue;
        }
      ...
      var calculateEnergyProduction = solarFarm.get(
        "calculateEnergyProduction"
      );
      if (currentSeason != 3 && calculateEnergyProduction) {
        var energyLoss =
          calculateEnergyProduction(game, currentSeason) -
          calculateEnergyProduction(game, 3);
        this.energyWinterProd -=
          solarFarm.get("on") * energyLoss * energyProdRatio;
      }
    },
    ...
    getEnergyConsumptionRatio: function () {
      ...
      return (
        (1 +
          game.getEffect("energyConsumptionRatio") +
    ...
    fastforward: function (daysOffset) {
      ...
      for (var i in this.resources) {
        ...
        if (
          res.perTickCached &&
          (res.name != "catnip" || res.perTickCached >= 0)
        ) {
          if (res.maxValue) {
          ...
          this.addRes(
            res,
            res.perTickCached * daysOffset * this.game.calendar.ticksPerDay,
            true /*allowDuringParadoxes*/,
            true /*preventLimitCheck*/
          );
    ...
    resConsHackForResTable: function () {
      if (this.game.calendar.day >= 0) {
        for (var i in this.resources) {
          var res = this.resources[i];
          if (res.maxValue) {
            var perTickConvertion = this.game.getResourcePerTickConvertion(
              res.name
            );
            if (perTickConvertion < 0) {
              if (
                this.game.getResourcePerTick(res.name, true) > 0 &&
                res.maxValue + perTickConvertion <= res.value
              ) {
    ...
    isStorageLimited: function (prices) {
      if (prices && prices.length) {
        for (var i = 0; i < prices.length; i++) {
          ...
          if (res.craftable && price.val > res.value) {
            //account for chronosphere resets etc
    ...
    getEnergyDelta: function () {
      if (this.energyCons == 0) {
        ...
        if (
          this.game.challenges.getChallenge("energy").researched &&
          !this.game.challenges.isActive("energy")
        ) {
          delta = 1 - (1 - delta) / 2;
        }
        return delta;

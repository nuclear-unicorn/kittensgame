...
dojo.declare("classes.ui.DesktopUI", classes.ui.UISystem, {
  ...
  constructor: function (containerId) {
    ...
    dojo.connect($("html")[0], "onkeyup", this, function (event) {
      ...
      if (keybind && keybind.action) {
        ...
        !isInputElement &&
        keybind &&
        keybind.name != this.game.ui.activeTabId
      ) {
        // If a keybound is found and the tab isn't current
        for (var i = 0; i < this.game.tabs.length; i++) {
          if (
            this.game.tabs[i].tabId === keybind.name &&
            this.game.tabs[i].visible
          ) {
  ...
  render: function () {
    ...
    var tabNavigationDiv = dojo.create(
      "div",
    ...
    if (!this.calenderDivTooltip) {
      ...
      this.calenderDivTooltip = UIUtils.attachTooltip(
        game,
        calendarDiv,
        0,
        200,
        dojo.hitch(game.calendar, function () {
          var tooltip = "";
          var displayThreshold = 100000;
          if (this.year > displayThreshold) {
            tooltip =
              $I("calendar.year.tooltip") + " " + this.year.toLocaleString();
          }
    ...
    if (!this.calendarSignSpanTooltip) {
      ...
      this.calendarSignSpanTooltip = UIUtils.attachTooltip(
        game,
        calendarSignSpan,
        0,
        60,
        dojo.hitch(game.calendar, function () {
          ...
          if (
            game.prestige.getPerk("numeromancy").researched &&
            this.festivalDays
          ) {
  ...
  updateTabs: function () {
    ...
    if (this.game.village.leader) {
      dojo
        .query("a.tab.traitLeaderBonus")
        .removeClass(
          "traitLeaderBonus engineer metallurgist chemist merchant manager scientist wise"
        );
      switch (this.game.village.leader.trait.name) {
        case "engineer": // Crafting bonuses
          dojo
            .query("a.tab.Workshop")
            .addClass("traitLeaderBonus" + " engineer");
          break;
        case "metallurgist":
          dojo
            .query("a.tab.Workshop")
            .addClass("traitLeaderBonus" + " metallurgist");
          break;
        case "chemist":
          dojo
            .query("a.tab.Workshop")
            .addClass("traitLeaderBonus" + " chemist");
          break;
        ...
        case "scientist": // Science prices bonus
          dojo
            .query("a.tab.Science")
            .addClass("traitLeaderBonus" + " scientist");
          break;
  ...
  updateCalendar: function () {
    ...
    if (hasCalendarTech) {
      ...
      calendarDiv.innerHTML = $I("calendar.year.full", [
        year.toLocaleString(),
        seasonTitle + mod,
        Math.floor(calendar.day)
      ]);
      document.title =
        $I("navbar.title") +
        " - " +
  ...
  updateUndoButton: function () {
    ...
    if (isVisible) {
      $("#undoBtn").text(
        $I("ui.undo", [
          Math.floor(this.game.undoChange.ttl / this.game.ticksPerSecond)
        ])
      );
    }
  },
  ...
  updateLanguage: function () {
    ...
    $("#languageApplyLink").toggle(
      languageSelector.val() != i18nLang.getLanguage()
    );
  },
  ...
  updateOptions: function () {
    ...
    if (game.opts.tooltipsInRightColumn) {
      $("#tooltip")
        ...
        .addClass("tooltip-in-right-column");
    } else {
      $("#tooltip")
        .detach()
        .appendTo("#game")
        .removeClass("tooltip-in-right-column");
    }
    ...
    $("#usePercentageResourceValues")[0].checked =
      game.opts.usePercentageResourceValues;
    $("#showNonApplicableButtons")[0].checked =
      game.opts.showNonApplicableButtons;
    $("#usePercentageConsumptionValues")[0].checked =
    ...
    for (var i in notations) {
      $("<option />")
        .attr("value", notations[i])
        .text($I("opts.notation." + notations[i]))
        .appendTo(notationSelect);
    }
  ...
  unlockScheme: function (name) {
    if (this.game.unlockedSchemes.indexOf(name) < 0) {
      ...
      this.game.msg(
        $I("opts.theme.unlocked") + $I("opts.theme." + name),
        "important"
      );
  ...
  updateIndexHTMLLanguage: function () {
    ...
    $("#optionUsePercentageResourceValues").text(
      $I("ui.option.use.percentage.resource.values")
    );
    $("#optionShowNonApplicableButtons").text(
      $I("ui.option.show.non.applicable.buttons")
    );
    $("#optionUsePercentageConsumptionValues").text(
      $I("ui.option.use.percentage.consumption.values")
    );
    $("#optionHighlightUnavailable").text(
      $I("ui.option.highlight.unavailable")
    );
    ...
    $("#optionEnableRedshiftGflops").text(
      $I("ui.option.enable.redshiftGflops")
    );

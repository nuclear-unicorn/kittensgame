  ...
  com.nuclearunicorn.core.TabManager,
  {
    constructor: function (game) {
      ...
      this.registerMeta(false, this.challenges, {
        getEffect: function (challenge, effectName) {
          ...
          if (effectName == "kittenLaziness") {
            //Calculated in a special way just for the Anarchy Challenge
            return amt;
          }
          ...
          if (stackOptions.capMagnitude) {
            ...
            amt = Math.max(
              -stackOptions.capMagnitude,
              Math.min(stackOptions.capMagnitude, amt)
            );
          }
          return amt;
        }
    ...
    challenges: [
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          if (self.active) {
            ...
            self.effects["kittenLaziness"] =
              0.5 +
              game.getLimitedDR(
                0.05 * self.on,
                self.stackOptions["kittenLaziness"].LDRLimit
              );
          } else {
      ...
      },
      {
        ...
        checkCompletionCondition: function (game) {
          return (
            game.bld.get("pasture").val > 0 &&
            game.bld.get("pasture").stage == 1 &&
            game.bld.get("aqueduct").val > 0 &&
            game.bld.get("aqueduct").stage == 1 &&
            game.bld.get("steamworks").val > 0 &&
            game.bld.get("magneto").val > 0 &&
            game.bld.get("reactor").val > 0 &&
            game.space.getBuilding("sattelite").val > 0 &&
            game.workshop.get("solarSatellites").researched &&
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          if (self.active) {
            ...
            self.effects["bskSattelitePenalty"] =
              0.1 * (game.ironWill ? 0 : self.on || 0);
          } else {
        ...
        checkCompletionCondition: function (game) {
          return (
            game.space.getBuilding("spaceBeacon").val >
            (game.challenges.getChallenge("blackSky").on || 0)
          );
        },
        upgrades: {
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          if (self.active) {
            ...
            self.effects["steamworksFakeBought"] =
              Math.floor(1.5 * self.on || 1) / (self.on || 1);
            ...
            self.effects["tradeKnowledgeRatio"] =
              self.getTradeBonusEffect(game);
          }
          game.upgrade(self.upgrades); //this is a hack, might need to think of a better sollution later
        },
        ...
        checkCompletionConditionOnReset: function (game) {
          if (game.diplomacy.baseManpowerCost > 0) {
            // BSK+IW precaution
            if (
              !game.village.sim.hadKittenHunters &&
              game.stats.getStatCurrent("totalTrades").val > 0
            ) {
        ...
        getTradeBonusEffect: function (game) {
          ...
          var tradepostLimit =
            (7 + tradeKnowledge * 3) * (0.99 + tradeKnowledge * 0.01);
          var tradepostRatioLimit = game.getLimitedDR(
            0.099 + tradeKnowledge * 0.0075,
            0.25
          );
          return (
            tradepost.effects["tradeRatio"] *
            Math.min(tradepostLimit, tradepost.val * tradepostRatioLimit)
          );
        }
      },
      {
    ...
    save: function (saveData) {
      ...
      for (var i in this.game.challenges.reserves.reserveKittens) {
        var _kitten = this.game.challenges.reserves.reserveKittens[i].save(
          this.game.opts.compressSaveFile,
          this.game.village.jobNames
        );
    ...
    load: function (saveData) {
      ...
      for (var i = 0; i < this.challenges.length; i++) {
        ...
        if (
          this.challenges[i].unlocks &&
          this.challenges[i].on &&
          !this.challenges[i].active
        ) {
      ...
      if (saveData.challenges.reserves) {
        ...
        this.game.challenges.reserves.reserveResources =
          saveData.challenges.reserves.reserveResources;
        this.game.challenges.reserves.reserveCryochambers =
          saveData.challenges.reserves.reserveCryochambers || kittens.length;
      }
    },
    ...
    update: function () {
      ...
      if (this.getChallenge("energy").unlocked == false) {
        if (
          this.game.resPool.energyProd != 0 ||
          this.game.resPool.energyCons != 0
        ) {
      ...
      for (var i = 0; i < this.challenges.length; i++) {
        if (
          this.challenges[i].active &&
          this.challenges[i].checkCompletionCondition &&
          this.challenges[i].checkCompletionCondition(this.game)
        ) {
	...
	*/
    ...
    researchChallenge: function (challenge) {
      if (this.isActive(challenge)) {
        ...
        this.game.msg(
          $I("challendge.btn.log.message.on.complete", [
            this.getChallenge(challenge).label
          ])
        );
    ...
    onRunReset: function () {
      for (var i = 0; i < this.challenges.length; i++) {
        if (
          this.challenges[i].active &&
          this.challenges[i].checkCompletionConditionOnReset &&
          this.challenges[i].checkCompletionConditionOnReset(this.game)
        ) {
          this.researchChallenge(this.challenges[i].name);
        }
      }
    ...
    applyPending: function (isIronWillPending) {
      var game = this.game;
      var winterAndPA =
        game.challenges.getChallenge("postApocalypse").pending &&
        game.challenges.getChallenge("winterIsComing").pending &&
        !isIronWillPending &&
        game.time.getVSU("cryochambers").val;
      var WPA_warning = $I(
      ...
      game.ui.confirm(
        ...
        winterAndPA
          ? $I("challendge.btn.confirmation.msg") + WPA_warning
          : $I("challendge.btn.confirmation.msg"),
        function () {
          ...
          if (
            ...
            game.challenges.getChallenge("anarchy").pending &&
...
dojo.declare("classes.reserveMan", null, {
  ...
  calculateReserveResources: function () {
    var saveRatio =
      this.game.bld.get("chronosphere").val > 0
        ? this.game.getEffect("resStasisRatio")
        : 0;
    if (!saveRatio) {
      return;
    }
    ...
    for (var i in this.game.resPool.resources) {
      ...
      if (value > 0) {
        reserveResources[res.name] = Math.max(
          reserveResources[res.name] || 0,
          Math.min(value, Number.MAX_VALUE)
        );
      }
    }
  ...
  calculateReserves: function (isIronWillPending) {
    ...
    if (
      !this.game.challenges.getChallenge("postApocalypse").pending ||
      isIronWillPending
    ) {
  ...
  addReserves: function () {
    for (var i in this.reserveResources) {
      ...
      if (!resCap) {
        this.game.resPool.get(i).value = Math.min(
          this.game.resPool.get(i).value + this.reserveResources[i],
          Number.MAX_VALUE
        );
      } else {
        this.game.resPool.get(i).value = Math.max(
          this.game.resPool.get(i).value,
          this.reserveResources[i]
        );
      }
      delete this.reserveResources[i];
    }
  ...
  reservesExist: function () {
    return (
      Object.keys(this.reserveResources).length || this.reserveKittens.length
    );
  }
});
dojo.declare(
  "classes.ui.ChallengeBtnController",
  com.nuclearunicorn.game.ui.BuildingBtnController,
  {
    ...
    getDescription: function (model) {
      if (this.game.bld.get("chronosphere").val > 0) {
        var msgChronosphere =
          model.metadata.name == "ironWill"
            ? $I("challendge.btn.chronosphere.with.ironWill.desc")
            : $I("challendge.btn.chronosphere.desc");
      } else {
        var msgChronosphere = "";
      }
      return (
        this.inherited(arguments) +
        $I("challendge.btn.desc", [model.metadata.effectDesc, msgChronosphere])
      );
    },
    ...
    buyItem: function (model, event, callback) {
      ...
      callback(true /*itemBought*/, {
        reason:
          "item-is-free" /*We just toggled the pending state; simple, really*/
      });
    },
...
dojo.declare("classes.ui.ChallengePanel", com.nuclearunicorn.game.ui.Panel, {
  ...
  render: function (container) {
    ...
    dojo.forEach(this.game.challenges.challenges, function (challenge, i) {
      var button = new com.nuclearunicorn.game.ui.BuildingBtn(
        { id: challenge.name, controller: controller },
        self.game
      );
...
dojo.declare(
  "classes.ui.ChallengeEffectsPanel",
  com.nuclearunicorn.game.ui.Panel,
  {
    ...
    setChallengeName: function (challengeName) {
      ...
      this.name = $I("challendge.effects.panel.label", [
        this.game.challenges.getChallenge(challengeName).label
      ]);
    },

    render: function (container) {
      var content = this.inherited(arguments);
      this.listElement = dojo.create(
        "ul",
    ...
    generateEffectsList: function () {
      ...
      for (var effectName in challengeData.effects) {
        var displayParams = this.game.getEffectDisplayParams(
          effectName,
          challengeData.totalEffectsCached[effectName],
          false /*showIfZero*/
        );
        ...
        var textToDisplay =
          displayParams.displayEffectName +
          ": " +
          displayParams.displayEffectValue;
    ...
    update: function () {
      ...
      this.setVisible(
        Boolean(this.game.detailedChallengeInfo) &&
          this.listElement.hasChildNodes()
      );
    }
  }
);
...
dojo.declare("classes.tab.ChallengesTab", com.nuclearunicorn.game.ui.tab, {
  render: function (container) {
    this.challengesPanel = new classes.ui.ChallengePanel(
      $I("challendge.panel.label"),
    ...
    var reclaimReservesBtn = new com.nuclearunicorn.game.ui.ButtonModern(
      {
        ...
        controller: new com.nuclearunicorn.game.ui.ButtonController(this.game, {
          updateVisible: function (model) {
            model.visible =
              !this.game.challenges.anyChallengeActive() &&
              !this.game.ironWill &&
        ...
        controller: new com.nuclearunicorn.game.ui.ButtonController(this.game, {
          updateVisible: function (model) {
            ...
            if (effectsPanels) {
              ...
              model.visible = effectsPanels.some(
                function (challengeEffectsPanel) {
          ...
          getName: function () {
            return this.game.detailedChallengeInfo
              ? $I("challendge.effects.hide.label")
              : $I("challendge.effects.show.label");
          }
        })
      },
      this.game
    ...
    for (var i = 0; i < this.game.challenges.challenges.length; i += 1) {
      var effectPanel = new classes.ui.ChallengeEffectsPanel(
        "",
        this.game.challenges
      );

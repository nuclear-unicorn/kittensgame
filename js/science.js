 ...
 */
dojo.declare(
  "classes.managers.ScienceManager",
  com.nuclearunicorn.core.TabManager,
  {
    ...
    policies: [
        ...
        calculateEffects: function (self, game) {
          ...
          for (var i = 0; i < game.bld.buildingGroups.length; i++) {
            if (game.bld.buildingGroups[i].name == "population") {
              for (
                var k = 0;
                k < game.bld.buildingGroups[i].buildings.length;
                k++
              ) {
                var building = game.bld.getBuildingExt(
                  game.bld.buildingGroups[i].buildings[k]
                );
                if (
                  !game.resPool.isStorageLimited(
                    game.bld.getPrices(building.meta.name)
                  )
                ) {
                  uncappedHousing += 1;
                  building.meta.almostLimited =
                    self.researched &&
                    game.resPool.isStorageLimited(
                      game.bld.getPrices(building.meta.name, 1)
                    );
                } else {
      ...
      //----------------	industrial age --------------------
      {
        ...
        evaluateLocks: function (game) {
          return (
            (game.science.getPolicy("monarchy").researched ||
              game.science.getPolicy("republic").researched) &&
      ...
      },
      {
        ...
        evaluateLocks: function (game) {
          return (
            (game.science.getPolicy("republic").researched ||
              game.science.getPolicy("authocracy").researched) &&
            game.bld.getBuildingExt("factory").meta.val > 0
          );
        }
      },
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          ...
          if (
            game.religion.getPact("fractured").on >= 1 ||
      ...
      },
      {
        ...
        evaluateLocks: function (game) {
          return (
            game.science.getPolicy("diplomacy").researched &&
            game.science.get("astronomy").researched
          );
        }
      },
      {
      ...
      },
      {
        ...
        evaluateLocks: function (game) {
          return (
            game.science.getPolicy("isolationism").researched &&
            game.science.get("astronomy").researched &&
      ...
      },
      {
        ...
        evaluateLocks: function (game) {
          return (
            (game.science.getPolicy("stripMining").researched ||
              game.science.getPolicy("clearCutting").researched) &&
      ...
      },
      {
        ...
        evaluateLocks: function (game) {
          return (
            game.science.getPolicy("environmentalism").researched &&
            game.science.get("ecology").researched
          );
        }
      },
      {
      ...
      },
      {
        ...
        prices: [{ name: "manpower", val: 10000 }],
        unlocked: false,
        blocked: false,
    ...
    getPrices: function (tech) {
      ...
      prices_result = this.game.village.getEffectLeader(
        "scientist",
        prices_result
      );
    ...
    save: function (saveData) {
      saveData.science = {
        ...
        techs: this.filterMetadata(this.techs, [
          "name",
          "unlocked",
          "researched"
        ]),
        policies: this.filterMetadata(this.policies, [
          "name",
          "unlocked",
          "blocked",
          "researched"
...
dojo.declare(
  "classes.ui.PolicyBtnController",
  com.nuclearunicorn.game.ui.BuildingNotStackableBtnController,
  {
    ...
    shouldBeBought: function (model, game) {
      //fail checks:
      if (
        this.game.village.leader &&
        model.metadata.requiredLeaderJob &&
        ...
        this.game.msg(
          $I("msg.policy.wrongLeaderJobForResearch", [
        ...
        for (var i = 0; i < model.metadata.blocks.length; i++) {
          if (
            this.game.science.getPolicy(model.metadata.blocks[i]).researched
          ) {
...
dojo.declare("classes.ui.PolicyPanel", com.nuclearunicorn.game.ui.Panel, {
  ...
  render: function (container) {
    ...
    dojo.connect(groupCheckbox, "onclick", this, function () {
      this.game.science.policyToggleResearched =
        !this.game.science.policyToggleResearched;
    ...
    dojo.create(
      "label",
      {
        innerHTML: $I("science.policyToggleResearched.label") + "<br>",
        for: "policyToggleResearched"
      },
      this.toggleResearchedSpan
    );
    ...
    var groupCheckbox1 = dojo.create(
      "input",
      {
    ...
    dojo.connect(groupCheckbox1, "onclick", this, function () {
      this.game.science.policyToggleBlocked =
        !this.game.science.policyToggleBlocked;
    ...
    dojo.create(
      "label",
      {
        innerHTML: $I("science.policyToggleBlocked.label"),
        for: "policyToggleBlocked"
      },
      this.toggleBlockedSpan
    );
    ...
    dojo.forEach(this.game.science.policies, function (policy, i) {
      var button = new com.nuclearunicorn.game.ui.BuildingResearchBtn(
        {
          id: policy.name,
  ...
  update: function () {
    ...
    dojo.style(
      this.panelDiv,
      "display",
      dojo.some(allPolicies, function (policy) {
        return policy.unlocked;
      })
        ? "block"
        : "none"
    );
...
dojo.declare(
  ...
  com.nuclearunicorn.game.ui.BuildingNotStackableBtnController,
  {
    ...
    getPrices: function (model) {
      var prices_result = this.game.village.getEffectLeader(
        "scientist",
        this.inherited(arguments)
      );
...
dojo.declare(
  "com.nuclearunicorn.game.ui.tab.Library",
  com.nuclearunicorn.game.ui.tab,
  {
    ...
    render: function (tabContainer) {
      ...
      dojo.create(
        "label",
        {
          innerHTML: $I("science.toggleResearched.label"),
      ...
      this.policyPanel = new classes.ui.PolicyPanel(
        $I("policy.panel.label"),
        this.game.science
      );
      ...
      var showMetaphysics =
        this.game.science.get("metaphysics").researched &&
      ...
      if (showMetaphysics) {
        this.metaphysicsPanel = new classes.ui.PrestigePanel(
          $I("prestige.panel.label"),
          this.game.prestige
        );
      ...
      if (this.game.detailedPollutionInfo) {
        this.detailedPollutionInfo = dojo.create(
          "span",
    ...
    update: function () {
      ...
      if (this.game.detailedPollutionInfo) {
        if (this.detailedPollutionInfo) {
          ...
          this.detailedPollutionInfo.innerHTML =
            "Pollution is " +
            Math.floor(currentCathPollution) +
            " (" +
            this.game.getDisplayValueExt(currentCathPollution) +
            ") " +
            "<br>Polution per tick is " +
            Math.floor(currenCathPerTickPollution);
          var pollutionLevel = this.game.bld.getPollutionLevel();
          this.detailedPollutionInfo.innerHTML +=
            "<br>Pollution level is " + pollutionLevel;
          if (pollutionLevel >= 0) {
            this.detailedPollutionInfo.innerHTML +=
              "<br>Pollution future effects might be at this pollution level:";
            this.detailedPollutionInfo.innerHTML +=
              "<br>— Less catnip production";
            if (pollutionLevel >= 1) {
              this.detailedPollutionInfo.innerHTML +=
                "<br>— Less kitten happiness: " +
            ...
            if (pollutionLevel > 4) {
              this.detailedPollutionInfo.innerHTML +=
                "<br>— SR effect doesn't apply to wood and catnip";
            } else if (pollutionLevel >= 3) {
              this.detailedPollutionInfo.innerHTML +=
                "<br>— Less SR effect on wood and catnip";
            }
          }
          if (currenCathPerTickPollution < 0 && currentCathPollution) {
            var toZero =
              -currentCathPollution /
              currenCathPerTickPollution /
              this.game.calendar.ticksPerDay;
            this.detailedPollutionInfo.innerHTML +=
              "<br> To zero " + this.game.toDisplaySeconds(toZero.toFixed());
          } else if (currenCathPerTickPollution > 0) {
            var toNextLevel =
              (Math.pow(10, 1 + pollutionLevel) *
                this.game.bld.getPollutionLevelBase() -
                currentCathPollution) /
              currenCathPerTickPollution /
              this.game.calendar.ticksPerDay;
            this.detailedPollutionInfo.innerHTML +=
              "<br> To next level " +
              this.game.toDisplaySeconds(toNextLevel.toFixed());
          }
        }
      }
    },
    ...
    createTechBtn: function (tech) {
      var controller = new com.nuclearunicorn.game.ui.TechButtonController(
        this.game
      );
      var btn = new com.nuclearunicorn.game.ui.BuildingResearchBtn(
        { id: tech.name, controller: controller },
        this.game
      );
      return btn;
    }

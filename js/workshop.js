dojo.declare(
  "classes.managers.WorkshopManager",
  com.nuclearunicorn.core.TabManager,
  {
    ...
    upgrades: [
      ...
      //--------------------- hunt upgrades ----------------------
      {
        ...
        calculateEffects: function (self, game) {
          self.effects["manpowerJobRatio"] =
            0.5 * (1 + game.getEffect("weaponEfficency")); //weaponEfficency can't go beyond -1, see challenges.js for more info
        },
        prices: [
          { name: "wood", val: 200 },
          { name: "iron", val: 100 },
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          self.effects["manpowerJobRatio"] =
            0.25 * (1 + game.getEffect("weaponEfficency")); //weaponEfficency can't go beyond -1, see challenges.js for more info
        },
        prices: [
          { name: "iron", val: 1500 },
    ...
    save: function (saveData) {
      saveData.workshop = {
        ...
        upgrades: this.filterMetadata(this.upgrades, [
          "name",
          "unlocked",
          "researched"
        ]),
        crafts: this.filterMetadata(this.crafts, [
          "name",
          "unlocked",
          "value",
          "progress"
        ]),
        zebraUpgrades: this.filterMetadata(this.zebraUpgrades, [
          "name",
          "unlocked",
          "researched"
        ])
    ...
    getCraftPrice: function (craft) {
      ...
      if (
        craft.name == "manuscript" &&
        this.game.science.getPolicy("tradition").researched
      ) {
        return [
          ...
          prices[i].val =
            ...
            (1 -
              this.game.getLimitedDR(
                this.game.getEffect("satnavRatio") *
                  this.game.space.getBuilding("sattelite").on,
                0.75
              ));
        }
      }
    ...
    craft: function (res, amt, suppressUndo, forceAll, bypassResourceCheck) {
      ...
      if (bypassResourceCheck || this.game.resPool.hasRes(prices)) {
        ...
        if (!suppressUndo) {
          ...
          undo.addEvent(
            "workshop" /* TODO: use manager.id and pass it in proper way as manager constructor*/,
            {
              metaId: res,
              val: amt
            }
    ...
    getConsumptionEngineers: function () {
      var result = {};
      var kittenResProductionModifier = this.game.workshop.get("neuralNetworks")
        .researched
        ? 2
        : 1;
      ...
      for (var ind in this.crafts) {
        ...
        var kittenResProduction =
          this.game.village.getResProduction()["ES" + craft.name] || 0;
        if (!kittenResProduction) {
          continue;
        }
        var tierCraftRatio =
          this.game.getEffect("t" + craft.tier + "CraftRatio") || 0;
        if (tierCraftRatio == 0) {
          tierCraftRatio = 1;
        }
        ...
        var effectPerTick =
          (kittenResProduction * tierCraftRatio) /
          (600 * this.game.ticksPerSecond * craft.progressHandicap);
    ...
    getEffectEngineer: function (resName, afterCraft) {
      ...
      var kittenResProduction =
        (this.game.village.getResProduction()["ES" + resName] || 0) *
        (this.game.workshop.get("neuralNetworks").researched ? 2 : 1);
      ...
      return (
        effectPerTick *
        (1 + (afterCraft ? this.game.getResCraftRatio(resName) : 0))
      );
    },
    ...
    undo: function (data) {
      ...
      if (this.craft(metaId, -val, true /*do not create cyclic undo*/)) {
        ...
        this.game.msg(
          $I("workshop.undo.msg", [
            this.game.getDisplayValueExt(val * (1 + craftRatio)),
            res.title || res.name
          ])
        );
      }
    ...
    craftAll: function (craftName) {
      ...
      if (0 < minAmt && minAmt < Number.MAX_VALUE) {
        ...
        if (
          ...
          this.game.msg(
            $I("workshop.crafted.msg", [
              this.game.getDisplayValueExt(minAmt * (1 + craftRatio)),
              res.title || craftName
            ]),
            null,
            "craft"
    ...
    fastforward: function (daysOffset) {
      ...
      this.effectsBase["oilMax"] = Math.floor(
        this.game.resPool.get("tanker").value * 500
      );

      var scienceMaxCap = this.game.bld.getEffect("scienceMax");
      scienceMaxCap +=
        this.game.getEffect("pyramidSpaceCompendiumRatio") *
        this.game.space.getEffect("scienceMax"); //lets treat trasnfered science max from space same way
      ...
      if (this.game.prestige.getPerk("codexLeviathanianus").researched) {
        ...
        scienceMaxCap *=
          1 + 0.05 * ttBoostRatio * this.game.religion.transcendenceTier;
      }
      ...
      var compendiaScienceMax = Math.floor(
        this.game.resPool.get("compedium").value * 10
      );
      this.effectsBase["scienceMax"] = Math.min(
        compendiaScienceMax,
        scienceMaxCap
      );

      var cultureBonusRaw = Math.floor(
        this.game.resPool.get("manuscript").value
      );
      this.effectsBase["cultureMax"] = this.game.getUnlimitedDR(
        cultureBonusRaw,
        0.01
      );

      this.effectsBase["cultureMax"] *=
        1 + this.game.getEffect("cultureFromManuscripts");
    ...
    craftByEngineers: function (times) {
      for (var i = 0; i < this.crafts.length; i++) {
        ...
        var currentProgress =
          Math.max(craft.progress, 0) +
          times * this.getEffectEngineer(craft.name, false);
        if (currentProgress >= 1) {
          var currentProgress = Math.min(
            currentProgress,
            this._getCraftAllCountInternal(craft, prices)
          );
...
dojo.declare(
  "com.nuclearunicorn.game.ui.CraftButtonController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    ...
    getName: function (model) {
      ...
      if (
        this.game.science.get("mechanization").researched &&
        craft.value != 0
      ) {
        var progressDisplayed = this.game.toDisplayPercentage(
          craft.progress,
          0,
          true
        );
    ...
    getDescription: function (model) {
      ...
      if (craftBonus > 0) {
        desc +=
          "<br /><br />" +
          $I("workshop.craftBtn.desc.effectivenessBonus", [
            this.game.getDisplayValueExt(100 * craftBonus, false, false, 0)
          ]);
      }
      ...
      if (this.game.science.get("mechanization").researched) {
        desc +=
          "<br /><br />" +
          $I("workshop.craftBtn.desc.tier") +
          ": " +
          craft.tier;

        var tierBonus =
          this.game.getEffect("t" + craft.tier + "CraftRatio") || 1;
        if (tierBonus != 1) {
          desc +=
            "<br />" +
            $I("workshop.craftBtn.desc.craftRatio") +
            ": " +
            this.game.getDisplayValueExt(
              (100 * (tierBonus - 1)).toFixed(),
              true
            ) +
            "%";
        }
        ...
        if (craft.progressHandicap != 1) {
          var difficulty = this.game.getDisplayValueExt(
            (-100 * (1 - 1 / craft.progressHandicap)).toFixed(2),
            true
          );
          desc +=
            "<br />" +
            $I("workshop.craftBtn.desc.progressHandicap") +
            ": " +
            difficulty +
            "%";
        }
        ...
        if (craft.value != 0) {
          var countdown = (
            1 /
            (this.game.workshop.getEffectEngineer(craft.name, false) *
              this.game.getTicksPerSecondUI())
          ).toFixed(0);
          desc +=
            "<br />=> " + $I("workshop.craftBtn.desc.countdown", [countdown]);
        }
      }
      return desc;
    },

    assignCraftJob: function (model, value) {
      //TODO, assign one kitten, not just a value to manage with exp
      var craft = model.craft;

      var valueCorrected =
        this.game.village.getFreeEngineers() > value
          ? value
          : this.game.village.getFreeEngineers();
    ...
    unassignCraftJob: function (model, value) {
      //TODO, aunssign one kitten, not just a value to manage with exp
      var craft = model.craft;
...
dojo.declare(
  "com.nuclearunicorn.game.ui.CraftButton",
...
dojo.declare(
  "com.nuclearunicorn.game.ui.ZebraUpgradeButtonController",
  com.nuclearunicorn.game.ui.BuildingNotStackableBtnController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
...
dojo.declare(
  "com.nuclearunicorn.game.ui.tab.Workshop",
  com.nuclearunicorn.game.ui.tab,
  {
    ...
    render: function (tabContainer) {
      ...
      dojo.create(
        "span",
        {
          innerHTML: $I("workshop.craft.effectiveness", [
            this.game.getDisplayValueExt(
              100 * this.game.getCraftRatio(),
              false,
              false,
              0
            )
      ...
      dojo.create(
        "label",
        {
          innerHTML: $I("workshop.toggleResearched.label"),
          for: "toggleResearched"
        },
        div
      );
      //---------------------------------------------------------------------

      var upgradePanel = new com.nuclearunicorn.game.ui.Panel(
        $I("workshop.upgradePanel.label"),
        this.game.workshop
      );
      ...
      var craftPanel = new com.nuclearunicorn.game.ui.Panel(
        $I("workshop.craftPanel.label"),
        this.game.workshop
      );
      ...
      var tdTop = dojo.create(
        "td",
        { colspan: 2, style: { cursor: "pointer" } },
        td
      );
      ...
      var controller = new com.nuclearunicorn.game.ui.CraftButtonController(
        this.game
      );
      ...
      var zebraUpgradesPanel = new com.nuclearunicorn.game.ui.Panel(
        $I("workshop.zebraUpgradesPanel.label"),
        this.game.workshop
      );
      ...
      var td = dojo.create(
        "td",
        { className: "craftStuffPanel", style: { paddingLeft: "50px" } },
        table
      );
      this.resTd = td;
    ...
    renderResources: function (container) {
      ...
      dojo.create(
        "span",
        { innerHTML: $I("workshop.craftPanel.counts.label") + ":" },
        container
      );
      ...
      for (var i = 0; i < resources.length; i++) {
        ...
        if (res.craftable && res.value) {
          ...
          dojo.create(
            "td",
            { innerHTML: this.game.getDisplayValueExt(res.value) },
            tr
          );
        }
      }
    ...
    createBtn: function (upgrade) {
      var controller = new com.nuclearunicorn.game.ui.UpgradeButtonController(
        this.game
      );
      return new com.nuclearunicorn.game.ui.BuildingResearchBtn(
        { id: upgrade.name, controller: controller },
        this.game
      );
    },
    ...
    createZebraUpgradeBtn: function (upgrade) {
      var controller =
        new com.nuclearunicorn.game.ui.ZebraUpgradeButtonController(this.game);
      return new com.nuclearunicorn.game.ui.BuildingResearchBtn(
        { id: upgrade.name, controller: controller },
        this.game
      );
    ...
    update: function () {
      ...
      if (this.tdTop && this.game.science.get("mechanization").researched) {
        this.tdTop.innerHTML =
          $I("workshop.craftPanel.header.freeEngineers") +
          ": " +
          this.game.village.getFreeEngineers() +
          " / " +
          this.game.village.getWorkerKittens("engineer");
      } else {
        this.tdTop.innerHTML = "";
      }
    ...
    updateTab: function () {
      ...
      if (freeEngineers > 0) {
        this.tabName +=
          ...
          this.game.getDisplayValueExt(freeEngineers, false, false, 0) +
          ")</span>";
      }
      if (this.domNode) {

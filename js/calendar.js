...
dojo.declare("com.nuclearunicorn.game.Calendar", null, {
  ...
  cycleYearColors: ["#A00000", "#DBA901", "#14CD61", "#01A9DB", "#9A2EFE"],
  ...
  observeHandler: function () {
    ...
    var celestialBonus = this.game.workshop.get("celestialMechanics").researched
      ...
      : 1;
    var sciBonus =
      25 * celestialBonus * (1 + this.game.getEffect("scienceRatio"));
    ...
    if (this.game.science.get("astronomy").researched) {
      ...
      var chanceRatio =
        (this.game.prestige.getPerk("chronomancy").researched ? 1.1 : 1) *
        timeRatioBonus;
      var eventChance =
        (0.0025 + this.game.getEffect("starEventChance")) * chanceRatio;
      ...
      var starcharts =
        eventChance <= 1
          ? 1
          : Math.floor(eventChance) +
            (this.game.rand(10000) <
            (eventChance - Math.floor(eventChance)) * 10000
              ? 1
              : 0);
      sciBonus *= starcharts;
      ...
      if (sciGain > 0 && !isSilent) {
        this.game.msg(
          $I("calendar.msg.science", [this.game.getDisplayValueExt(sciGain)]),
          "",
          "astronomicalEvent",
          true
        );
      ...
      if (!isSilent) {
        this.game.msg(
          $I("calendar.msg.starchart", [starcharts]),
          "",
          "astronomicalEvent"
        );
      }
  ...
  observeTimeout: function () {
    ...
    if ((this.game.ironWill && rand <= 25) || rand <= autoChance * 100) {
      this.observeHandler();
    }
  }, //observeTimeout
  ...
  cycleEffectsFestival: function (effects) {
    if (
      this.game.prestige.getPerk("numeromancy").researched &&
  ...
  trueYear: function () {
    return (
      (this.day / this.daysPerSeason + this.season) / this.seasonsPerYear +
      this.year -
      this.game.time.flux
    );
  },
  ...
  darkFutureYears: function (withImpedance) {
    var darkFutureActualBeginning =
      this.darkFutureBeginning +
      (withImpedance ? this.game.getEffect("timeImpedance") : 0);
  ...
  tick: function () {
    ...
    var ticksPerYear =
      this.ticksPerDay * this.daysPerSeason * this.seasonsPerYear;
    if (this.day < 0) {
    ...
    if (this.day >= this.daysPerSeason) {
      ...
      if (this.season >= this.seasonsPerYear) {
        this.season = 0;
        this.year +=
          this.game.challenges.isActive("1000Years") && this.year >= 500
            ? 0
            : 1;
        newYear = true;
      }
  ...
  onNewDay: function () {
    if (this.festivalDays) {
      ...
      if (this.game.getEffect("festivalLuxuryConsumptionRatio")) {
        if (
          !this.game.resPool.get("furs").value ||
          !this.game.resPool.get("ivory").value ||
          !this.game.resPool.get("spice").value
        ) {
    ...
    var chanceRatio =
      (this.game.prestige.getPerk("chronomancy").researched ? 1.1 : 1) *
      timeRatioBonus;
    var unicornChanceRatio =
      (this.game.prestige.getPerk("unicornmancy").researched ? 1.1 : 1) *
      timeRatioBonus;
    ...
    if (this.day < 0) {
      ...
      this.game.resPool.addResEvent(
        "void",
        this.game.resPool.getVoidQuantity()
      ); // addResEvent because during Temporal Paradox
    } else {
      //------------------------- relic -------------------------
      this.game.resPool.addResPerTick(
        "relic",
        this.game.getEffect("relicPerDay")
      );
    }
    //------------------------- necrocorns pacts -------------------------
    //deficit changing
    ...
    if (
      this.game.religion.pactsManager.necrocornDeficit >=
      this.game.religion.pactsManager.fractureNecrocornDeficit
    ) {
    ...
    if (
      this.game.rand(10000) < baseChance + iwChance &&
      this.game.science.get("mining").researched
    ) {
      //0.1% chance of meteors
      ...
      if (mineralsGain > 0 || sciGain > 0) {
        ...
        if (mineralsGain > 0) {
          meteorMsg +=
            ", +" +
            this.game.getDisplayValueExt(mineralsGain) +
            " " +
            $I("resources.minerals.title");
        }
        if (sciGain > 0) {
          meteorMsg +=
            ", +" +
            this.game.getDisplayValueExt(sciGain) +
            " " +
            $I("resources.science.title");
        }
    ...
    if (this.game.challenges.isActive("pacifism")) {
      ...
      if (
        this.game.rand(100000) <= 17 * unicornChanceRatio &&
        unicorns.value < 2 &&
        animal.researched
      ) {
    ...
    if (this.game.ironWill) {
      ...
      if (!zebras.value && archery.researched) {
        ...
        zebras.value > 0 &&
        zebras.value <= this.game.karmaZebras &&
        this.game.karmaZebras > 0
      ) {
        var chanceModifier =
          this.game.workshop.getZebraUpgrade("darkBrew").researched &&
          this.festivalDays
            ? 2 + this.game.getEffect("festivalArrivalRatio")
            : 1; //bigger chance for zebras to arive after darkBrew is researched
      ...
      if (this.game.prestige.getPerk("zebraDiplomacy").researched) {
        zTreshold = Math.floor(0.1 * (this.game.karmaZebras + 1)); //5 - 10% of hunters will stay
      }
      if (this.game.prestige.getPerk("zebraCovenant").researched) {
        zTreshold = Math.floor(0.5 * (this.game.karmaZebras + 1)); //5 - 50% of hunters will stay
      }
    ...
    if (this.game.rand(10000) < riftChance * 10000 * unicornChanceRatio) {
      var unicornBonus =
        500 * (1 + this.game.getEffect("unicornsRatioReligion") * 0.1);
      this.game.msg(
        $I("calendar.msg.rift", [this.game.getDisplayValueExt(unicornBonus)]),
        "notice",
        "unicornRift"
      );
    ...
    aliChance *=
      1 +
      this.game.getLimitedDR(this.game.getEffect("alicornPerTickRatio"), 1.2);
    ...
    if (this.game.rand(10000) < meteorChance * 10000 * unicornChanceRatio) {
      var ivory =
        250 +
        this.game.rand(1500) * (1 + this.game.getEffect("ivoryMeteorRatio"));
      this.game.msg(
        $I("calendar.msg.ivoryMeteor", [this.game.getDisplayValueExt(ivory)]),
        "notice",
        "ivoryMeteor"
      );
  ...
  fastForward: function (daysOffset) {
    ...
    if (
      ...
      //blackSky should block rare astr. events
      var eventChance =
        (0.0025 + this.game.getEffect("starEventChance")) * chanceRatio;
      if (this.game.prestige.getPerk("astromancy").researched) {
        eventChance *= 2;
      ...
      var celestialBonus = this.game.workshop.get("celestialMechanics")
        .researched
        ? this.game.ironWill
          ? 1.6
          : 1.2
      ...
      var sciBonus =
        ...
        25 *
        celestialBonus *
        (1 + this.game.getEffect("scienceRatio"));
    ...
    var baseChance = 10 * chanceRatio;
    if (this.game.science.get("mining").researched) {
      //0.1% chance of meteors
    ...
    if (this.game.ironWill) {
      ...
      if (unicorns.value < 2 && archery.researched) {
        numberEvents = Math.round(
          (daysOffset * 17 * unicornChanceRatio) / 100000
        );
    ...
    var yearsOffset = Math.floor(
      daysOffset / (this.daysPerSeason * this.seasonsPerYear)
    );
    ...
    if (resPool.energyProd >= resPool.energyCons) {
      resPool.addResEvent(
        "antimatter",
        this.game.getEffect("antimatterProduction") * yearsOffset
      );
    }
    ...
    this.game.resPool.addResPerTick(
      "relic",
      this.game.getEffect("relicPerDay") * daysOffset
    );
    ...
    var daysBetweenParadox =
      daysInParadox +
      100 * Math.max(1, 100 / this.game.bld.get("chronosphere").on);
    var percentTimeInParadox = daysInParadox / daysBetweenParadox;

    this.game.resPool.addResEvent(
      "void",
      Math.floor(
        this.game.resPool.getVoidQuantityStatistically() *
          daysOffset *
          percentTimeInParadox
      )
    );

    // Adjust crypto price
    if (this.game.science.get("antimatter").researched) {
      var logIncrease = this.game.math.loopOrGaussianApproximation(
        daysOffset - 1,
        false,
        1.249927083463528e-6,
        1.4427062504448777e-10,
        this.game.math.log1p(-1 / 40000),
        this.game.math.log1p(1 / 40000),
        function () {
          var y = Math.random();
          return y < 0.3
    ...
    if (
      this.game.challenges.isActive("1000Years") &&
      this.year + yearsOffset > 500
    ) {
    ...
    var paragon =
      Math.floor((this.year + yearsOffset) / 1000) -
      Math.floor(this.year / 1000);
    if (paragon > 0) {
  ...
  onNewSeason: function () {
    ...
    if (this.year > 3) {
      ...
      if (
        this.getCurSeason().name == "winter" &&
        this.game.challenges.getChallenge("winterIsComing").researched &&
    ...
    if (
      this.season == 2 &&
      this.game.workshop.get("advancedAutomation").researched
    ) {
  ...
  getMilleniaChanged: function (startYear, endYear) {
    return Math.max(
      0,
      (Math.floor(endYear / 1000) * 1000 -
        Math.floor(startYear / 1000) * 1000) /
        1000
    );
  },
  ...
  onNewYears: function (updateUI, years, milleniumChangeCalculated) {
    ...
    if (milleniumChangeCalculated) {
      this.calculateMilleniumProduction(
        this.getMilleniaChanged(this.year - years, this.year)
      );
    }

    var pyramidVal = this.game.religion
      .getZU("blackPyramid")
      .getEffectiveValue(this.game);
    var markerVal = this.game.religion
      .getZU("marker")
      .getEffectiveValue(this.game);
    ...
    if (pyramidVal > 0) {
      if (
        this.game.rand(1000) <
        35 * pyramidVal * years * (1 + 0.1 * markerVal)
      ) {
...
*/
    ...
    if (resPool.energyProd >= resPool.energyCons) {
      resPool.addResEvent(
        "antimatter",
        this.game.getEffect("antimatterProduction") * years
      );
    }

    resPool.addResEvent(
      "temporalFlux",
      this.game.getEffect("temporalFluxProduction") * years
    );
      ...
      if (!this.game.getEffect("shatterTCGain")) {
        for (var i in this.game.resPool.resources) {
          ...
          if (res.aiCanDestroy) {
            resPool.addResEvent(
              res.name,
              -res.value * (1 - Math.pow(1 - 0.01 * aiApocalypseLevel, years))
            ); //hopefully this will work close enough
          }
        }
      }
    }
  ...
  onNewYear: function (updateUI) {
    ...
    if (resPool.energyProd >= resPool.energyCons) {
      resPool.addResEvent(
        "antimatter",
        this.game.getEffect("antimatterProduction")
      );
    }

    resPool.addResEvent(
      "temporalFlux",
      this.game.getEffect("temporalFluxProduction")
    );
  ...
  adjustCryptoPrice: function () {
    if (this.game.science.get("antimatter").researched) {
      ...
      this.cryptoPrice +=
        (this.cryptoPrice * (1 - (this.game.rand(10) % 3)) * Math.random()) /
        40000;
  ...
  getWeatherMod: function (res) {
    var mod = this.getCurSeason().modifiers[res.name]
      ? this.getCurSeason().modifiers[res.name]
      : 1;
    ...
    if (
      this.game.science.getPolicy("communism").researched &&
      this.getCurSeason().name == "winter" &&
      this.weather == "cold"
    ) {
      return 0;
    ...
    if (
      this.game.challenges.getChallenge("winterIsComing").on &&
      this.weather == "cold"
    ) {
  ...
  getCurSeasonTitle: function () {
    var winterIsComingNumeral = this.game.challenges.isActive("winterIsComing")
      ? this.getWinterIsComingNumeral()
      : "";
  ...
  load: function (saveData) {
    if (saveData.calendar) {
      ...
      this.futureSeasonTemporalParadox =
        saveData.calendar.futureSeasonTemporalParadox || -1;

		...
		buildings: [{
			...
			calculateEffects: function(self, game) {
				self.effects = {
					...
					"starchartPerTickBaseSpace": game.challenges.getChallenge("blackSky").on ? 0 : 0.01
				};
            },
			unlockScheme: {
				name: "vessel",
    ...
    getPrices: function(model) {
		...
		if (this.game.challenges.getChallenge("blackSky").on
		 && model.metadata.name == "orbitalLaunch") {
			for (var i = 0; i < prices.length; i++) {
				prices[i].val *= prices[i].name == "starchart" ? 0 : 11 * this.game.challenges.getChallengeEffect("blackSky");
			}
		}
...
dojo.declare("classes.ui.space.PlanetBuildingBtnController", com.nuclearunicorn.game.ui.BuildingStackableBtnController, {
	...
	getPrices: function(model) {
		...
		if (this.game.challenges.getChallenge("blackSky").on
		 ...
		 && meta.val == 0) {
			for (var i = 0; i < prices.length; i++) {
				prices[i].val *= prices[i].name == "starchart" ? 0 : 14 * this.game.challenges.getChallengeEffect("blackSky");
			}
		}

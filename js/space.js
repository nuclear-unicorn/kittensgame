  ...
  com.nuclearunicorn.core.TabManager,
  {
        ...
        buildings: [
          ...
          },
          {
            ...
            calculateEffects: function (self, game) {
              ...
              var observatoryRatioTemp =
                0.05 * (1 + game.getEffect("satelliteSynergyBonus"));
              self.effects = {
              ...
              if (game.challenges.isActive("blackSky")) {
                self.effects["starchartPerTickBaseSpace"] *=
                  1 / (1 + game.getEffect("bskSattelitePenalty"));
              }
        ...
        buildings: [
          {
            ...
            calculateEffects: function (self, game) {
              var effects = {
                ...
                unobtainiumPerTickSpace:
                  0.007 * (1 + game.getEffect("lunarOutpostRatio"))
              };
            ...
            action: function (self, game) {
              ...
              self.effects["unobtainiumPerTickSpace"] =
                0.007 * (1 + game.getEffect("lunarOutpostRatio"));
          ...
          },
          {
            ...
            calculateEffects: function (self, game) {
              ...
              effects["energyConsumption"] = game.workshop.get("amBases")
                .researched
                ? 5
                : 10;
              ...
              if (game.workshop.get("aiBases").researched) {
                ...
                for (var key in effects) {
                  if (key != "energyConsumption") {
                    effects[key] *=
                      1 + game.bld.get("aiCore").on * 0.1 * aiBasesModifier;
                  }
                }
              }
              self.effects = effects;
        ...
        buildings: [
          {
            ...
            calculateEffects: function (self, game) {
              self.effects = {
                ...
                uraniumMax:
                  1750 * (1 + game.getEffect("planetCrackerStorageBonus"))
              };
            }
          },
          {
        ...
        buildings: [
          {
            ...
            calculateEffects: function (self, game) {
              self.effects = {
                ...
                starchartPerTickBaseSpace: game.challenges.isActive("blackSky")
                  ? 0
                  : 0.01
              };
            },
        ...
        buildings: [
          ...
          },
          {
            ...
            calculateEffects: function (self, game) {
              var effects = {
                antimatterMax:
                  100 * (1 + game.space.getBuilding("heatsink").val * 0.02),
                energyConsumption:
                  50 * (1 + game.space.getBuilding("heatsink").val * 0.01)
              };
        ...
        buildings: [
          {
            ...
            action: function (self, game) {
              ...
              var rrBoost =
                1 +
                game.getEffect("relicRefineRatio") *
                  game.religion.getZU("blackPyramid").getEffectiveValue(game) *
                  0.1; //10% per BP * BN combo
              ...
              var entBoost =
                1 +
                game.space.getBuilding("entangler").effects["hashRateLevel"] *
                  0.25; //25% per entangler hashrate
        ...
        buildings: [
          {
            ...
            action: function (self, game) {
              self.effects["maxKittens"] =
                1 + game.getEffect("terraformingMaxKittensRatio");
            },
            unlocks: {
              tabs: ["village"]
            },
          ...
          },
          {
            ...
            updateEffects: function (self, game) {
              ...
              // 100 HP = +100%
              // 300 HP = +200%
              self.effects["terraformingMaxKittensRatio"] =
                game.getUnlimitedDR(self.on, 100) / self.on;
        ...
        buildings: [
          {
            ...
            action: function (self, game) {
              ...
              if (
                game.resPool.get("gflops").value < gflopsPerTick &&
                game.resPool.get("gflops").value > 0
              ) {
              ...
              self.effects["nextHashLevelAt"] =
                difficulty * Math.pow(rate, self.effects["hashRateLevel"] + 1);
              self.effects["hrProgress"] =
                hr /
                (difficulty *
                  Math.pow(rate, self.effects["hashRateLevel"] + 1));
              if (hr > difficulty) {
                self.effects["hashRateLevel"] = Math.floor(
                  Math.log(hr / difficulty) / Math.log(rate)
                );
              } else {
    ...
    constructor: function (game) {
      ...
      for (var i in this.planets) {
        ...
        this.registerMeta(false, planet.buildings, {
          getEffect: function (building, effectName) {
            if (!building.effects) {
              ...
              if (
                effectName === "hashrate" ||
                effectName === "hashRateLevel" ||
                effectName === "nextHashLevelAt" ||
                effectName === "hrProgress"
              ) {
              ...
              var spaceRatio =
                effectName == "spaceRatio" &&
                game.resPool.energyCons > game.resPool.energyProd
                  ? game.resPool.getEnergyDelta()
                  : 1;
    ...
    save: function (saveData) {
      var planets = this.filterMetadata(this.planets, [
        "name",
        "buildings",
        "reached",
        "unlocked",
      ...
      for (var i = 0; i < planets.length; i++) {
        ...
        if (planet.buildings) {
          planet.buildings = this.filterMetadata(planet.buildings, [
            "name",
            "val",
            "on",
            "unlocked"
      ...
      saveData.space = {
        ...
        programs: this.filterMetadata(this.programs, [
          "name",
          "val",
          "on",
          "unlocked"
      ...
      for (var i in this.planets) {
        ...
        if (!planet.reached && planet.unlocked) {
          if (planet.routeDays > 0) {
            var routeSpeed =
              this.game.getEffect("routeSpeed") != 0
          ...
          if (planet.routeDays > 0) {
            var routeSpeed =
              this.game.getEffect("routeSpeed") != 0
                ...
                : 1;
            planet.routeDays -=
              (times * routeSpeed) / this.game.calendar.ticksPerDay;
          }
        }
      }
    ...
    getAutoProductionRatio: function (useTransferBonus) {
      ...
      if (useTransferBonus) {
        ratio *=
          1 +
          (this.game.bld.getAutoProductionRatio(false, 0.05) - 1) *
            this.game.getEffect("prodTransferBonus");
      }
...
dojo.declare(
  ...
  com.nuclearunicorn.game.ui.BuildingStackableBtnController,
  {
    ...
    getPrices: function (model) {
      ...
      for (var i = 0; i < prices.length; i++) {
        if (prices[i].name == "oil") {
          var reductionRatio = this.game.getLimitedDR(
            this.game.getEffect("oilReductionRatio"),
            0.75
          );
    ...
    updateEnabled: function (model) {
      var meta = model.metadata;
      if (meta.val != 0 && meta.on == 0) {
        //This checks for if the mission is "in progress"
        model.enabled = false;
        return;
      }
    ...
    build: function (model, maxBld) {
      ...
      if (model.metadata.name == "rorschachMission") {
        model.metadata.on = 1;
        this.game.msg(
          $I("space.mission.rorschach.complete.log.msg"),
          "important"
        );
      }
      return counter;
    ...
    getPrices: function (model) {
      ...
      for (var i = 0; i < prices.length; i++) {
        ...
        var resPriceDiscount = this.game.getLimitedDR(
          this.game.getEffect(prices[i].name + "CostReduction"),
          1
        );
...
dojo.declare("classes.ui.space.PlanetPanel", com.nuclearunicorn.game.ui.Panel, {
  ...
  render: function () {
    ...
    var controller = new classes.ui.space.PlanetBuildingBtnController(
      this.game
    );
    dojo.forEach(this.planet.buildings, function (building, i) {
      var button = new com.nuclearunicorn.game.ui.BuildingStackableBtn(
        { id: building.name, planet: self.planet, controller: controller },
        self.game
      );
  ...
  update: function () {
    if (
      !this.planet.reached &&
      this.planet.unlocked &&
      this.planet.routeDays > 0
    ) {
      var routeSpeed =
        this.game.getEffect("routeSpeed") != 0
          ? this.game.getEffect("routeSpeed")
      ...
      this.title.innerHTML =
        this.name +
        " | ETA: " +
        this.game.toDisplayDays(Math.round(this.planet.routeDays / routeSpeed));
    } else {
...
dojo.declare(
  "classes.ui.space.FurthestRingPanel",
  [classes.ui.space.PlanetPanel],
  {
...
dojo.declare(
  "com.nuclearunicorn.game.ui.tab.SpaceTab",
  com.nuclearunicorn.game.ui.tab,
  {
    ...
    constructor: function () {
      //------------ void stuff --------------
      this.aPanel = new com.nuclearunicorn.game.ui.Panel(
        $I("space.panel.rorshach.label")
      );
    ...
    render: function (container) {
      ...
      dojo.create(
        "label",
        {
          innerHTML: $I("space.tab.hide.complete.missions"),
      ...
      this.GCPanel = new com.nuclearunicorn.game.ui.Panel(
        $I("space.ground.control.label"),
        this.game.space
      );
      var content = this.GCPanel.render(container);
      var controller = new com.nuclearunicorn.game.ui.SpaceProgramBtnController(
        this.game
      );
      dojo.forEach(this.game.space.programs, function (program, i) {
        var button = new com.nuclearunicorn.game.ui.BuildingStackableBtn(
          { id: program.name, controller: controller },
          self.game
        );
        ...
        if (planet.unlocked) {
          if (this.game.prestige.getPerk("numerology").researched) {
            ...
            dojo.forEach(this.game.calendar.cycles, function (cycle, i) {
              if (
                cycle.name == planet.name ||
                (planet.name == "moon" && cycle.name == "redmoon")
              ) {
          ...
          if (planet.name == "furthestRing") {
            planetPanel = new classes.ui.space.FurthestRingPanel(
              planetTitle,
              self.game.space,
              self.game
            );
          } else {
            planetPanel = new classes.ui.space.PlanetPanel(
              planetTitle,
              self.game.space
            );
          }

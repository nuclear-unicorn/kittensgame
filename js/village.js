dojo.declare(
  "classes.managers.VillageManager",
  com.nuclearunicorn.core.TabManager,
  {
    kittens: 0,
          ...
          if (game.challenges.isActive("atheism")) {
            ...
            for (var i in game.village.sim.kittens) {
              ...
              if (kitten.job == "priest") {
                ...
                console.warn(
                  "Kitten was unasigned from being a priest in atheism! " +
                    kitten.name +
                    " " +
                    kitten.surname
                );
    ...
    getEffectLeader: function (trait, defaultObject) {
      ...
      if (this.leader) {
        ...
        if (leaderTrait == trait) {
          var burnedParagonRatio =
            1 + this.game.prestige.getBurnedParagonRatio();
          ...
          switch (trait) {
            ...
            case "scientist": // Science prices bonus
              for (var i = 0; i < defaultObject.length; i++) {
                if (defaultObject[i].name == "science") {
                  defaultObject[i].val -=
                    defaultObject[i].val *
                    this.game.getLimitedDR(
                      0.05 * burnedParagonRatio * leaderRatio,
                      1.0
                    ); //5% before BP
                }
              }
            ...
            case "wise": // Religion bonus
              for (var i = 0; i < defaultObject.length; i++) {
                if (
                  defaultObject[i].name == "faith" ||
                  defaultObject[i].name == "gold"
                ) {
                  defaultObject[i].val -=
                    defaultObject[i].val *
                    this.game.getLimitedDR(
                      (0.09 + 0.01 * burnedParagonRatio) * leaderRatio,
                      1.0
                    ); //10% before BP
                }
              }
    ...
    assignJob: function (job, amt) {
      ...
      amt = Math.min(
        amt,
        this.getFreeKittens(),
        this.getJobLimit(job.name) - jobRef.value
      );
    ...
    calculateSimMaxKittens: function () {
      ...
      var withRatioMaxKittens = Math.round(
        this.maxKittens * (1 + this.game.getLimitedDR(maxKittensRatio, 1))
      );
    ...
    update: function () {
      ...
      if (this.sim.getKittens() > 0) {
        if (
          resDiff < 0 ||
          (this.game.challenges.isActive("winterIsComing") &&
            this.sim.getKittens() > this.sim.maxKittens &&
            this.game.calendar.weather == "cold")
        ) {
          ...
          if (starvedKittens > 0 && this.deathTimeout <= 0) {
            ...
            if (resDiff < 0) {
              this.game.msg(
                starvedKittens +
                  (starvedKittens === 1
                    ? " " + $I("village.msg.kitten") + " "
                    : " " + $I("village.msg.kittens") + " ") +
                  $I("village.msg.starved")
              );
            } else {
              this.game.msg(
                starvedKittens +
    ...
    getFreeKittens: function () {
      ...
      var diligentKittens = this.game.challenges.isActive("anarchy")
        ? Math.round(
            this.getKittens() * (1 - this.game.getEffect("kittenLaziness"))
          ) //LDR specified in challenges.js
        : this.getKittens();
    ...
    getResProduction: function () {
      ...
      if (zebras.value > 1) {
        var zebraPreparations = Math.floor(
          this.game.getEffect("zebraPreparations")
        );
        res["manpower"] += this.game.getLimitedDR(
          (zebras.value - 1) * 0.05,
          2 + zebraPreparations * 0.05
        );
      }
    ...
    updateResourceProduction: function () {
      ...
      for (var i in this.sim.kittens) {
        ...
        if (
          kitten.isLeader &&
          theocracy.researched &&
          kitten.job != theocracy.requiredLeaderJob
        ) {
          kitten.isLeader = false;
          this.game.village.leader = null;
          var jobTitle = this.game.village.getJob(
            theocracy.requiredLeaderJob
          ).title;
          this.game.msg(
            $I("msg.policy.wrongLeaderJobDemoted", [theocracy.label, jobTitle]),
            "important"
          );
        }
        if (kitten.job) {
          ...
          if (job) {
            ...
            for (var jobResMod in job.modifiers) {
              var diff =
                job.modifiers[jobResMod] + job.modifiers[jobResMod] * mod;
              ...
              if (diff > 0) {
                ...
                if (!kitten.isLeader && this.game.village.leader) {
                  diff *=
                    1 +
                    (this.getLeaderBonus(this.game.village.leader.rank) - 1) *
            ...
            if (
              job.name == "engineer" &&
              typeof kitten.engineerSpeciality != "undefined" &&
              ...
              var automationBonus =
                this.game.getEffect(
                  kitten.engineerSpeciality + "AutomationBonus"
                ) || 0;
              var diff = 1 + automationBonus;

              var rankDiff =
                this.game.workshop.getCraft(kitten.engineerSpeciality).tier -
                kitten.rank;
              if (rankDiff > 0) {
    ...
    getResConsumption: function () {
      ...
      var philosophyLuxuryModifier =
        (1 + this.game.getEffect("luxuryDemandRatio")) *
        (1 +
          (this.game.calendar.festivalDays
            ? this.game.getEffect("festivalLuxuryConsumptionRatio")
            : 0));
      var res = {
    ...
    save: function (saveData) {
      ...
      for (var i in this.sim.kittens) {
        var _kitten = this.sim.kittens[i].save(
          this.game.opts.compressSaveFile,
          this.jobNames
        );
      ...
      saveData.village = {
        ...
        biomes: this.filterMetadata(this.map.biomes, [
          "name",
          "unlocked",
          "level",
          "cp"
    ...
    load: function (saveData) {
      if (saveData.village) {
        ...
        this.sim.hadKittenHunters =
          saveData.village.hadKittenHunters === undefined
            ? true
            : saveData.village.hadKittenHunters;
    ...
    getUnhappiness: function () {
      ...
      return (
        (this.getKittens() - 5) *
        populationPenalty *
        (1 + this.game.getEffect("unhappinessRatio"))
      );
    },
    ...
    getEnvironmentEffect: function () {
      ...
      return (
        game.getEffect("environmentHappinessBonus") +
        game.getEffect("environmentUnhappiness") +
        game.bld.pollutionEffects["pollutionHappines"]
      );
    },
    ...
    updateHappines: function () {
      ...
      for (var i = resources.length - 1; i >= 0; i--) {
        if (resources[i].type != "common" && resources[i].value > 0) {
          ...
          if (
            resources[i].name == "elderBox" &&
            this.game.resPool.get("wrappingPaper").value
          ) {
    ...
    huntAll: function () {
      ...
      if (
        squads >= 1000 &&
        !this.game.challenges.getChallenge("pacifism").unlocked
      ) {
    ...
    gainHuntRes: function (squads) {
      var unicorns = this.game.resPool.addResEvent(
        "unicorns",
        this.game.math.binominalRandomInteger(squads, 0.05)
      );
      if (unicorns > 0) {
        var unicornMsg =
          unicorns == 1
            ? $I("village.new.one.unicorn")
            : $I("village.new.many.unicorns", [
                this.game.getDisplayValueExt(unicorns)
              ]);
      ...
      if (this.game.resPool.get("zebras").value >= 10) {
        ...
        var bloodstone = this.game.resPool.addResEvent(
          "bloodstone",
          this.game.math.binominalRandomInteger(
            squads,
            (this.game.resPool.get("bloodstone").value == 0 ? 0.05 : 0.0005) *
              bloodstoneRatio
          )
        );
      ...
      var hunterRatio =
        this.game.getEffect("hunterRatio") +
        this.game.village.getEffectLeader("manager", 0);
      ...
      if (this.game.ironWill && this.game.workshop.get("goldOre").researched) {
        ...
        var gold = this.game.resPool.addResEvent(
          "gold",
          5 * this.game.math.irwinHallRandom(goldHunts) +
            5 * hunterRatio * this.game.math.irwinHallRandom(goldHunts)
        );
        if (gold > 0) {
          this.game.msg(
            $I("village.msg.hunt.gold", [this.game.getDisplayValueExt(gold)]),
            null,
            "hunt",
            true
          );
      ...
      var ivoryHunts = this.game.math.binominalRandomInteger(
        squads,
        0.45 + 0.02 * hunterRatio
      );
      var ivory = this.game.resPool.addResEvent(
        "ivory",
        50 * this.game.math.irwinHallRandom(ivoryHunts) +
          40 * hunterRatio * this.game.math.irwinHallRandom(ivoryHunts)
      );
      if (ivory > 0) {
        this.game.msg(
          $I("village.msg.hunt.ivory", [this.game.getDisplayValueExt(ivory)]),
          null,
          "hunt",
          true
        );
      ...
      var furs = this.game.resPool.addResEvent(
        "furs",
        80 * this.game.math.irwinHallRandom(squads) +
          65 * hunterRatio * this.game.math.irwinHallRandom(squads)
      );
      if (furs > 0) {
        this.game.msg(
          $I("village.msg.hunt.furs", [this.game.getDisplayValueExt(furs)]),
          null,
          "hunt",
          true
        );
    ...
    holdFestival: function (amt) {
      ...
      if (this.game.prestige.getPerk("carnivals").researched) {
        this.game.calendar.festivalDays +=
          this.game.calendar.daysPerSeason *
          this.game.calendar.seasonsPerYear *
          amt;
      } else {
        this.game.calendar.festivalDays =
          this.game.calendar.daysPerSeason * this.game.calendar.seasonsPerYear;
      }
    ...
    optimizeJobs: function () {
      ...
      for (var i = this.game.village.sim.kittens.length - 1; i >= 0; i--) {
        ...
        if (job && job != "engineer") {
          // don't optimize engineers, headaches lie that way
      ...
      if (Object.getOwnPropertyNames(situationJobs).length !== 0) {
        ...
        if (
          ...
          this.game.science.getPolicy("theocracy").researched
        ) {
          //hack for theocracy; so that it stop being soo annoying
    ...
    promoteKittens: function () {
      ...
      for (var i = 0; i < this.sim.kittens.length; i++) {
        ...
        if (this.sim.kittens[i].engineerSpeciality != null) {
          tier = this.game.workshop.getCraft(
            this.sim.kittens[i].engineerSpeciality
          ).tier;
          if (this.sim.kittens[i].rank >= tier) {
            //if engineer already have required rank, it go to common pool
            tier = -1;
          }
        }
      ...
      if (candidates.length) {
        ...
        for (var i = 0; i < candidates.length; i++) {
          var promoted = this.sim.promote(
            candidates[i].kitten,
            candidates[i].rank > 0 ? candidates[i].rank : undefined
          );
          if (promoted > 0) {
        ...
        if (promotedKittensCount == 0) {
          this.game.msg(
            $I(
              noGold
                ? "village.kittens.promotion.nogold"
                : "village.kittens.have.best.rank"
            )
          );
          ...
          this.game.msg(
            $I("village.leader.promoted.many.kittens", [promotedKittensCount])
          );
        }
      }
    },
...
dojo.declare("classes.village.Map", null, {
  ...
  biomes: [
    ...
    },
    {
      ...
      evaluateLocks: function (game) {
        return (
          game.village.getBiome("plains").level >= 5 ||
          game.village.getBiome("forest").level >= 5
        );
      },
      lore: {
    ...
    },
    {
      ...
      evaluateLocks: function (game) {
        return (
          game.village.getBiome("forest").level >= 25 &&
          game.village.getBiome("rainForest").level >= 5
        );
      },
      lore: {
    ...
    },
    {
      ...
      title: "Swamp",
      description:
        "Everything that is edible is poisonous and so are the trees and the grass and the air is also poisonous slightly",
      terrainPenalty: 1.95,
      lore: {
  ...
  toLevel: function (biome) {
    return (
      100 *
      (1 + 1.1 * Math.pow(biome.level + 1, 1.18 + 0.1 * biome.terrainPenalty))
    );
  },
  ...
  explore: function (biomeId) {
    ...
    if (biome.fauna && biome.fauna.length) {
      ...
      if (this.hp <= 0) {
        ...
        this.game.msg(
          "All contact with the expedition have been lost",
          "important",
          "explore"
        );
      }
    ...
    if (this.energy <= 0) {
      this.currentBiome = null;
      this.game.msg(
        "Your explorers have returned from expedition",
        "important",
        "explore"
      );
    }
  ...
  getBiomeRewards: function (biome) {
    ...
    for (var i in rewards) {
      ...
      var resourcePassedNormalTradeAmount =
        this.game.math.binominalRandomInteger(1, reward.chance);
      var fuzzedNormalAmount = this.game.diplomacy._fuzzGainedAmount(
        resourcePassedNormalTradeAmount,
        reward.width
      );
  ...
  updateEffectCached: function () {
    this.game.globalEffectsCached["mapPriceReduction"] =
      -this.getPriceReduction();
...
dojo.declare(
  "classes.ui.village.BiomeBtnController",
      ...
      return function (controller, model) {
        ...
        if (biomeMeta.fauna) {
          for (var i in biomeMeta.fauna) {
            ...
            var nameSpan = dojo.create(
              "span",
              {
                innerHTML:
                  fauna.title /*+ " | a:" + fauna.atk + ", d:" + fauna.def*/,
                style: { float: "left" }
              },
              faunaNode
            );
            var statsSpan = dojo.create(
              "span",
              {
...
dojo.declare(
  "classes.village.ui.map.UpgradeHQController",
...
dojo.declare(
  "classes.village.ui.map.UpgradeExplorersController",
  com.nuclearunicorn.game.ui.BuildingStackableBtnController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
...
dojo.declare(
  "classes.village.ui.MapOverviewWgt",
  [mixin.IChildrenAware, mixin.IGameAware],
  {
    constructor: function (game) {
      this.setGame(game);
      ...
      for (var i in game.village.map.biomes) {
        ...
        this.addChild(
          new classes.ui.village.BiomeBtn(
            {
              id: biome.name,
              name: biome.title,
      ...
      this.upgradeExplorersBtn = new com.nuclearunicorn.game.ui.ButtonModern(
        {
          ...
          controller: new classes.village.ui.map.UpgradeExplorersController(
            this.game
          )
        },
        this.game
    ...
    render: function (container) {
      ...
      dojo.create(
        "div",
        { innerHTML: "Biomes", style: { paddingBottom: "10px" } },
        div
      );
      ...
      this.explorerDiv = dojo.create(
        "div",
        {
          innerHTML:
            "Explorers: lvl 0, HP: " + map.hp.toFixed(1) + "/" + map.getMaxHP()
        },
        div
      );
    ...
    update: function () {
      ...
      var biome = map.currentBiome
        ? this.game.village.getBiome(map.currentBiome)
        : null;
      ...
      this.explorerDiv.innerHTML =
        "Explorers: HP: " + map.hp.toFixed(1) + "/" + map.getMaxHP().toFixed(1);
      this.inherited(arguments);
    }
  }
);
...
dojo.declare("classes.village.KittenSim", null, {
  ...
  update: function (kittensPerTick, times) {
    ...
    var hgSkillModifier =
      this.kittens.length <= this.maxKittens
        ? 1 /
          (1 +
            this.game.getLimitedDR(this.game.getEffect("maxKittensRatio"), 1))
        : 1; //overcrowding makes it not work
    var baseSkillXP = game.workshop.get("internet").researched
      ? Math.max((this.getKittens() * hgSkillModifier) / 10000, 0.01)
      : 0.01;
    ...
    for (var i = this.kittens.length - 1; i >= 0; i--) {
      ...
      if (
        kitten.job &&
        game.calendar.day >= 0 &&
        !game.challenges.isActive("anarchy")
      ) {
        ...
        if (kitten.job != "engineer" || kitten.engineerSpeciality != null) {
          // Engineers who don't craft don't learn
          if (kitten.skills[kitten.job] < skillsCap) {
            kitten.skills[kitten.job] = Math.min(
              kitten.skills[kitten.job] + skillXP,
              skillsCap
            );
          }
          kitten.exp += skillXP;
  ...
  assignJob: function (job, amt) {
    var freeKittens = [];
    var optimizeJobs =
      this.game.workshop.get("register").researched && this.game.village.leader;
    ...
    if (optimizeJobs) {
      ...
      freeKittens.sort(function (a, b) {
        return b.val - a.val || b.leader - a.leader;
      });
    }
  ...
  removeJob: function (job, amt) {
    ...
    if (optimizeJobs) {
      ...
      jobKittens.sort(function (a, b) {
        return a.val - b.val || a.leader - b.leader;
      });
    }
  ...
  unassignCraftJob: function (craft) {
    var optimization =
      this.game.workshop.get("register").researched &&
      this.game.village.leader != undefined
        ? true
        : false;
    ...
    if (jobKittens.length) {
      this.kittens[jobKittens[jobKittens.length - 1].id].engineerSpeciality =
        null;
      return true;
    } else {
      return false;
  ...
  promote: function (kitten, rank) {
    ...
    var goldToPromote = this.goldToPromote(
      kittenRank,
      rank,
      this.game.resPool.get("gold").value
    );
    ...
    if (expToPromote[0] && goldToPromote[0]) {
      ...
      // If rank is unreachable, try one rank
  ...
  clearJobs: function (hard) {
    for (var i = this.kittens.length - 1; i >= 0; i--) {
      var kitten = this.kittens[i];
      if (hard || kitten.job != "engineer") {
        // only fire engineers if hard flag is passed
        kitten.job = null;
        kitten.engineerSpeciality = null;
      }
    }
...
dojo.declare(
  "com.nuclearunicorn.game.ui.JobButtonController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
    ...
    updateEnabled: function (model) {
      this.inherited(arguments);
      if (
        !this.game.village.hasFreeKittens() ||
        this.game.village.getJobLimit(model.options.job) <=
          this.game.village.getWorkerKittens(model.options.job)
      ) {
        model.enabled = false;
      }
    },
      ...
      if (event.ctrlKey || event.metaKey /*osx tears*/) {
        ...
        if (this.game.opts.noConfirm) {
          ...
          this.game.ui.confirm(
            "",
            $I("village.btn.assignall.confirmation.msg"),
            function () {
...
dojo.declare(
  "com.nuclearunicorn.game.ui.JobButton",
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    renderLinks: function () {
...
dojo.declare("classes.ui.village.Census", null, {
  ...
  render: function (container) {
    ...
    //--------------------------------------------
    var navbar = dojo.create(
      "div",
      {
        className: "censusFilters",
        style: {
          height: "24px"
        }
      },
    ...
    dojo.create(
      "option",
      { value: "", innerHTML: $I("village.trait.filter.all") },
      traitSelect
    );
    ...
    dojo.create(
      "option",
      { value: "", innerHTML: $I("village.census.filter.all") },
      jobSelect
    );
    ...
    for (var i = sim.kittens.length - 1; i >= 0 && kittensLimit < 10; i--) {
      ...
      if (
        (this.filterJob && kitten.job !== this.filterJob) ||
        (this.filterTrait && kitten.trait.name !== this.filterTrait)
      ) {
        continue;
      }
      ...
      var unassignHref = dojo.create(
        "a",
        {
          href: "#",
          innerHTML: $I("village.btn.unassign.job"),
        ...
        dojo.connect(
          ...
          dojo.partial(
            function (census, i, event) {
              ...
              if (!kitten) {
                //if kitten is dead, old button may still stay, causing very strange behavior
                return;
              }
  ...
  makeLeader: function (kitten) {
    ...
    if (theocracy.researched && kitten.job != theocracy.requiredLeaderJob) {
      ...
      this.game.msg(
        $I("msg.policy.kittenNotMadeLeader", [theocracy.label, jobTitle]),
        "important"
      );
  ...
  getGovernmentInfo: function () {
    ...
    if (leader) {
      var title =
        leader.trait.name == "none"
          ? $I("village.census.trait.none")
          : leader.trait.title +
            " (" +
            $I("village.bonus.desc." + leader.trait.name) +
            ") [" +
            $I("village.census.rank") +
            " " +
            leader.rank +
            "]";
      var nextRank = Math.floor(this.game.village.getRankExp(leader.rank));
      leaderInfo =
        this.getStyledName(leader, true /*is leader panel*/) +
        ", " +
        title +
        "<br /> exp: " +
      ...
      if (leader.rank > 0) {
        leaderInfo +=
          ...
          $I("village.job.bonus") +
          ": x" +
          this.game.village.getLeaderBonus(leader.rank).toFixed(1) +
          " (" +
  ...
  getSkillInfo: function (kitten) {
    //--------------- skills ----------------
    var skillsArr = kitten.job
      ? this.game.village.sim.getSkillsSortedWithJob(kitten.skills, kitten.job)
    ...
    for (var j = 0; j < Math.min(skillsArr.length, 3); j++) {
      ...
      if (skillsArr[j].name == kitten.job) {
        ...
        var mod = this.game.village.getValueModifierPerSkill(
          kitten.skills[kitten.job]
        );
        bonus =
          mod > 0 && kitten.isLeader
            ? this.game.village.getLeaderBonus(kitten.rank) * (mod + 1) - 1
            : mod;

        //TODO: move me to getFromLeaderBonus
        bonus =
          bonus > 0 && !kitten.isLeader && this.game.village.leader
            ? (this.game.village.getLeaderBonus(
                (this.game.village.leader || 0).rank
              ) *
                this.game.getEffect("boostFromLeader") +
                1) *
                (bonus + 1) -
      ...
      info +=
        ...
        style +
        ">" +
        this.game.village.getJob(skillsArr[j].name).title +
        bonus +
        " (" +
        this.game.villageTab.skillToText(exp) +
        " " +
        expPercent.toFixed() +
        "%)" +
        "</span><br>";
  ...
  renderGovernment: function (container) {
    ...
    if (!this.governmentDiv) {
      governmentDiv = dojo.create(
        "div",
        {
          className: "currentGovernment",
          style: {
            paddingBottom: "10px"
          }
        },
    ...
    this.leaderDiv = dojo.create(
      "div",
      { className: "currentLeader" },
      governmentDiv
    );
    //------------------------------------
    var leader = this.game.village.leader;
    ...
    if (leader) {
      ...
      this.promoteLeaderHref = dojo.create(
        "a",
        {
          href: "#",
          innerHTML: $I("village.census.leader.propmote", [
            this.game.getDisplayValueExt(expToPromote.toFixed()),
            goldToPromote
          ]),
          style: {
            display:
              leader.exp < expToPromote || gold.value < goldToPromote
                ? "none"
                : "block"
          }
        },
      ...
      dojo.connect(
        this.promoteLeaderHref,
        "onclick",
        this,
        dojo.partial(
          function (census, leader, event) {
            event.preventDefault();
      ...
      this.unassignLeaderJobHref = dojo.create(
        "a",
        {
          href: "#",
          innerHTML: $I("village.btn.unassign"),
          style: {
  ...
  getStyledName: function (kitten, isLeaderPanel) {
    if (
      !this.game.religion.getPact("fractured").val ||
      !this.game.getFeatureFlag("MAUSOLEUM_PACTS")
    ) {
      return (
        "<span class='name color-" +
        (kitten.color && kitten.colors[kitten.color + 1]
          ? kitten.colors[kitten.color + 1].color
          : "none") +
        " variety-" +
        (kitten.variety && kitten.varieties[kitten.variety + 1]
          ? kitten.varieties[kitten.variety + 1].style
          : "none") +
        "'>" +
        kitten.name +
        " " +
      ...
      if (!kitten.randTimer) {
        ...
        color_and_variety = this.game.createRandomVarietyAndColor(
          this.game.rand(76),
          this.game.rand(76)
        );
        kitten.fakeColor = color_and_variety[0];
        kitten.fakeName =
          this.game.createRandomName() +
          this.game.createRandomName(1, "    -/_") +
          this.game.createRandomName();
      ...
      return (
        "<span class='name color-" +
        (kitten.fakeColor && kitten.colors[kitten.fakeColor + 1]
          ? kitten.colors[kitten.fakeColor + 1].color
          : "none") +
        " variety-" +
        (kitten.fakeVariety && kitten.varieties[kitten.fakeVariety + 1]
          ? kitten.varieties[kitten.fakeVariety + 1].style
          : "none") +
        "'>" +
  ...
  update: function () {
    ...
    if (!this.game.challenges.isActive("anarchy")) {
      this.leaderDiv.innerHTML =
        "<span>" +
        $I("village.census.lbl.leader") +
        ":</span> " +
        govInfo.leaderInfo;
    }
    ...
    for (var i = 0; i < this.records.length; i++) {
      ...
      record.content.innerHTML =
        ...
        (kitten.rank == 0
          ? ""
          : " (" + $I("village.census.rank") + " " + kitten.rank + ")") +
        "</div>";
...
dojo.declare(
  "com.nuclearunicorn.game.ui.CensusPanel",
  com.nuclearunicorn.game.ui.Panel,
  {
    census: null,
...
dojo.declare(
  "classes.village.ui.VillageButtonController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
...
dojo.declare(
  "classes.village.ui.FestivalButtonController",
  classes.village.ui.VillageButtonController,
  {
    fetchModel: function (options) {
      var model = this.inherited(arguments);
    ...
    _newLink: function (holdQuantity) {
      ...
      return {
        ...
        visible:
          this.game.prestige.getPerk("carnivals").researched &&
          (this.game.opts.showNonApplicableButtons ||
            this.hasMultipleResources(holdQuantity)),
...
dojo.declare(
  "classes.village.ui.FestivalButton",
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    renderLinks: function () {
    ...
    update: function () {
      ...
      dojo.style(
        this.x10.link,
        "display",
        this.model.x10Link.visible ? "" : "none"
      );
      dojo.style(
        this.x100.link,
        "display",
        this.model.x100Link.visible ? "" : "none"
      );
 ...
 */
dojo.declare(
  "com.nuclearunicorn.game.ui.tab.Village",
  com.nuclearunicorn.game.ui.tab,
  {
    ...
    render: function (tabContainer) {
      ...
      this.jobsPanel = new com.nuclearunicorn.game.ui.Panel(
        $I("village.panel.job"),
        this.game.village
      );
      ...
      var btn = new com.nuclearunicorn.game.ui.ButtonModern(
        {
          ...
          handler: dojo.hitch(this, function () {
            ...
            if (game.opts.noConfirm) {
              ...
              game.ui.confirm(
                "",
                $I("village.tab.clear.job.confirmation.msg"),
                function () {
                  game.village.clearJobs(true);
                }
              );
          ...
          controller: new com.nuclearunicorn.game.ui.ButtonModernController(
            this.game
          )
        },
        this.game
      ...
      this.statisticsPanel = new com.nuclearunicorn.game.ui.Panel(
        $I("village.panel.management"),
        this.game.village
      );
      if (
      ...
      var advVillageTable = dojo.create(
        "table",
        {
          style: {
            width: "100%"
      ...
      var statsTd = dojo.create(
        "td",
        { style: "cursor: pointer; width: 50%; text-align: center;" },
        tr
      );

      UIUtils.attachTooltip(
        this.game,
        statsTd,
        0,
        200,
      ...
      var huntBtn = new com.nuclearunicorn.game.ui.ButtonModern(
        {
          ...
          prices: [{ name: "manpower", val: 100 }],
          controller: new classes.village.ui.VillageButtonController(
            this.game,
            {
              updateVisible: function (model) {
                model.visible =
                  this.game.science.get("archery").researched &&
                  !this.game.challenges.isActive("pacifism");
              }
            }
          )
        },
      ...
      var redeemGiftBtn = new com.nuclearunicorn.game.ui.ButtonModern(
        {
          ...
          handler: dojo.hitch(this, function () {
            ...
            game.ui.confirm(
              "",
              $I("village.btn.unwrap.confirmation.msg"),
              function () {
                game.redeemGift();
                game.render();
              }
          ...
          controller: new classes.village.ui.VillageButtonController(
            this.game,
            {
              updateVisible: function (model) {
                model.visible =
                  !this.game.isEldermass() &&
                  this.game.resPool.get("elderBox").value > 0;
              }
            }
          )
        },
      ...
      //--------------- bureaucracy ------------------
      this.censusPanel = new com.nuclearunicorn.game.ui.CensusPanel(
        $I("village.panel.census"),
        this.game.village,
        this.game
      );
    ...
    update: function () {
      ...
      if (this.tdTop) {
        this.tdTop.innerHTML =
          $I("village.general.free.kittens.label") +
          ": " +
          this.game.village.getFreeKittens() +
          " / " +
          this.game.resPool.get("kittens").value;
      }
      ...
      if (this.happinessStats) {
        ...
        var happinessVal =
          happiness < 10000
            ...
            : this.game.getDisplayValueExt(happiness);
        this.happinessStats.innerHTML =
          $I("village.census.lbl.happiness") + ":  " + happinessVal + "%";
      }
      ...
      if (festivalDays) {
        this.happinessStats.innerHTML +=
          "<br><br> " +
          $I("village.census.lbl.festival.duration") +
          " " +
          this.game.toDisplayDays(festivalDays);
      }
    ...
    updateTab: function () {
      ...
      if (freeKittens > 0) {
        this.tabName +=
          " <span class='genericWarning'>(" +
          this.game.getDisplayValueExt(freeKittens, false, false, 0) +
          ")</span>";
      }
      if (this.domNode) {

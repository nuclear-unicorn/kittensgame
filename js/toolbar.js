...
dojo.declare("classes.ui.Toolbar", null, {
  ...
  update: function (forceUpdate) {
    ...
    $("#sorrowTooltip").html(
      sorrow ? $I("resources.sorrow.short") + ": " + sorrow.toFixed() + "%" : ""
    );
...
dojo.declare("classes.ui.toolbar.ToolbarHappiness", classes.ui.ToolbarIcon, {
  update: function () {
    ...
    this.container.innerHTML =
      "(:3)&nbsp;" + Math.floor(this.game.village.happiness * 100) + "%";
  },
  ...
  getTooltip: function () {
    ...
    if (this.game.calendar.festivalDays > 0) {
      var festivalHappinessEffect = (
        30 *
        (1 + this.game.getEffect("festivalRatio"))
      ).toFixed();
      tooltip +=
        $I("village.happiness.festival") +
        ": +" +
        festivalHappinessEffect +
        "%<br>";
    }
    ...
    if (challengeEffect) {
      tooltip +=
        $I("village.happiness.challenges") +
        ": " +
        this.game.getDisplayValueExt(challengeEffect, false, false, 0) +
        "%<br>";
    }
    var overpopulation =
      this.game.village.getKittens() - this.game.village.maxKittens;
    if (overpopulation > 0) {
      tooltip +=
...
dojo.declare("classes.ui.toolbar.ToolbarEnergy", classes.ui.ToolbarIcon, {
  update: function () {
    ...
    this.container.innerHTML =
      "&#9889;&nbsp;" +
      this.game.getDisplayValueExt(resPool.energyProd - resPool.energyCons) +
      $I("unit.watt");

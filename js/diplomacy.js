...
dojo.declare("classes.managers.DiplomacyManager", null, {
  ...
  getTradeRatio: function () {
    return (
      this.game.getEffect("tradeRatio") +
      this.game.village.getEffectLeader("merchant", 0)
    );
  },
  ...
  isValidTrade: function (sell, race) {
    ...
    var hasHighEnoughEmbassyLevel =
      !sell.minLevel || race.embassyLevel >= sell.minLevel;
    var isResourceTradeable =
  ...
  unlockRandomRace: function () {
    ...
    if (
      !spiders.unlocked &&
      this.game.resPool.get("ship").value >= 100 &&
    ...
    if (
      !dragons.unlocked &&
      this.game.science.get("nuclearFission").researched
    ) {
    ...
    if (unmetRaces[raceId]) {
      //someone reported a bug there, to be investigated later
  ...
  update: function () {
    if (!this.hasUnlockedRaces()) {
      var unlockYear =
        this.game.prestige.getPerk("navigationDiplomacy").researched &&
        this.game.resPool.get("ship").value > 0
          ? 0
          : this.game.prestige.getPerk("diplomacy").researched
            ? 1
    ...
    if (this.game.ironWill && this.game.challenges.isActive("blackSky")) {
      ...
      if (!this.game.workshop.get("goldOre").researched) {
        this.baseGoldCost =
          this.game.resPool.get("gold").value == 0 ? 0 : this.defaultGoldCost;
        this.baseManpowerCost =
          this.game.resPool.get("manpower").value == 0
            ? 0
            : this.defaultManpowerCost;
      }
  ...
  unlockElders: function () {
    ...
    if (elders.duration || !this.hasUnlockedRaces()) {
      //elder visits do not stack, and elders do not like being first
      return;
    }
  ...
  tradeImpl: function (race, totalTradeAmount) {
    ...
    var standingRatio =
      this.game.getEffect("standingRatio") +
      this.game.diplomacy.calculateStandingFromPolicies(race.name, this.game);

    var failedTradeAmount =
      race.standing < 0
        ? this.game.math.binominalRandomInteger(
            totalTradeAmount,
            -(race.standing + standingRatio)
          )
        : 0;
    ...
    if (successfullTradeAmount == 0) {
      if (printMessages) {
        this.game.msg(
          $I("trade.msg.trade.failure", [race.title]),
          null,
          "trade"
        );
      }
    ...
    race.duration = Math.min(
      race.duration,
      this.game.calendar.daysPerSeason *
        (this.game.calendar.seasonsPerYear + race.energy)
    );

    var bonusTradeAmount =
      race.standing > 0
        ? this.game.math.binominalRandomInteger(
            totalTradeAmount,
            race.standing + standingRatio / 2
          )
        : 0;
    ...
    if (bonusTradeAmount > 0) {
      if (printMessages) {
        this.game.msg(
          $I("trade.msg.trade.success", [race.title]),
          null,
          "trade"
        );
      }
    ...
    var tradeRatio =
      ...
      this.game.diplomacy.calculateTradeBonusFromPolicies(
        race.name,
        this.game
      ) +
      this.game.challenges
        .getChallenge("pacifism")
        .getTradeBonusEffect(this.game);
    ...
    for (var i = 0; i < race.sells.length; i++) {
      ...
      var resourcePassedNormalTradeAmount =
        this.game.math.binominalRandomInteger(normalTradeAmount, tradeChance);
      var resourcePassedBonusTradeAmount =
        this.game.math.binominalRandomInteger(bonusTradeAmount, tradeChance);

      if (
        resourcePassedNormalTradeAmount + resourcePassedBonusTradeAmount ==
        0
      ) {
        continue;
      }

      var fuzzedNormalAmount = this._fuzzGainedAmount(
        resourcePassedNormalTradeAmount,
        sellResource.width
      );
      var fuzzedBonusAmount = this._fuzzGainedAmount(
        resourcePassedBonusTradeAmount,
        sellResource.width
      );
      var resourceSeasonTradeRatio =
        1 + (sellResource.seasons ? sellResource.seasons[currentSeason] : 0);
      boughtResources[sellResource.name] =
        (fuzzedNormalAmount + fuzzedBonusAmount * 1.25) *
        sellResource.value *
        tradeRatio *
        raceRatio *
    ...
    boughtResources["spice"] =
      25 * spiceTradeAmount +
      50 * tradeRatio * this.game.math.irwinHallRandom(spiceTradeAmount);
    if (spiceChance > 1) {
      //Past 100% chance, additional embassies increase amount of spice gained.
    ...
    if (race.name == "zebras") {
      ...
      var zebraRelationModifierTitanium =
        this.game.getEffect("zebraRelationModifier") *
        this.game.bld.getBuildingExt("tradepost").meta.effects["tradeRatio"];
      boughtResources["titanium"] =
        (1.5 + shipAmount * 0.03) *
        (1 + zebraRelationModifierTitanium) *
        this.game.math.binominalRandomInteger(
          successfullTradeAmount,
          0.15 + shipAmount * 0.0035
        );
    }

    //Update Trade Stats
    this.game.stats.getStat("totalTrades").val = Math.min(
  ...
  _fuzzGainedAmount: function (amount, width) {
    return (
      amount + width * (this.game.math.irwinHallRandom(amount) - amount / 2)
    );
  },

  getManpowerCost: function () {
    var manpowerCost =
      this.baseManpowerCost - this.game.getEffect("tradeCatpowerDiscount");
  ...
  hasMultipleResources: function (race, amt) {
    return (
      this.game.resPool.get("gold").value >= this.getGoldCost() * amt &&
    ...
    if (yieldResTotal) {
      ...
      for (var res in yieldResTotal) {
        ...
        if (amt > 0) {
          ...
          var msg = $I("trade.msg.resources", [
            this.game.getDisplayValueExt(amt),
            name
          ]);
          var type = null;
  ...
  getMaxTradeAmt: function (race) {
    ...
    var amt = [
      Math.floor(this.game.resPool.get("gold").value / Math.max(goldCost, 1)),
      Math.floor(
        this.game.resPool.get("manpower").value / Math.max(manpowerCost, 1)
      ),
      Math.floor(
        this.game.resPool.get(race.buys[0].name).value / race.buys[0].val
      )
    ];
  ...
  getMarkerCap: function () {
    return Math.floor(
      (this.game.religion.getZU("marker").getEffectiveValue(this.game) * 5 +
        5) *
  ...
  buyBcoin: function () {
    var amt =
      this.game.resPool.get("relic").value / this.game.calendar.cryptoPrice;
    amt = this.game.resPool.addResEvent("blackcoin", amt);
    this.game.resPool.get("relic").value = 0;
    this.game.msg(
      $I("trade.bcoin.buy.msg", [this.game.getDisplayValueExt(amt)]),
      "",
      "blackcoin"
    );
  },
  ...
  sellBcoin: function () {
    var amt =
      this.game.resPool.get("blackcoin").value * this.game.calendar.cryptoPrice;
    var amt = this.game.resPool.addResEvent("relic", amt);
    this.game.resPool.get("blackcoin").value = 0;
    this.game.msg(
      $I("trade.bcoin.sell.msg", [this.game.getDisplayValueExt(amt)])
    ),
      "",
      "blackcoin";
  },
  ...
  calculateStandingFromPolicies: function (raceName, game) {
    var tradepostStandingRatio =
      game.bld.getBuildingExt("tradepost").meta.effects["standingRatio"];
    var phantomTradeposts = game.diplomacy.calculatePhantomTradeposts(
      raceName,
      game
    );
  ...
  calculateTradeBonusFromPolicies: function (raceName, game) {
    var tradepostsTradeRatio =
      game.bld.getBuildingExt("tradepost").meta.effects["tradeRatio"];
    var phantomTradeposts = game.diplomacy.calculatePhantomTradeposts(
      raceName,
      game
    );
...
dojo.declare(
  "classes.diplomacy.ui.RacePanel",
  com.nuclearunicorn.game.ui.Panel,
  {
    ...
    render: function (container) {
      var attitudeFromPolicies =
        this.game.diplomacy.calculateStandingFromPolicies(
          this.race.name,
          this.game
        );
      var attitude =
...
dojo.declare(
  "classes.diplomacy.ui.EldersPanel",
  classes.diplomacy.ui.RacePanel,
  {
    feedBtn: null,
    ...
    render: function (container) {
      ...
      if (
        this.game.science.get("blackchain").researched ||
        this.game.resPool.get("blackcoin").value > 0
      ) {
      ...
      if (
        this.game.science.get("antimatter").researched &&
        this.game.workshop.get("invisibleBlackHand").researched
      ) {
        this.crashBcoin = new com.nuclearunicorn.game.ui.ButtonModern(
          {
            ...
            controller:
              new com.nuclearunicorn.game.ui.CrashBcoinButtonController(
                this.game
              ),
            handler: function () {
...
dojo.declare(
  "com.nuclearunicorn.game.ui.CrashBcoinButtonController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    ...
    getPrices: function () {
      ...
      var tcPerStandardYear =
        0.002401 * this.game.space.getBuilding("moonOutpost").val;
      tcPerStandardYear *=
        1 +
        this.game.space.getBuilding("spaceElevator").val * 0.01 +
        this.game.space.getBuilding("orbitalArray").val * 0.02;
      tcPerStandardYear *= 1 + this.game.bld.get("factory").val * 0.045;
      tcPerStandardYear *=
        1.03 +
        this.game.getEffect("tradeRatio") +
        this.game.prestige.getBurnedParagonRatio() * 0.03;
      tcPerStandardYear *=
        1 + this.game.diplomacy.get("leviathans").energy * 0.02;
      ...
      var heatPerShatter = this.game.challenges.getChallenge("1000Years")
        .researched
        ? 5
        : 10;
      var tcPerSkip =
        1.13 *
        tcPerStandardYear *
        this.game.getEffect("shatterTCGain") *
        (1 + this.game.getEffect("rrRatio"));
      var tcPerShatter = (1 + heatPerShatter / 100) * tcPerSkip - 1;
      var tcPerTick_phase2 =
        tcPerTick_phase0 +
        (tcPerShatter / heatPerShatter) * this.game.getEffect("heatPerTick");

      var tcPerTick = Math.max(
        tcPerTick_phase0,
        tcPerTick_phase1,
        tcPerTick_phase2
      );
      // 10 ticks/day / (1.2499270834635280e-6 logInc/day), see calendar.js
      var ticksUntilNextNaturalCrash =
        8000466.693057134 * Math.log(1100 / this.game.calendar.cryptoPrice);
...
dojo.declare(
  "com.nuclearunicorn.game.ui.TradeButtonController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
...
dojo.declare(
  "com.nuclearunicorn.game.ui.TradeButton",
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    ...
    update: function () {
      ...
      this.tradeAllHref.link.title =
        "x" + this.game.getDisplayValueExt(tradeMax, null, false, 0);

      // Update tradeHalfHref Link
      var tradeHalf = Math.floor(tradeMax / 2);
      // Change button content
      this.tradeHalfHref.link.textContent = this.game.opts
        .usePercentageConsumptionValues
        ? "50%"
        : "x" + this.game.getDisplayValueExt(tradeHalf, null, false, 0);
      this.tradeHalfHref.link.title = this.game.opts
        .usePercentageConsumptionValues
        ? "x" + this.game.getDisplayValueExt(tradeHalf, null, false, 0)
        : "50%";
      // Change handler
      dojo.disconnect(this.tradeHalfHref.linkHandler);
      this.tradeHalfHref.linkHandler = dojo.connect(
        this.tradeHalfHref.link,
        "onclick",
        this,
        dojo.partial(function (event) {
      ...
      dojo.style(
        ...
        this.game.opts.showNonApplicableButtons ||
          this.game.diplomacy.hasMultipleResources(this.race, 50)
          ? ""
          : "none"
      );
      ...
      // Change button content
      this.tradeFifthHref.link.textContent = this.game.opts
        .usePercentageConsumptionValues
        ? "20%"
        : "x" + this.game.getDisplayValueExt(tradeFifth, null, false, 0);
      this.tradeFifthHref.link.title = this.game.opts
        .usePercentageConsumptionValues
        ? "x" + this.game.getDisplayValueExt(tradeFifth, null, false, 0)
        : "20%";
      // Change handler
      dojo.disconnect(this.tradeFifthHref.linkHandler);
      this.tradeFifthHref.linkHandler = dojo.connect(
        this.tradeFifthHref.link,
        "onclick",
        this,
        dojo.partial(function (event) {
          event.stopPropagation();
      ...
      dojo.style(
        ...
        "display",
        this.game.opts.showNonApplicableButtons ||
          this.game.diplomacy.hasMultipleResources(this.race, 25)
          ? ""
          : "none"
      );
    }
...
dojo.declare(
  "classes.diplomacy.ui.EmbassyButtonController",
        ...
        prices[i].val *=
          ...
          Math.pow(
            ...
            model.options.race.embassyLevel +
              this.game.getEffect("embassyFakeBought")
          );
      }
      return prices;
    },
...
var EmbassyButtonHelper = {
  ...
  getTooltipHTML: function (controller, model) {
    ...
    if ((prices && prices != "") || effects || (flavor && flavor != "")) {
      ...
      if (
        controller.game.stats.getStatCurrent("totalTrades").val >=
        TOTAL_TRADES_TO_SEE_CHANCES
      ) {
        EmbassyButtonHelper.renderResourceChances(
          tooltip,
          controller.game,
          model.options.race
        );
  ...
  renderResourceChances: function (tooltip, game, race) {
    ...
    for (var i = 0; i < race.sells.length; i += 1) {
      ...
      var sellChance = Math.min(
        game.diplomacy.getResourceTradeChance(sellOptions, race),
        1
      ); //Cap at 100%
      if (sellChance == 0) {
    ...
    if (spiceResource.unlocked && spiceChance > 0) {
      ...
      chancesToDisplay.push({
        title: spiceResource.title,
        chance: spiceChance,
        multi: spiceMulti
      });
    }
    ...
    for (var i = 0; i < chancesToDisplay.length; i += 1) {
      dojo.create(
        ...
        {
          innerHTML:
            chancesToDisplay[i].title +
            ": " +
            (100 * chancesToDisplay[i].chance).toFixed(1) +
            "%" +
            (chancesToDisplay[i].multi
              ? ", ×" + game.getDisplayValueExt(chancesToDisplay[i].multi)
              : ""),
...
dojo.declare(
  "classes.diplomacy.ui.EmbassyButton",
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    pinLinkHref: null,
    race: null,
...
dojo.declare(
  "classes.diplomacy.ui.autoPinnedButtonController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
    ...
    getName: function (model) {
      ...
      return isAutoPinned
        ? $I("trade.autopinned.labelOn")
        : $I("trade.autopinned.labelOff");
    },
...
dojo.declare(
  "classes.diplomacy.ui.autoPinnedButton",
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    pinLinkHref: null,
    race: null,
...
dojo.declare(
  "classes.trade.ui.SendExplorersButtonController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
...
dojo.declare(
  "classes.trade.ui.SendExplorersButton",
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    afterRender: function () {
      this.inherited(arguments);
...
dojo.declare(
  "com.nuclearunicorn.game.ui.tab.Diplomacy",
  com.nuclearunicorn.game.ui.tab,
  {
    ...
    render: function (tabContainer) {
      ...
      var div = dojo.create(
        "div",
        { class: "expandAllBar", style: { float: "left" } },
        tabContainer
      );
      dojo.create(
        "span",
        {
          innerHTML: $I("trade.effectiveness", [
      ...
      var expandDiv = dojo.create(
        "div",
        { class: "expandAllBar" },
        tabContainer
      );

      var expandAll = dojo.create(
        "a",
        { innerHTML: $I("common.expand.all"), href: "#" },
        expandDiv
      );
      dojo.create("span", { innerHTML: " | " }, expandDiv);
      var collapseAll = dojo.create(
        "a",
        { innerHTML: $I("common.collapse.all"), href: "#" },
        expandDiv
      );
      ...
      for (var i = 0; i < races.length; i++) {
        ...
        if (!racePanel) {
          racePanel =
            race.name === "leviathans"
              ? new classes.diplomacy.ui.EldersPanel(race)
              : new classes.diplomacy.ui.RacePanel(race);
        ...
        if (racePanel.feedBtn) {
          var leviathansInfo = dojo.create(
            "div",
            {
              innerHTML: ""
            },
        ...
        for (var j = 0; j < race.sells.length; j++) {
          ...
          var average =
            ...
            tradeRatio *
            (1 + race.energy * 0.02) *
            (1 + (s.seasons ? s.seasons[currentSeason] : 0));

          var prefix =
            j == 0
              ? "<span class='sells'>" + $I("trade.sells") + ": </span>"
        ...
        if (race.name == "zebras") {
          var zebraRelationModifierTitanium =
            this.game.getEffect("zebraRelationModifier") *
            this.game.bld.getBuildingExt("tradepost").meta.effects[
              "tradeRatio"
            ];
          var titanium = this.game.resPool.get("titanium");
          var displayedVal = this.game.getDisplayValueExt(
            (1.5 + this.game.resPool.get("ship").value * 0.03) *
              (1 + zebraRelationModifierTitanium),
            false,
            false,
            0
        ...
        var tradePrices = [
          { name: "manpower", val: this.game.diplomacy.getManpowerCost() },
          { name: "gold", val: this.game.diplomacy.getGoldCost() }
        ];
        if (this.game.challenges.isActive("postApocalypse")) {
        ...
        var tradeBtn = new com.nuclearunicorn.game.ui.TradeButton(
          {
            ...
            prices: tradePrices,
            race: race,
            controller: new com.nuclearunicorn.game.ui.TradeButtonController(
              this.game
            ),
          ...
          var embassyButton = new classes.diplomacy.ui.EmbassyButton(
            {
              ...
              race: race,
              controller: new classes.diplomacy.ui.EmbassyButtonController(
                this.game
              )
            },
            this.game
          ...
          var autoPinnedButton = new classes.diplomacy.ui.autoPinnedButton(
            {
              ...
              race: race,
              controller: new classes.diplomacy.ui.autoPinnedButtonController(
                this.game
              ),
              handler: dojo.partial(function (race) {
        ...
        if (racePanel.buyBcoin && racePanel.sellBcoin) {
          var tradePanel = dojo.create(
            "div",
            { className: "crypto-trade" /*, style:{display:"none"}*/ },
            null
          );
      ...
      var exploreBtn = new classes.trade.ui.SendExplorersButton(
        {
          ...
          controller: new classes.trade.ui.SendExplorersButtonController(
            this.game
          )
        },
        this.game
    ...
    update: function () {
      ...
      if (this.leviathansInfo) {
        ...
        if (leviathans.energy) {
          var markerCap = this.game.diplomacy.getMarkerCap();
          var leviathansInfoEnergy = leviathans.energy
            ? leviathans.energy + " / " + markerCap
            : "N/A";
          this.leviathansInfo.innerHTML +=
            $I("trade.leviathans.energy") + leviathansInfoEnergy + "<br />";
        }
        //Time to leave:
        this.leviathansInfo.innerHTML +=
          $I("trade.leviathans.timeToLeave") +
          this.game.toDisplayDays(leviathans.duration);
        //B-coin price:

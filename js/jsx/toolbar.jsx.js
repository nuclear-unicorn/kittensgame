...
WToolbarHappiness = React.createClass({
  ...
  getTooltip: function () {
    ...
    var tooltip =
      $I("village.happiness.base") +
      ": 100%<br>" +
      $I("village.happiness.buildings") +
      ": +" +
      Math.floor(base) +
      "%<br>";
    ...
    tooltip +=
      $I("village.happiness.rare.resources") +
      ": +" +
      this.game.getDisplayValueExt(resHappiness, false, false, 0) +
      "%<br>";
    //---------------------
    var karma = this.game.resPool.get("karma");
    if (karma.value > 0) {
      tooltip +=
        $I("village.happiness.karma") +
        ": +" +
        this.game.getDisplayValueExt(karma.value, false, false, 0) +
        "%<br>";
    }

    if (this.game.calendar.festivalDays > 0) {
      var festivalHappinessEffect =
        30 * (1 + this.game.getEffect("festivalRatio"));
      tooltip +=
        $I("village.happiness.festival") +
        ": +" +
        this.game.getDisplayValueExt(festivalHappinessEffect, false, false, 0) +
        "%<br>";
    }

    var unhappiness =
        this.game.village.getUnhappiness() /
        (1 + this.game.getEffect("unhappinessRatio")),
      unhappinessReduction =
        unhappiness * this.game.getEffect("unhappinessRatio", true);
    var environmentEffect = this.game.village.getEnvironmentEffect();
    tooltip +=
      $I("village.happiness.penalty") +
      ": -" +
      this.game.getDisplayValueExt(
        unhappiness + unhappinessReduction,
        false,
        false,
        0
      ) +
    ...
    tooltip +=
      "* " +
      $I("village.happiness.penalty.base") +
      ": -" +
      this.game.getDisplayValueExt(unhappiness, false, false, 0) +
      "%<br>";
    tooltip +=
      "* " +
      $I("village.happiness.penalty.mitigated") +
      ": " +
      this.game.getDisplayValueExt(-unhappinessReduction, false, false, 0) +
      "%<br>";
    tooltip +=
      $I("village.happiness.environment") +
      ": " +
      this.game.getDisplayValueExt(environmentEffect, false, false, 0) +
      "%<br>";
    var overpopulation = this.game.village.getOverpopulation();
    if (overpopulation > 0) {
      tooltip +=
        $I("village.happiness.overpopulation") +
        ": -" +
        overpopulation * 2 +
        "%<br>";
    }
...
WToolbarEnergy = React.createClass({
  render: function () {
    ...
    return $r(
      ...
      $r("div", {
        className: "energyText",
        dangerouslySetInnerHTML: {
          __html:
            game.getDisplayValueExt(resPool.energyProd - resPool.energyCons) +
            $I("unit.watt")
        }
      })
    );
  ...
  getTooltip: function () {
    ...
    var penalty =
      energy >= 0
        ? ""
        : "<br><br>" +
          $I("navbar.energy.penalty") +
          "<span class='energyPenalty'>-" +
          Math.floor((1 - delta) * 100) +
          "%</span>";

    return (
      $I("navbar.energy.prod.short") +
      " <span class='energyProduction'>" +
      this.game.getDisplayValueExt(resPool.energyProd, true, false) +
      $I("unit.watt") +
      "</span>" +
      "<br>" +
      $I("navbar.energy.cons.short") +
      " <span class='energyConsumption'>-" +
...
WToolbarPollution = React.createClass({
    ...
    if (
      ...
      return $r(
        WToolbarIconContainer,
        {
          ...
          getTooltip: this.getTooltip,
          className:
            "pollutionIcon " + (this.freshMessage ? "energy warning" : null)
        },
        $r(
          "div",
          { className: "pollutionText" },
          game.science.get("ecology").researched
            ? this.getPollutionMod()
            : "\xa0"
        )
      );
    }
  ...
  getTooltip: function (notUpdateFreshMessage) {
    ...
    if (pollution * 1.5 <= eqPol || eqPolLvl > polLvl) {
      ...
      pollution >= 0 &&
      game.bld.cathPollutionPerTick <= 0 &&
      eqPolLvl < polLvl
    ) {
...
WCloudSaveRecord = React.createClass({
  ...
  render: function () {
    ...
    return $r(
      "div",
      { className: "save-record " + (save.archived ? "archived" : "") },
      [
        $r(
          "div",
          { className: "save-record-cell" },
        ...
        $r(
          ...
          save.index
            ? "Year " +
                save.index.calendar.year +
                ", day " +
                save.index.calendar.day
            : "loading..."
        ),
        $r(
          "div",
        ...
        isActiveSave &&
          $r(
            "a",
            {
              className: "link",
              title:
                "Upload your current game save to the server (this will owerwrite your old cloud save)",
              onClick: function (e) {
                e.stopPropagation();
                game.ui.confirm(
                  "[S]ave",
                  "This will override [SERVER] save. Y/N",
                  function () {
        ...
        $r(
          "a",
          {
            className: "link",
            title:
              "Download a cloud save and apply it to your game (your current data will be lost)",
            onClick: function (e) {
              e.stopPropagation();
              game.ui.confirm(
                "[L]oad",
                "This will override [LOCAL] save. Y/N",
                function () {
...
WCloudSaves = React.createClass({
  ...
  render: function () {
    ...
    return $r("div", null, [
      $r(
        "div",
        { className: "save-record-container" },
        //header
        saveData &&
          $r("div", { className: "save-record header" }, [
        ...
        $r("div", { className: "save-record" }, [
          $r(
            "a",
            {
              className: "link",
              title:
                "Fetch the latest information about your cloud saves from the serer. This is a safe operation and it wont change any data.",
              onClick: function (e) {
                e.stopPropagation();
...
WLogin = React.createClass({
  ...
  render: function () {
    ...
    var lastBackup =
      (new Date().getTime() - game.lastBackup) / (1000 * 60 * 60 * 24);
      ...
      $r(
        ...
        [
          $r(
            ...
            {
              className:
                "kgnet-login-link status-indicator-" +

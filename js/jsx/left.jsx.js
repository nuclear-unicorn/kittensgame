...
WResourceRow = React.createClass({
  ...
  render: function () {
    ...
    if (
      game.resPool.isNormalCraftableResource(res) &&
      game.workshop.getCraft(res.name).unlocked
    ) {
      return null;
    }
    ...
    perTick = game.opts.usePerSecondValues
      ? perTick * game.getTicksPerSecondUI()
      : perTick;
    ...
    var perTickVal =
      ...
      game.getResourcePerTickConvertion(res.name)
        ...
        : res.calculatePerDay
          ...
          : res.calculateOnYear
            ? game.getDisplayValueExt(
                game.getResourceOnYearProduction(res.name),
                true,
                false
              ) +
              "/" +
    ...
    if (season.modifiers[res.name] && perTick !== 0) {
      ...
      weatherModValue = modifier
        ? "[" + (modifier > 0 ? "+" : "") + modifier.toFixed() + "%]"
        : "";
    ...
    return $r("div", { className: resRowClass }, [
      ...
      $r(
        "div",
        { className: "res-cell " + resAmtClassName + specialClass },
        game.getDisplayValueExt(res.value)
      ),
      $r(
        "div",
        { className: "res-cell maxRes" },
        res.maxValue ? "/" + game.getDisplayValueExt(res.maxValue) : ""
      ),
      $r(
        "div",
        { className: "res-cell resPerTick", ref: "perTickNode" },
        isTimeParadox ? "???" : perTickVal
      ),
      $r(
        "div",
        { className: "res-cell" + (weatherModCss ? " " + weatherModCss : "") },
        weatherModValue
      )
    ]);
  },
...
WCraftShortcut = React.createClass({
  ...
  render: function () {
      ...
      elem = this.hasMinAmt(recipe)
        ? $r(
            "div",
            {
              ...
              onClick: this.doCraftAll,
              title:
                "+" +
                game.getDisplayValueExt(
                  allCount * (1 + craftRatio),
                  null,
                  null,
                  0
                )
    ...
    return $r(
      "div",
      { ref: "linkBlock", style: { display: "contents" } },
      elem
    );
  },
...
WCraftRow = React.createClass({
  ...
  render: function () {
    ...
    var hasVisibility =
      res.unlocked && recipe.unlocked; /*&& this.workshop.on > 0*/
    if (!hasVisibility || (!this.state.visible && !this.props.isEditMode)) {
      return null;
    }
    ...
    return $r(
      ...
      {
        className:
          "res-row craft resource_" +
          res.name +
          (game.workshop.getEffectEngineer(res.name) != 0
            ? " craftEngineer "
            : "") +
          (this.props.isRequired ? " highlited" : "")
      },
      [
        ...
        $r(
          "div",
          {
            className: "res-cell resource-value",
            ref: "perTickNode",
            title: resVal
          },
          resVal
    ...
    return $r("div", null, [
      $r("div", null, [
        ...
        $r(
          ...
          $r(WTooltip, { body: "?" }, $I("left.resources.tip"))
        )
      ]),
        ...
        $r("div", null, [
          this.state.isEditMode &&
            $r("div", { style: { textAlign: "right" } }, [
              $r(
                "a",
                { className: "link", onClick: game.ui.zoomUp.bind(game.ui) },
                $I("left.font.inc")
              ),
              $r(
                "a",
                { className: "link", onClick: game.ui.zoomDown.bind(game.ui) },
                $I("left.font.dec")
              )
            ]),
    ...
    for (var i in pins) {
      ...
      pinLinks.push(
        $r(
          "div",
          { className: "pin-link" },
          $r("a", { href: "#", onClick: pin.handler }, pin.title)
        )
      );
    }
    return $r(WCollapsiblePanel, { title: $I("left.trade") }, pinLinks);
  }
});
...
WLeftPanel = React.createClass({
  ...
  render: function () {
    ...
    var canHunt =
      (game.resPool.get("paragon").value > 0 ||
        game.science.get("archery").researched) &&
    ...
    if (game.bld.get("field").on > 0) {
      ...
      showAdvisor =
        game.resPool.get("catnip").value +
          winterDays * catnipPerTick * calendar.ticksPerDay <=
        0;
    }
    //----------------------------
    ...
    return $r("div", null, [
      ...
      $r(
        "div",
        {
          id: "fastPraiseContainer",
          className: "pin-link",
          style: { visibility: "hidden" }
        },
        $r("a", { href: "#", onClick: this.praiseAll }, $I("left.praise"))
      ),
      $r(WPins, { game: game }),
    ...
    return $r(
      ...
      [
        ...
        this.state.showTooltip
          ? $r("div", { className: "tooltip-content" }, this.props.children)
          : null
      ]
    );
  },

...
WQueueItem = React.createClass({
  ...
  attachTooltip: function () {
    ...
    var controllerAndModel =
      game.time.queue.getQueueElementControllerAndModel(item);
    ...
    UIUtils.attachTooltip(
      ...
      200,
      dojo.partial(
        ButtonModernHelper.getTooltipHTML,
...
WQueue = React.createClass({
  ...
  getQueueTypeSelect: function () {
    ...
    for (var i in queueSources) {
      options.push(
        $r("option", { value: queueSources[i].name }, queueSources[i].label)
      );
    }
    return $r(
      "select",
  ...
  getQueueItemSelect: function (options) {
    ...
    for (var i in options) {
      ...
      selectOpts.push(
        $r("option", { value: i, "data-label": option.label }, option.label)
      );
    }

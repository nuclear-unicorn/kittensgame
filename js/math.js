...
dojo.declare("com.nuclearunicorn.game.Math", null, {
  ...
  irwinHallRandom: function (trials) {
    return this.loopOrGaussianApproximation(
      trials,
      false,
      1 / 2,
      1 / 12,
  ...
  binominalRandomInteger: function (trials, successProbability) {
    return successProbability <= 0
      ...
      : successProbability >= 1
        ? trials
        : this.loopOrGaussianApproximation(
            trials,
            true,
            successProbability,
  ...
  loopOrGaussianApproximation: function (
    trials,
    isDiscrete,
    mean,
    variance,
    ...
    while (true) {
      var result =
        globalStandardDeviation * this.standardGaussianRandom() + globalMean;
      if (isDiscrete) {

 ...
 */
dojo.declare(
  "classes.managers.ReligionManager",
  com.nuclearunicorn.core.TabManager,
  {
      ...
      this.registerMeta(/*"stackable"*/ false, this.zigguratUpgrades, {
        getEffect: function (bld, effect) {
          if (bld.name == "blackPyramid") {
            return bld.effects
              ? bld.effects[effect] * bld.getEffectiveValue(game)
              : 0;
          }
    ...
    save: function (saveData) {
      saveData.religion = {
        ...
        zu: this.filterMetadata(this.zigguratUpgrades, [
          "name",
          "val",
          "on",
          "unlocked"
        ...
        tu: this.filterMetadata(this.transcendenceUpgrades, [
          "name",
          "val",
          "on",
          "unlocked"
        ]),
        pact: this.filterMetadata(this.pactsManager.pacts, [
          "name",
          "val",
          "on",
          "unlocked"
    ...
    load: function (saveData) {
      ...
      this.pactsManager.necrocornDeficit =
        saveData.religion.necrocornDeficit || 0;

      // Read old save
      if (this.transcendenceTier == 0 && _data.tcratio > 0) {
        this.transcendenceTier = Math.max(
          0,
          Math.round(
            Math.log(10 * this.game.getUnlimitedDR(_data.tcratio, 0.1))
          )
        );
      }
      ...
      for (var i = 0; i < this.transcendenceUpgrades.length; i++) {
        var tu = this.transcendenceUpgrades[i];
        if (
          this.transcendenceTier >= tu.tier &&
          (!tu.evaluateLocks || tu.evaluateLocks(this.game))
        ) {
          tu.unlocked = true;
        }
      ...
      this.getZU("blackPyramid").updateEffects(
        this.getZU("blackPyramid"),
        this.game
      );
      console.log("pactsAdjustment");
      if (
        !this.getPact("fractured").researched &&
        this.getZU("blackPyramid").val > 0 &&
        (this.game.religion.getTU("mausoleum").val > 0 ||
          this.game.science.getPolicy("radicalXenophobia").researched)
      ) {
        this.game.unlock({
          pacts: [
            "pactOfCleansing",
    ...
    getCorruptionPerTickProduction: function (pretendExistNecrocorn) {
      if (this.game.resPool.get("alicorn").value > 0) {
        ...
        var existNecrocorn =
          pretendExistNecrocorn === undefined
            ? this.game.resPool.get("necrocorn").value > 0
            : pretendExistNecrocorn;
        var blsBoost =
          1 +
          Math.sqrt(
        ...
        var corruptionBoost = existNecrocorn
          ...
          : 1;
        return (
          this.game.getEffect("corruptionRatio") * blsBoost * corruptionBoost
        );
      }
      return 0;
    },
    getCorruptionPerTickConsumption: function () {
      if (
        this.game.getFeatureFlag("MAUSOLEUM_PACTS") > 0 &&
        this.game.science.getPolicy("siphoning").researched
      ) {
        return (
          this.game.getEffect("necrocornPerDay") /
    ...
    getCorruptionDeficitPerTick: function () {
      return Math.max(
        -this.getCorruptionPerTickProduction() -
          this.getCorruptionPerTickConsumption(),
        0
      );
    },
    ...
    update: function () {
      if (
        this.game.resPool.get("faith").value > 0 ||
        (this.game.challenges.isActive("atheism") &&
          this.game.bld.get("ziggurat").val > 0)
      ) {
    ...
    corruptNecrocorns: function () {
      ...
      var alicornsToCorrupt = Math.floor(
        Math.min(this.corruption, maxAlicornsToCorrupt)
      );
    ...
    necrocornsNaiveFastForward: function (daysOffset, times) {
      ...
      if (alicorns.value > 0) {
        ...
        var ticksBeforeFirstNecrocorn =
          this.game.resPool.get("necrocorn").value == 0
            ? // this.corruption is <= 1 at this point, no need to check if 1-this.corruption is negative
              Math.min(
                Math.ceil((1 - this.corruption) / corruptionPerTick),
                times
              )
            : 0;
        if (
          ticksBeforeFirstNecrocorn > 0 &&
          ticksBeforeFirstNecrocorn < times
        ) {
          //this.game.resPool.get("necrocorn").value = 1;
          this.corruption = 1;
          var ticksAfterFirstNecrocorn = times - ticksBeforeFirstNecrocorn;
          var corruptionPerTickAfterFirstNecrocorn =
            this.getCorruptionPerTick(true);
          this.corruption +=
            ticksAfterFirstNecrocorn * corruptionPerTickAfterFirstNecrocorn;
          this.game.resPool.get("necrocorn").value = 0;
        } else {
    ...
    gesSiphoningAlicornConsumptionPerDay: function () {
      if (this.game.science.getPolicy(["siphoning"]).researched) {
        ...
        if (this.pactsManager.necrocornDeficit > 0) {
          necrocornDeficitRepaymentModifier =
            1 + 0.15 * (1 + this.game.getEffect("deficitRecoveryRatio") / 2);
        }
        return (
          this.game.getEffect("necrocornPerDay") *
          necrocornDeficitRepaymentModifier
    ...
    necrocornFastForward: function (days, times) {
      ...
      var corruptionWithExisting =
        this.game.religion.getCorruptionPerTickProduction(true);
      var worstPerTickDelta =
        corruptionWithExisting * days * this.game.calendar.ticksPerDay +
        corruptionWithExisting;
      //if(!this.game.science.getPolicy(["siphoning"]).researched){
      if (
        worstPerTickDelta >= 0 ||
        (worstPerTickDelta < 0 &&
          necrocornVal + worstPerTickDelta * days > 0 &&
          this.game.resPool.get("alicorns").value -
            1 +
            necrocornPerDay * days >=
            0)
      ) {
        //naive solution here
      ...
      //}
      var corruptionWithoutExisting =
        this.game.religion.getCorruptionPerTickProduction(false);
      //here we chech if the necrocorns will get into cycles of being produced and spent, which also works with syphening
      if (
        corruptionWithExisting * this.game.calendar.ticksPerDay +
          necrocornPerDay <
          0 &&
        corruptionWithoutExisting * this.game.calendar.ticksPerDay +
          necrocornPerDay >
          0
      ) {
        var alicornsResult =
          this.game.resPool.get("alicorns").value - 1 + necrocornPerDay * days;
        var alicornsSpent = necrocornPerDay * days;
        if (
          (corruptionWithoutExisting * days) / this.game.calendar.ticksPerDay +
            necrocornPerDay * necrocornDeficitRepaymentModifier >
            0 &&
          this.pactsManager.necrocornDeficit > 0
        ) {
          corruptionWithoutExisting +=
            necrocornPerDay * (necrocornDeficitRepaymentModifier - 1);
        }
        var daysUntilSpentOne = -(
          necrocornPerDay * days +
          corruptionWithExisting / this.game.calendar.ticksPerDay
        );
        var daysUntilCorrupted =
          necrocornPerDay * days +
          corruptionWithoutExisting / this.game.calendar.ticksPerDay;
        ...
        var state =
          timePeriodWorking % (daysUntilCorrupted + daysUntilSpentOne);
        this.pactsManager.necrocornDeficit +=
          ((timePeriodWorking * daysUntilCorrupted) /
            (daysUntilCorrupted + daysUntilSpentOne)) *
          necrocornPerDay *
      ...
      if (
        this.game.resPool.get("necrocorn").value +
          necrocornPerDay * days * necrocornDeficitRepaymentModifier -
          compensatedNecrocorns <
        0
      ) {
        this.necrocornDeficit += Math.max(
          -necrocornPerDay * days -
            this.game.resPool.get("necrocorn").value +
            compensatedNecrocorns,
          0
        ...
        this.necrocornDeficit +=
          necrocornPerDay *
          (0.15 * (1 + this.game.getEffect("deficitRecoveryRatio")) * days);
        this.necrocornDeficit = Math.max(this.necrocornDeficit, 0);
      }
      this.game.resPool.addResPerTick(
        "necrocorn",
        necrocornPerDay * necrocornDeficitRepaymentModifier * days -
          compensatedNecrocorns
      );
    ...
    triggerOrderOfTheVoid: function (numberOfTicks) {
      if (this.game.prestige.getPerk("voidOrder").researched) {
        var convertedFaith =
          numberOfTicks *
          this.game.calcResourcePerTick("faith") *
          0.1 *
          (1 + this.game.getEffect("voidResonance"));
    ...
    zigguratUpgrades: [
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          self.effects["corruptionRatio"] =
            0.000001 * (1 + game.getEffect("corruptionBoostRatioChallenge")); //LDR specified in challenges.js
        },
        unlocked: false,
        getEffectiveValue: function (game) {
          return (
            this.val * (1 + game.getEffect("corruptionBoostRatioChallenge"))
      ...
      },
      {
        ...
        cashPreDeficitEffects: function (game) {
          var transcendenceTierModifier = Math.max(
            game.religion.transcendenceTier - 25,
            1
          );
          var self = game.religion.getZU("blackPyramid");
          for (var counter in self.simpleEffectNames) {
            self.effectsPreDeficit[
              "pyramid" + self.simpleEffectNames[counter]
            ] =
              game.getEffect("pact" + self.simpleEffectNames[counter]) *
              transcendenceTierModifier;
          }
          self.effectsPreDeficit["deficitRecoveryRatio"] = game.getEffect(
            "pactDeficitRecoveryRatio"
          );
          var pactBlackLibraryBoost =
            game.getEffect("pactBlackLibraryBoost") * transcendenceTierModifier;
          if (pactBlackLibraryBoost) {
            var unicornGraveyard = game.religion.getZU("unicornGraveyard");
            self.effectsPreDeficit["blackLibraryBonus"] =
              pactBlackLibraryBoost *
              unicornGraveyard.effects["blackLibraryBonus"] *
              (1 + unicornGraveyard.on);
          }
        },
        updateEffects: function (self, game) {
          ...
          self.effects["deficitRecoveryRatio"] =
            self.effectsPreDeficit["deficitRecoveryRatio"];
          //applying deficit
          var deficiteModifier =
            1 - game.religion.pactsManager.necrocornDeficit / 50;
          ...
          for (var name in self.effectsPreDeficit) {
            if (name != "deficitRecoveryRatio") {
              ...
              self.effects[name] =
                self.effectsPreDeficit[name] * deficiteModifier;
        ...
        getEffectiveValue: function (game) {
          return (
            this.val +
            (game.challenges.getChallenge("blackSky").researched &&
    ...
    religionUpgrades: [
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          self.effects["solarRevolutionRatio"] =
            game.religion.getSolarRevolutionRatio();
        },
        noStackable: true
      },
      {
    ...
    transcendenceUpgrades: [
      {
        ...
        calculateEffects: function (self, game) {
          self.effects["solarRevolutionLimit"] =
            0.05 * game.religion.transcendenceTier;
        },
        unlocked: false,
    ...
    necrocornDeficitPunishment: function () {
      ...
      this.game.msg(
        $I("msg.pacts.fractured", [
          Math.round(100 * this.game.resPool.get("alicorn").value) / 100
        ]),
        "alert",
        "ai"
      );
      ...
      this.game.religion
        .getZU("blackPyramid")
        .updateEffects(this.game.religion.getZU("blackPyramid"), this.game);
    },
    ...
    getSolarRevolutionRatio: function () {
      var uncappedBonus = this.getRU("solarRevolution").on
        ? this.game.getUnlimitedDR(this.faith, 1000) / 100
        : 0;
      return (
        this.game.getLimitedDR(
          uncappedBonus,
          10 +
            this.game.getEffect("solarRevolutionLimit") +
            (this.game.challenges.getChallenge("atheism").researched
              ? this.game.religion.transcendenceTier
              : 0)
        ) *
        (1 +
          this.game.getEffect(
    ...
    getHGScalingBonus: function () {
      //TODO: test this
      var scalingRatio = this.game.getLimitedDR(
        this.game.getEffect("simScalingRatio"),
        1
      );
    ...
    praise: function () {
      ...
      this.game.msg(
        $I("religion.praise.msg", [
          this.game.getDisplayValueExt(faith.value, false, false, 0)
        ]),
        "",
        "faith"
      );
    ...
    resetFaith: function (bonusRatio, withConfirmation) {
      if (withConfirmation && !this.game.opts.noConfirm) {
        ...
        this.game.ui.confirm(
          "",
          $I("religion.adore.confirmation.msg"),
          function () {
    ...
    _resetFaithInternal: function (bonusRatio) {
      var ttPlus1 =
        (this.game.religion.getRU("transcendence").on
          ? this.game.religion.transcendenceTier
          : 0) + 1;
      this.faithRatio +=
        (this.faith / 1000000) * ttPlus1 * ttPlus1 * bonusRatio;
    ...
    transcend: function () {
      ...
      var game = this.game;
      game.ui.confirm(
        $I("religion.transcend.confirmation.title"),
        $I("religion.transcend.confirmation.msg"),
        function () {
          ...
          if (religion.faithRatio > needNextLevel) {
            ...
            if (
              game.getFeatureFlag("MAUSOLEUM_PACTS") &&
              game.religion.getTU("mausoleum").val
            ) {
            ...
            game.msg(
              $I("religion.transcend.msg.success", [religion.transcendenceTier])
            );
          } else {
            game.msg(
              $I("religion.transcend.msg.failure", [
                game.toDisplayPercentage(
                  religion.faithRatio / needNextLevel,
                  2,
                  true
                )
    ...
    _getTranscendNextPrice: function () {
      return (
        this._getTranscendTotalPrice(this.transcendenceTier + 1) -
        this._getTranscendTotalPrice(this.transcendenceTier)
      );
    },
  ...
  com.nuclearunicorn.game.ui.BuildingStackableBtnController,
  {
    ...
    getName: function (model) {
      if (model.metadata.name == "marker" && model.metadata.val > 0) {
        var progress = this.game.toDisplayPercentage(
          this.game.religion.corruption,
          0,
          true
        );
        return (
...
dojo.declare(
  ...
  com.nuclearunicorn.game.ui.BuildingStackableBtnController,
  {
    ...
    hasSellLink: function (model) {
      return (
        !this.game.opts.hideSell &&
        !model.metadata.noStackable &&
        model.metadata.val > 1
      );
    },

    getPrices: function (model) {
      return this.game.village.getEffectLeader(
        "wise",
    ...
    updateVisible: function (model) {
      model.visible =
        model.metadata.on > 0 ||
        this.game.religion.faith >= model.metadata.faith;
    }
  }
);
...
dojo.declare(
  "com.nuclearunicorn.game.ui.PraiseBtnController",
    ...
    getName: function (model) {
      if (this.game.religion.faithRatio > 0) {
        return (
          model.options.name +
          " [" +
          this.game.getDisplayValueExt(
            this.game.religion.getApocryphaBonus() * 100,
            true,
            false,
            3
          ) +
...
dojo.declare(
  "com.nuclearunicorn.game.ui.ResetFaithBtnController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    getName: function (model) {
...
dojo.declare(
  "com.nuclearunicorn.game.ui.TranscendBtnController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    getName: function (model) {
      return (
...
dojo.declare(
  "classes.ui.religion.TransformBtnController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
        ...
        title:
          divider == 1
            ...
            : this.game.opts.usePercentageConsumptionValues
              ? 100 / divider + "%"
              : "x" +
                this.game.getDisplayValueExt(transformations, null, false, 0),
        tooltip:
          divider == 1 || this.game.opts.usePercentageConsumptionValues
            ? "x" +
              this.game.getDisplayValueExt(transformations, null, false, 0)
            : 100 / divider + "%",
    ...
    buyItem: function (model, event, callback) {
      ...
      if (!event) {
        event = {}; /*event is an optional parameter*/
      }
      var batchSize = event.shiftKey
        ? 10000
        : event.ctrlKey || event.metaKey
          ? this.game.opts.batchSize
          : 1;
    ...
    _canAfford: function (model) {
      return Math.floor(
        this.game.resPool.get(model.prices[0].name).value / model.prices[0].val
      );
    },
    ...
    _transform: function (model, amt) {
      ...
      this.game.resPool.addResEvent(
        this.controllerOpts.gainedResource,
        gainCount
      );
      ...
      this.game.msg(
        $I(this.controllerOpts.logTextID, [
          this.game.getDisplayValueExt(priceCount),
          this.game.getDisplayValueExt(gainCount)
        ]),
        "",
        this.controllerOpts.logfilterID
      );
...
dojo.declare(
  "classes.ui.religion.MultiLinkBtn",
...
dojo.declare(
  "classes.ui.religion.RefineTearsBtnController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
    ...
    _newLink: function (model, count) {
      ...
      return {
        visible:
          ...
          (this.game.resPool.get("sorrow").value <=
            this.game.resPool.get("sorrow").maxValue - count &&
    ...
    buyItem: function (model, event, callback, count) {
      ...
      for (
        var batchSize = count || (event.ctrlKey ? this.game.opts.batchSize : 1);
        batchSize > 0 &&
...
dojo.declare(
  "classes.ui.CryptotheologyPanel",
...
dojo.declare("classes.ui.PactsWGT", [mixin.IChildrenAware, mixin.IGameAware], {
  ...
  render: function (container) {
    var div = dojo.create("div", null, container);
    var span1 = dojo.create(
      "span",
      { innerHTML: this.game.religion.pactsManager.getPactsTextSum() },
      div
    );
    var btnsContainer = dojo.create("div", null, div);
    var span2 = dojo.create(
      "span",
      { innerHTML: this.game.religion.pactsManager.getPactsTextDeficit() },
      div
    );
    this.inherited(arguments, [btnsContainer]);
  },
 ...
 */
dojo.declare(
  "com.nuclearunicorn.game.ui.PactsBtnController",
  com.nuclearunicorn.game.ui.BuildingStackableBtnController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
    ...
    updateEnabled: function (model) {
      this.inherited(arguments);
      if (
        this.game.getEffect("pactsAvailable") <= 0 &&
        model.metadata.effects["pactsAvailable"] != 0
      ) {
        model.enabled = false;
      }
    },
    shouldBeBough: function (model, game) {
      return (
        game.getEffect("pactsAvailable") +
          model.metadata.effects["pactsAvailable"] >=
        0
      );
    },
    buyItem: function (model, event, callback) {
      ...
      if (!this.shouldBeBough(model, this.game)) {
        callback(false /*itemBought*/, {
          reason: "already-bought" /*No more pacts available*/
        });
        return;
      }
    ...
    build: function (model, maxBld) {
      ...
      var counter = 0;
      if (
        typeof meta.limitBuild == "number" &&
        meta.limitBuild - meta.val < maxBld
      ) {
      ...
      if ((model.enabled && this.hasResources(model)) || this.game.devMode) {
        ...
        if (counter > 1) {
          this.game.msg(
            meta.label + " x" + counter + " constructed.",
            "notice"
          );
        }
  ...
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    ...
    update: function () {
      ...
      dojo.style(
        this.five.link,
        "display",
        this.model.fiveLink.visible ? "" : "none"
      );
      dojo.style(
        this.twentyFive.link,
        "display",
        this.model.twentyFiveLink.visible ? "" : "none"
      );
      dojo.style(
        this.hundred.link,
        "display",
...
dojo.declare("classes.religion.pactsManager", null, {
  ...
  pacts: [
    {
      ...
      calculateEffects: function (self, game) {
        ...
        self.effects["necrocornPerDay"] = game.getEffect(
    ...
    },
    {
      ...
      onNewDay: function (game) {
        ...
        self.prices[0].val = Math.ceil(
          game.religion.pactsManager.necrocornDeficit
        );
      ...
      calculateEffects: function (self, game) {
        ...
        if (self.on >= 1) {
          for (var i = 0; i < game.religion.pactsManager.pacts.length; i++) {
            ...
            game.religion.pactsManager.pacts[i].unlocked =
              game.religion.pactsManager.pacts[i].name == "fractured";
          }
          self.val = 1;
          self.on = 1;
        }
  ...
  getPactsTextSum: function () {
    return $I("msg.pacts.info", [
      this.game.getEffect("pactsAvailable"),
      -this.game.getEffect("pactNecrocornConsumption")
    ]); //Every TT above 25 adds 100% to pact effects (not consumption) and 10% to karma per millenia effect
  },
  getPactsTextDeficit: function () {
    if (this.game.religion.pactsManager.necrocornDeficit > 0) {
      return $I("msg.necrocornDeficit.info", [
        Math.round(this.game.religion.pactsManager.necrocornDeficit * 10000) /
          10000,
        -Math.round(
          100 * (this.game.religion.pactsManager.necrocornDeficit / 50)
        ),
        Math.round(
          10000 * (0.15 * (1 + this.game.getEffect("deficitRecoveryRatio") / 2))
        ) / 100,
        -Math.round(
          this.game.getEffect("necrocornPerDay") *
            (0.15 * (1 + this.game.getEffect("deficitRecoveryRatio"))) *
            1000000
        ) / 1000000
      ]);
    } else {
  ...
  getNecrocornDeficitConsumptionModifier: function () {
    ...
    if (this.game.science.getPolicy(["siphoning"]).researched) {
      if (
        this.game.religion.getCorruptionDeficitPerTick() == 0 &&
        this.game.resPool.get("alicorn").value - necrocornPerDay >= 1
      ) {
        //check if siphening is enough to pay for per day consumption
        return 1;
      }
    }
    var necrocornDeficitRepaymentModifier = 1;
    ...
    if (
      ...
      necrocornDeficitRepaymentModifier = Math.max(
        (necrocornPerDay * necrocornDeficitRepaymentModifier +
          this.game.resPool.get("necrocorn").value) /
          necrocornPerDay,
        0
  ...
  getSiphonedCorruption: function (days) {
    ...
    if (this.game.science.getPolicy(["siphoning"]).researched) {
      ...
      return Math.min(
        -necrocornPerDay * days,
        this.game.religion.getCorruptionPerTickProduction() *
          this.game.calendar.ticksPerDay *
          days
      );
    }
  ...
  necrocornConsumptionDays: function (days) {
    ...
    if (this.game.science.getPolicy(["siphoning"]).researched) {
      if (
        this.game.religion.getCorruptionDeficitPerTick() == 0 &&
        this.game.resPool.get("alicorn").value - necrocornPerDay * days >= 1
      ) {
        //check if siphening is enough to pay for per day consumption
  ...
  pactsMilleniumKarmaKittens: function (millenium) {
    ...
    var kittens = Math.round(
      this.game.resPool.get("kittens").value *
        (1 + this.game.getEffect("simScalingRatio"))
    );
    if (kittens > 35 && this.game.getEffect("pactsAvailable") > 0) {
      var oldKarmaKittens = this.game.karmaKittens;
      var kittensKarmaPerMinneliaRatio = this.game.getEffect(
        "kittensKarmaPerMinneliaRatio"
      );
      this.game.karmaKittens +=
        millenium *
        this.game._getKarmaKittens(kittens) *
      ...
      console.log(
        "produced " +
          String(this.game.resPool.get("karma").value - karmaOld) +
          " karma"
      );
      console.log(
        "produced " +
          String(this.game.karmaKittens - oldKarmaKittens) +
          " karmaKittens"
      ); //for testing purposes - comment over before merging into ML
...
dojo.declare(
  "com.nuclearunicorn.game.ui.tab.ReligionTab",
  com.nuclearunicorn.game.ui.tab,
  {
    ...
    constructor: function () {
      ...
      var ctPanel = new classes.ui.CryptotheologyPanel(
        $I("religion.panel.cryptotheology.label")
      );
    ...
    render: function (container) {
      ...
      if (zigguratCount > 0) {
        var zigguratPanel = new com.nuclearunicorn.game.ui.Panel(
          $I("religion.panel.ziggurat.label"),
          game.religion
        );
            ...
            controller: new classes.ui.religion.TransformBtnController(game, {
              updateVisible: function (model) {
                model.visible =
                  this.hasResources(model) ||
                  (this.game.resPool.get("alicorn").value > 0 &&
                    this.game.resPool.get("timeCrystal").unlocked);
              },
              gainMultiplier: function () {
        ...
        var refineBtn = new classes.ui.religion.RefineBtn(
          {
            ...
            controller: new classes.ui.religion.RefineTearsBtnController(game, {
              updateVisible: function (model) {
                model.visible =
                  this.game.religion.getZU("blackPyramid").unlocked;
              }
            })
          },
          game
        ...
        var refineTCBtn = new classes.ui.religion.MultiLinkBtn(
          {
            ...
            controller: new classes.ui.religion.TransformBtnController(game, {
              ...
              gainMultiplier: function () {
                return (
                  1 +
                  this.game.getEffect("relicRefineRatio") *
                    this.game.religion
                      .getZU("blackPyramid")
                      .getEffectiveValue(this.game)
                );
              },
        ...
        var zigguratController =
          new com.nuclearunicorn.game.ui.ZigguratBtnController(game);
      ...
      if (!game.challenges.isActive("atheism")) {
        //------------------- religion -------------------
        var religionPanel = new com.nuclearunicorn.game.ui.Panel(
          $I("religion.panel.orderOfTheSun.label"),
          game.religion
        );
        var content = religionPanel.render(container);

        var faithCount = dojo.create(
          "span",
        ...
        this.praiseBtn = new com.nuclearunicorn.game.ui.ButtonModern(
          {
            ...
            controller: new com.nuclearunicorn.game.ui.PraiseBtnController(
              game
            ),
            handler: function () {
        ...
        this.adoreBtn = new com.nuclearunicorn.game.ui.ButtonModern(
          {
            ...
            controller: new com.nuclearunicorn.game.ui.ResetFaithBtnController(
              game
            ),
        ...
        this.transcendBtn = new com.nuclearunicorn.game.ui.ButtonModern(
          {
            ...
            controller: new com.nuclearunicorn.game.ui.TranscendBtnController(
              game
            ),
            handler: function (btn) {
              ...
              for (
                var i = 0;
                i < game.religion.transcendenceUpgrades.length;
                i++
              ) {
                var check =
                  !game.religion.transcendenceUpgrades[i].evaluateLocks ||
                  game.religion.transcendenceUpgrades[i].evaluateLocks(game);
                if (
                  check &&
                  transcendenceLevel >=
                    game.religion.transcendenceUpgrades[i].tier
                ) {
        ...
        var controller = new com.nuclearunicorn.game.ui.ReligionBtnController(
          game
        );
    ...
    update: function () {
      ...
      if (!this.game.challenges.isActive("atheism")) {
        ...
        if (religion.faith && this.faithCount) {
          this.faithCount.innerHTML = $I("religion.faithCount.pool", [
            this.game.getDisplayValueExt(religion.faith)
          ]);
        } else {
        ...
        if (bonus != 0) {
          this.faithCount.innerHTML +=
            " (+" +
            this.game.getDisplayValueExt(100 * bonus) +
            "% " +
            $I("religion.faithCount.bonus") +
            ")";
        }

        dojo.forEach(this.rUpgradeButtons, function (e, i) {
          e.update();
        });
      }
      var hasCT =
        this.game.science.get("cryptotheology").researched &&
        this.game.religion.transcendenceTier > 0;
      if (hasCT) {
        this.ctPanel.setVisible(true);
      }

      dojo.forEach(this.zgUpgradeButtons, function (e, i) {
        e.update();
      });
      var canSeePacts =
        !this.game.religion.getPact("fractured").researched &&
        this.game.religion.getZU("blackPyramid").val > 0 &&
        (this.game.religion.getTU("mausoleum").val > 0 ||
          this.game.science.getPolicy("radicalXenophobia").researched);

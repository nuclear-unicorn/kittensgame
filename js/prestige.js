dojo.declare(
  "classes.managers.PrestigeManager",
  com.nuclearunicorn.core.TabManager,
  {
    ...
    save: function (saveData) {
      saveData.prestige = {
        perks: this.filterMetadata(this.perks, [
          "name",
          "unlocked",
          "researched"
        ])
    ...
    getBurnedParagonRatio: function () {
      return this.game.getUnlimitedDR(
        this.game.resPool.get("burnedParagon").value,
        500
      );
    },
    ...
    getParagonProductionRatio: function () {
      ...
      var productionRatioParagon =
        this.game.resPool.get("paragon").value * 0.01 * paragonRatio;
      productionRatioParagon = this.game.getLimitedDR(
        productionRatioParagon,
        2 * paragonRatio
      );

      var ratio = this.game.calendar.darkFutureYears() >= 0 ? 4 : 1;
      var productionRatioBurnedParagon =
        this.game.resPool.get("burnedParagon").value * 0.01 * paragonRatio;
      productionRatioBurnedParagon = this.game.getLimitedDR(
        productionRatioBurnedParagon,
        ratio * paragonRatio
      );
    ...
    getParagonStorageRatio: function () {
      var paragonRatio = this.getParagonRatio();
      var storageRatio =
        (this.game.resPool.get("paragon").value / 1000) * paragonRatio; //every 100 paragon will give a 10% bonus to the storage capacity
      if (this.game.calendar.darkFutureYears() >= 0) {
        storageRatio +=
          (this.game.resPool.get("burnedParagon").value / 500) * paragonRatio;
      } else {
        storageRatio +=
          (this.game.resPool.get("burnedParagon").value / 2000) * paragonRatio;
      }
      return storageRatio;
    },
...
dojo.declare(
  "classes.ui.PrestigeBtnController",
  com.nuclearunicorn.game.ui.BuildingNotStackableBtnController,
  {
    ...
    updateVisible: function (model) {
      ...
      model.visible =
        meta.unlocked &&
        (meta.researched || this.game.science.get("metaphysics").researched);
...
dojo.declare(
  "classes.ui.BurnParagonBtnController",
...
dojo.declare(
  "classes.ui.turnHGOffButtonController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    updateVisible: function (model) {
      model.visible =
        this.game.religion.activeHolyGenocide > 0 ||
        this.game.religion.getTU("holyGenocide").on > 0;
    }
  }
);
  ...
  render: function (container) {
    ...
    dojo.forEach(this.game.prestige.perks, function (perk, i) {
      var button = new com.nuclearunicorn.game.ui.BuildingResearchBtn(
        { id: perk.name, controller: controller },
        self.game
      );

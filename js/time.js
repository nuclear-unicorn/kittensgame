  ...
  com.nuclearunicorn.core.TabManager,
  {
    ...
    save: function (saveData) {
      saveData.time = {
        ...
        cfu: this.filterMetadata(this.chronoforgeUpgrades, [
          "name",
          "val",
          "on",
          "heat",
    ...
    load: function (saveData) {
      ...
      if (saveData.time.usedCryochambers) {
        //after reset
        this.loadMetadata(
          this.voidspaceUpgrades,
          saveData.time.usedCryochambers
        );
      }
      ...
      this.queue.queueSources =
        saveData["time"].queueSources || this.queue.queueSourcesDefault;
      if (
        this.queue.queueSources === {} ||
        typeof this.queue.queueSources["buildings"] != "boolean"
      ) {
    ...
    gainTemporalFlux: function (timestamp) {
      ...
      var energyRatio =
        1 + temporalAccelerator.val * temporalAccelerator.effects["timeRatio"];
      var temporalFluxGained = Math.round(
        (delta / (60 * 1000)) * (this.game.ticksPerSecond * energyRatio)
      ); // 5 every 60 seconds

      var temporalFluxAdded = this.game.resPool.addResEvent(
        "temporalFlux",
        temporalFluxGained
      );

      var bonusSeconds = Math.floor(
        temporalFluxAdded / this.game.ticksPerSecond
      );
      if (bonusSeconds > 0) {
    ...
    applyRedshift: function (daysOffset, ignoreCalendar) {
      ...
      if (this.heat > 0) {
        ...
        var heatAttemptTransfer =
          daysOffset * this.game.calendar.ticksPerDay * perTickHeatTransfer;
        ...
        if (
          blastFurnace.on &&
          blastFurnace.isAutomationEnabled &&
          blastFurnace.heat >= 100
        ) {
          var amt = Math.floor(blastFurnace.heat / 100);
          blastFurnace.heat -= 100 * amt;
          //this.shatter(amt);
          if (this.testShatter == 1) {
            this.shatterInGroupCycles(amt);
          }
    ...
    calculateRedshift: function () {
      ...
      this.game.msg(
        $I("time.redshift", [daysOffset]) +
          (numberEvents ? $I("time.redshift.ext", [numberEvents]) : "")
      );
    },
    ...
    chronoforgeUpgrades: [
      ...
      },
      {
        ...
        description:
          $I("time.cfu.blastFurnace.desc") +
          "<br>" +
          $I("time.cfu.blastFurnace.desc2"),
        prices: [
        ...
        action: function (self, game) {
          ...
          if (self.heat >= 100) {
            ...
            if (
              game.time.getCFU("temporalPress").isAutomationEnabled &&
              test_shatter == 1
            ) {
            ...
            if (test_shatter == 1) {
              game.time.shatterInGroupCycles(amt);
            }
      ...
      },
      {
        ...
        description:
          $I("time.cfu.temporalAccelerator.desc") +
          "<br>" +
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          self.effects["timeImpedance"] = Math.round(
            1000 * (1 + game.getEffect("timeRatio"))
          );
        },
        unlocked: false
      },
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          if (game.challenges.getChallenge("1000Years").on > 1) {
            ...
            self.description =
              $I("time.cfu.temporalPress.desc") +
              "<br>" +
              $I("time.cfu.temporalPress.desc.automation");
            if (self.isAutomationEnabled == null) {
          ...
          self.effects["shatterYearBoost"] = self.isAutomationEnabled
            ? 5 * game.calendar.yearsPerCycle
            : game.calendar.yearsPerCycle; //25 or 5 currently
          self.limitBuild = game.getEffect("temporalPressCap");
          self.priceRatio = Math.max(
            1.01,
            1.1 - game.challenges.getChallenge("1000Years").on * 0.001
          ); //first 90 completions of 1000Years make priceRatio cheaper
        },
        isAutomationEnabled: null,
        unlocked: false
      }
    ...
    voidspaceUpgrades: [
      {
        ...
        calculateEffects: function (self, game) {
          self.limitBuild =
            game.bld.get("chronosphere").on +
            game.getEffect("cryochamberSupport");
      ...
      },
      {
        ...
        calculateEffects: function (self, game) {
          self.effects["temporalParadoxDay"] =
            1 + game.getEffect("temporalParadoxDayBonus");
        },
        unlockScheme: {
          name: "vintage",
          threshold: 1
    ...
    shatter: function (amt) {
      ...
      var shatterTCGain =
        game.getEffect("shatterTCGain") * (1 + game.getEffect("rrRatio"));
      var triggersOrderOfTheVoid = game.getEffect("voidResonance") > 0;

      var daysPerYear = cal.daysPerSeason * cal.seasonsPerYear;
      var remainingDaysInFirstYear =
        cal.daysPerSeason * (cal.seasonsPerYear - cal.season) - cal.day;
      cal.day = 0;
      cal.season = 0;

      for (var i = 0; i < amt; i++) {
        var remainingDaysInCurrentYear =
          i == 0 ? remainingDaysInFirstYear : daysPerYear;
        var remainingTicksInCurrentYear =
          remainingDaysInCurrentYear * cal.ticksPerDay;

        // Space ETA
        for (var j in game.space.planets) {
          var planet = game.space.planets[j];
          if (planet.unlocked && !planet.reached) {
            planet.routeDays = Math.max(
              0,
              planet.routeDays - remainingDaysInCurrentYear * routeSpeed
            );
          }
        }
        ...
        if (shatterTCGain > 0) {
          ...
          for (var j = 0; j < game.resPool.resources.length; j++) {
            ...
            limits[res.name] = Math.max(
              res.value,
              res.maxValue || Number.MAX_VALUE
            );
            game.resPool.addRes(
              res,
              game.getResourcePerTick(res.name, true) *
                remainingTicksInCurrentYear *
                shatterTCGain,
              false,
              true
            );
          ...
          if (this.game.workshop.get("chronoEngineers").researched) {
            this.game.workshop.craftByEngineers(
              remainingTicksInCurrentYear * shatterTCGain
            );
          }
          for (var j = 0; j < game.resPool.resources.length; j++) {
            var res = game.resPool.resources[j];
          ...
          game.bld.cathPollutionFastForward(
            remainingTicksInCurrentYear * shatterTCGain
          );
        }
    ...
    shatterInCycles: function (amt) {
      ...
      for (var j in game.space.planets) {
        ...
        if (planet.unlocked && !planet.reached) {
          planet.routeDays = Math.max(
            0,
            planet.routeDays - remainingDays * routeSpeed
          );
        }
      }

      while (maxYearsShattered > 0) {
        var remainingYearsInCurrentCycle = Math.min(
          cal.yearsPerCycle - cal.cycleYear,
          maxYearsShattered
        );
        var remainingDaysInCurrentCycle =
          (remainingYearsInCurrentCycle - 1) * daysPerYear +
          remainingDaysInFirstYear;
        var remainingTicksInCurrentCycle =
          remainingDaysInCurrentCycle * cal.ticksPerDay;
        ...
        if (shatterTCGain > 0) {
          ...
          for (var j = 0; j < game.resPool.resources.length; j++) {
            ...
            limits[res.name] = Math.max(
              res.value,
              res.maxValue || Number.POSITIVE_INFINITY
            );
            game.resPool.addRes(
              res,
              game.getResourcePerTick(res.name, true) *
                remainingTicksInCurrentCycle *
                shatterTCGain,
              false,
              true
            );
          ...
          if (this.game.workshop.get("chronoEngineers").researched) {
            this.game.workshop.craftByEngineers(
              remainingTicksInCurrentCycle * shatterTCGain
            );
          }
          for (var j = 0; j < game.resPool.resources.length; j++) {
            var res = game.resPool.resources[j];
        ...
        cal.onNewYears(
          endYear == cal.year,
          remainingYearsInCurrentCycle,
          false
        );
        cal.calculateMilleniumProduction(
          cal.getMilleniaChanged(
            cal.year - remainingYearsInCurrentCycle,
            cal.year
          )
        );
    ...
    */
    shatterInGroupCycles: function (amt) {
      ...
      if (
        aiLevel > 14 &&
        this.game.science.getPolicy("transkittenism").researched != true &&
        amt != 1
      ) {
        //if amt == 1 we just use usual onNewYear calendar function
      ...
      if (maxYearsShattered % 50 == 0) {
        ...
        remainingCyclesYears[cal.cycle] += Math.min(
          cal.yearsPerCycle - cal.cycleYear,
          maxYearsShattered
        );
        maxYearsShattered -= Math.min(
          cal.yearsPerCycle - cal.cycleYear,
          maxYearsShattered
        );
        for (j = 1; j < cal.cyclesPerEra; j++) {
          remainingCyclesYears[(cal.cycle + j) % 10] += Math.min(
            cal.yearsPerCycle,
            maxYearsShattered
          );
      ...
      for (var cycleNum = 0; cycleNum < cal.cyclesPerEra; cycleNum++) {
        var yearsInCurrentCycle =
          remainingCyclesYears[(cycleNum + startingCycleNum) % 10];
        if (!yearsInCurrentCycle) {
          continue;
        }
        var daysInCurrentCycle =
          (yearsInCurrentCycle - 1) * daysPerYear + remainingDaysInFirstYear;
        var ticksInCurrentCycle = daysInCurrentCycle * cal.ticksPerDay;

        // ShatterTC gain
        if (shatterTCGain > 0) {
          if (yearsInCurrentCycle == 1) {
            //no need to do AI Apocalypse twice
            aiApocalypseLevel = 0;
          }
          // XXX Partially duplicates resources#fastforward and #enforceLimits, some nice factorization is probably possible
          var limits = {};
          ...
          for (var j = 0; j < game.resPool.resources.length; j++) {
            var res = game.resPool.resources[j];
            var resLimit = Math.max(
              res.value,
              res.maxValue || Number.POSITIVE_INFINITY
            );
            var deltaRes = game.resPool.addRes(
              res,
              game.getResourcePerTick(res.name, true) *
                ticksInCurrentCycle *
                shatterTCGain,
          ...
          if (this.game.workshop.get("chronoEngineers").researched) {
            this.game.workshop.craftByEngineers(
              ticksInCurrentCycle * shatterTCGain
            );
          }
          for (var j = 0; j < game.resPool.resources.length; j++) {
            var res = game.resPool.resources[j];
            ...
            if (aiApocalypseLevel && res.aiCanDestroy) {
              ...
              if (
                resLimit == res.MaxValue &&
                oldVal +
                  delta[res.name] -
                  (oldVal + delta[res.name]) * aiDestructionMod >=
                  resLimit
              ) {
                resLimit =
                  Math.min(resLimit, res.value) * (1 + aiDestructionMod);
                ...
                var decreaseOfDelta =
                  (-delta[res.name] *
                    (1 -
                      Math.abs(
                        Math.pow(aiDestructionMod, yearsInCurrentCycle)
                      ))) /
                  (Math.abs(1 - aiDestructionMod) || 1);
                game.resPool.addResEvent(
                  res.name,
                  decreaseOfDelta -
                    oldVal *
                      (1 - Math.pow(1 + aiDestructionMod, yearsInCurrentCycle))
                ); //this is no longer broken
              } /*if (resLimit == res.value)*/ else {
                resLimit =
                  Math.min(resLimit, res.value) *
                  Math.pow(1 + aiDestructionMod, yearsInCurrentCycle);
              }
            }
            res.value = Math.min(res.value, limits[res.name]);
          }
        ...
        cal.onNewYears(
          endYear == cal.year,
          Math.min(5, maxYearsShattered),
          false
        );
        maxYearsShattered -= Math.min(5, maxYearsShattered);
        remainingDaysInFirstYear = cal.daysPerSeason * cal.seasonsPerYear;
      }
      if (maxYearsShattered < 0) {
        console.error(
          "max years shattered negative " + toString(maxYearsShattered)
        );
      }
      cal.year += maxYearsShattered;
      cal.onNewYears(endYear == cal.year, maxYearsShattered, false);
      cal.calculateMilleniumProduction(
        cal.getMilleniaChanged(startYear, cal.year)
      );
      if (amt == 1) {
        game.msg($I("time.tc.shatterOne"), "", "tcShatter");
      } else {
      ...
      if (aiApocalypseLevel) {
        this.game.msg(
          $I("ai.apocalypse.msg", [aiApocalypseLevel]),
          "alert",
          "ai"
        );
      }
    ...
    compareShatterTime: function (
      shatters,
      times,
      ignoreOldFunction,
      ignoreShatterInCycles,
      ignoreGroupCycles
    ) {
      if (!ignoreOldFunction) {
        ...
        console.log(
          "oldShatterAverafe = " +
            (oldShatterD2.getTime() - oldShatterD1.getTime()) / times
        );
      }
      if (!ignoreGroupCycles) {
        ...
        console.log(
          "Group shatter average = " +
            (newShatterD2.getTime() - newShatterD1.getTime()) / times
        );
      }
      if (!ignoreShatterInCycles) {
        ...
        if (!ignoreShatterInCycles) {
          console.log(
            "Cycle shatter average= " +
              (new1ShatterD2.getTime() - new1ShatterD1.getTime()) / times
          );
        }
      }
      ...
      if (!ignoreOldFunction && !ignoreGroupCycles) {
        console.log(
          "newEfficensy = " +
            (oldShatterD2.getTime() - oldShatterD1.getTime()) /
              (newShatterD2.getTime() - newShatterD1.getTime())
        );
      }
      ...
      if (!ignoreOldFunction && !ignoreShatterInCycles) {
        console.log(
          "new1Efficensy = " +
            (oldShatterD2.getTime() - oldShatterD1.getTime()) /
              (new1ShatterD2.getTime() - new1ShatterD1.getTime())
        );
      }
    },
    unlockAll: function () {
...
dojo.declare(
  "classes.ui.time.AccelerateTimeBtnController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    fetchModel: function (options) {
      ...
      var self = this;
      model.toggle = {
        title: this.game.time.isAccelerated
          ? $I("btn.on.minor")
          : $I("btn.off.minor"),
        tooltip: this.game.time.isAccelerated
          ? $I("time.AccelerateTimeBtn.tooltip.accelerated")
          : $I("time.AccelerateTimeBtn.tooltip.normal"),
    ...
    buyItem: function (model, event, callback) {
      ...
      callback(true /*itemBought*/, {
        reason:
          "item-is-free" /*It costs flux, but you can still toggle it freely*/
      });
    }
  }
);
...
dojo.declare(
  "classes.ui.time.AccelerateTimeBtn",
...
dojo.declare(
  "classes.ui.TimeControlWgt",
  [mixin.IChildrenAware, mixin.IGameAware],
  {
    constructor: function (game) {
      this.addChild(
        new classes.ui.time.AccelerateTimeBtn(
          {
    ...
    update: function () {
      ...
      this.timeSpan.innerHTML =
        $I("time.flux") +
        ": " +
        this.game.getDisplayValueExt(temporalFlux.value) +
        " / " +
        temporalFlux.maxValue;

      var remainingTemporalFluxInSeconds =
        temporalFlux.value / this.game.ticksPerSecond;
      this.timeSpan.innerHTML +=
        " (" +
        (remainingTemporalFluxInSeconds < 1
          ? "0" + $I("unit.s")
          : this.game.toDisplaySeconds(remainingTemporalFluxInSeconds)) +
        " / " +
        this.game.toDisplaySeconds(
          temporalFlux.maxValue / this.game.ticksPerSecond
        ) +
        ")";
      ...
      if (this.game.workshop.get("chronoforge").researched) {
        ...
        if (this.game.time.heat > heatMax) {
          ...
          this.timeSpan.innerHTML += this.game.getDisplayValueExt(
            this.game.time.heat
          );
        }
        this.timeSpan.innerHTML +=
          " / " + this.game.getDisplayValueExt(heatMax);

        var heatPerSecond =
          this.game.getEffect("heatPerTick") * this.game.ticksPerSecond;
        var remainingHeatDissipationInSeconds =
          this.game.time.heat / heatPerSecond;
        this.timeSpan.innerHTML +=
          " (" +
          (remainingHeatDissipationInSeconds < 1
            ? "0" + $I("unit.s")
            : this.game.toDisplaySeconds(remainingHeatDissipationInSeconds)) +
          " / " +
...
dojo.declare(
  "classes.ui.time.ShatterTCBtnController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
    ...
    fetchModel: function (options) {
      var model = this.inherited(arguments);
      model.nextCycleLink = this._newLink(
        model,
        this.game.calendar.yearsPerCycle
      );
      model.previousCycleLink = this._newLink(
        model,
        this.game.calendar.yearsPerCycle * (this.game.calendar.cyclesPerEra - 1)
      );
      model.tenErasLink = this._newLink(
        model,
        10 * this.game.calendar.yearsPerCycle * this.game.calendar.cyclesPerEra
      );
    ...
    _newLink: function (model, shatteredQuantity) {
      ...
      return {
        visible:
          ...
          (this.getPricesMultiple(model, shatteredQuantity).timeCrystal <=
            this.game.resPool.get("timeCrystal").value &&
            this.getPricesMultiple(model, shatteredQuantity).void <=
              this.game.resPool.get("void").value),
    ...
    getPrices: function (model) {
      ...
      for (var i in prices_cloned) {
        ...
        if (price["name"] == "timeCrystal") {
          ...
          price["val"] *=
            1 +
            this.game.getEffect("shatterCostReduction") +
    ...
    getPricesMultiple: function (model, amt) {
      ...
      var heatFactor = this.game.challenges.getChallenge("1000Years").researched
        ? 5
        : 10;
        ...
        for (var i in prices_cloned) {
          ...
          if (price["name"] == "timeCrystal") {
            ...
            if (this.game.time.heat + k * heatFactor > heatMax) {
              priceLoop *=
                1 + (this.game.time.heat + k * heatFactor - heatMax) * 0.01; //1% per excessive heat unit
            }
    ...
    doShatterAmt: function (model, amt) {
      ...
      if (price.void) {
        if (
          price.timeCrystal <= this.game.resPool.get("timeCrystal").value &&
    ...
    doShatter: function (model, amt) {
      var factor = this.game.challenges.getChallenge("1000Years").researched
        ? 5
        : 10;
      this.game.time.heat += amt * factor;
      //this.game.time.shatter(amt);
      if (this.game.time.testShatter == 1) {
        this.game.time.shatterInGroupCycles(amt);
      }
  ...
  com.nuclearunicorn.game.ui.ButtonModern,
  {
    ...
    update: function () {
      ...
      dojo.style(
        this.nextCycle.link,
        "display",
        this.model.nextCycleLink.visible ? "" : "none"
      );
      dojo.style(
        this.previousCycle.link,
        "display",
        this.model.previousCycleLink.visible ? "" : "none"
      );
      dojo.style(
        this.tenEras.link,
        "display",
        this.model.tenErasLink.visible ? "" : "none"
      );
      if (this.custom) {
        dojo.style(
          this.custom.link,
          "display",
          this.model.customLink && this.model.customLink.visible ? "" : "none"
        );
      }
...
dojo.declare(
  ...
  com.nuclearunicorn.game.ui.BuildingStackableBtnController,
  {
    ...
    getName: function (model) {
      ...
      if (meta.heat) {
        return (
          this.inherited(arguments) +
          " [" +
          this.game.getDisplayValueExt(meta.heat) +
          "%]"
        );
      }
    ...
    handleToggleAutomationLinkClick: function (model) {
      //specify game.upgrade for cronoforge upgrades
...
dojo.declare(
  "classes.ui.ChronoforgeWgt",
  [mixin.IChildrenAware, mixin.IGameAware],
  {
    constructor: function (game) {
      this.addChild(
        new classes.ui.time.ShatterTCBtn(
          {
      ...
      for (var i in game.time.chronoforgeUpgrades) {
        ...
        this.addChild(
          new com.nuclearunicorn.game.ui.BuildingStackableBtn(
            { id: meta.name, controller: controller },
            game
          )
        );
      }
          ...
          if (prices[i].name == "karma") {
            prices[i].val -=
              ...
              this.game.getLimitedDR(
                0.01 * this.game.prestige.getBurnedParagonRatio(),
                1
              );
          }
        }
...
dojo.declare(
  "classes.ui.time.FixCryochamberBtnController",
  com.nuclearunicorn.game.ui.ButtonModernController,
  {
    defaults: function () {
      var result = this.inherited(arguments);
    ...
    buyItem: function (model, event, callback) {
      ...
      for (
        var count = 0;
        count < fixCount && this.hasResources(model);
        ++count
      ) {
        this.payPrice(model);
    ...
    updateVisible: function (model) {
      model.visible =
        this.game.workshop.get("chronoforge").researched &&
        this.game.time.getVSU("usedCryochambers").val != 0;
    },
...
dojo.declare("classes.ui.ResetWgt", [mixin.IChildrenAware, mixin.IGameAware], {
  ...
  update: function () {
    ...
    var stripe = 5;
    var karmaPointsPresent = this.game.getUnlimitedDR(
      this.game.karmaKittens,
      stripe
    );
    var karmaPointsAfter = this.game.getUnlimitedDR(karmaKittens, stripe);
    var karmaPoints =
      Math.floor((karmaPointsAfter - karmaPointsPresent) * 100) / 100;
    ...
    if (this.game.ironWill) {
      msg +=
        "<br>" + $I("time.reset.zebra") + ": " + this.game._getBonusZebras();
    }
...
dojo.declare("classes.tab.TimeTab", com.nuclearunicorn.game.ui.tab, {
  ...
  update: function () {
    ...
    var hasVS =
      this.game.science.get("voidSpace").researched ||
      this.game.time.getVSU("usedCryochambers").val > 0;
    if (hasVS) {
...
dojo.declare("classes.queue.manager", null, {
  ...
  getFirstItemEtaDay: function () {
    ...
    for (var ind in prices) {
      ...
      var engineersProduced = this.game.workshop.getEffectEngineer(
        res.name,
        true
      );
      var deltaPerTick =
        resPerTick + (engineersConsumed[res.name] || 0) + engineersProduced;
      if (deltaPerTick <= 0) {
      ...
      if (engineersProduced) {
        var countdown =
          1 /
          this.game.workshop.getEffectEngineer(res.name, false) /
  ...
  unlockQueueSource: function (source) {
    if (this.queueSources[source] === false) {
      ...
      this.queueSourcesArr.push({
        name: source,
        label: this.queueLabels[source]
      });
    }
  },
  cap: 0,
  ...
  resetState: function () {
    ...
    this.queueSourcesArr = [
      { name: "buildings", label: $I("buildings.tabName") }
    ];
  },
  ...
  calculateCap: function () {
    ...
    return (
      aiCore.meta.on +
      this.game.space.getBuilding("entangler").effects["hashRateLevel"] +
      this.baseCap +
      this.game.getEffect("queueCap")
    );
  },
  ...
  addToQueue: function (name, type, label, shiftKey /*add all*/) {
    if (!name || !type) {
      console.error(
        "queueMgr#addToQueue: unable to add item:",
        name,
        type,
        label
      );
  ...
  getQueueOptionsAlphabetical: function (type) {
    return this.getQueueOptionsUnsorted(type).sort(function (a, b) {
    ...
    switch (type) {
      ...
      case "religion":
        ...
        for (var i in religionUpgrades) {
          ...
          if (
            upgrade.faith <= this.game.religion.faith &&
            (!upgrade.noStackable || !upgrade.val)
          ) {
            options.push({
              name: upgrade.name,
              label: upgrade.label
  ...
  getQueueElementControllerAndModel: function (el) {
    ...
    switch (el.type) {
      case "policies":
        ...
        break;
      case "tech":
        props.controller = new com.nuclearunicorn.game.ui.TechButtonController(
          this.game
        );
        var model = props.controller.fetchModel(props);
        break;
      ...
      case "spaceMission":
        props.controller =
          new com.nuclearunicorn.game.ui.SpaceProgramBtnController(this.game);
        var model = props.controller.fetchModel(props);
        break;

      case "spaceBuilding":
        props.controller = new classes.ui.space.PlanetBuildingBtnController(
          this.game
        );
        var model = props.controller.fetchModel(props);
        break;
      ...
      case "chronoforge":
        props.controller = new classes.ui.time.ChronoforgeBtnController(
          this.game
        );
        var model = props.controller.fetchModel(props);
        break;
      ...
      case "voidSpace":
        props.controller = new classes.ui.time.VoidSpaceBtnController(
          this.game
        );
        var model = props.controller.fetchModel(props);
        if (el.name == "usedCryochambers") {
          //a bunch of model black magic
          props.controller = new classes.ui.time.FixCryochamberBtnController(
            this.game
          );
      ...
      case "zigguratUpgrades":
        props.controller = new com.nuclearunicorn.game.ui.ZigguratBtnController(
          this.game
        );
        var model = props.controller.fetchModel(props);
        break;
      ...
      case "religion":
        props.controller = new com.nuclearunicorn.game.ui.ReligionBtnController(
          this.game
        );
        var model = props.controller.fetchModel(props);
        break;
      ...
      case "pacts":
        props.controller = new com.nuclearunicorn.game.ui.PactsBtnController(
          this.game
        );
        var model = props.controller.fetchModel(props);
        break;
      ...
      case "upgrades":
        props.controller =
          new com.nuclearunicorn.game.ui.UpgradeButtonController(this.game);
        var model = props.controller.fetchModel(props);
        break;
  ...
  update: function () {
    ...
    if (
      !controllerAndModel ||
      !controllerAndModel.controller ||
      !controllerAndModel.model
    ) {
    ...
    controllerAndModel.controller.buyItem(
      controllerAndModel.model,
      null,
    ...
    if (
      typeof wasItemBought !== "boolean" ||
      typeof resultOfBuyingItem !== "object"
    ) {
      console.error(
        "Invalid result after attempting to buy item via queue",
        resultOfBuyingItem
      );
      return;
    }

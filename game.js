...
dojo.declare("classes.game.Telemetry", [mixin.IDataStorageAware], {
  ...
  generateGuid: function () {
    return "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(
      ...
      function (c) {
        return (
          c == "x" ? (16 * Math.random()) | 0 : (4 * Math.random()) | 8
        ).toString(16);
      }
    );
  },
  ...
  load: function (data) {
    ...
    if (window.newrelic && !this.game.opts.disableTelemetry) {
      ...
      window.newrelic.addRelease(
        "KG",
        this.version + ".r" + this.buildRevision
      );
      ...
      if (this.game.server.userProfile) {
        window.newrelic.setCustomAttribute(
          "uid",
          this.game.server.userProfile.uid
        );
      }
  ...
  effectMeta: function (effectName) {
    ...
    for (var i = 0; i < game.resPool.resources.length; i++) {
      ...
      if (effectName.indexOf(res.name) == 0) {
        ...
        restitle =
          restitle.charAt(0).toUpperCase() +
          restitle.substring(1, restitle.length);
    ...
    switch (true) {
      ...
      case type == "PerTickCon" ||
        type == "PerTickAutoprod" ||
        type == "PerTickProd" ||
        type == "PerTickSpace" ||
        type == "PerTickAutoprodSpace":
        return {
  ...
  statics: {
    effectMeta: {
      ...
      temporalFluxProductionChronosphere: {
        title: $I(
          "effectsMgr.statics.temporalFluxProductionChronosphere.title"
        ),
        type: "perYear"
      },
      ...
      "sattelite-starchartPerTickBaseSpace": {
        title: $I(
          "effectsMgr.statics.sattelite-starchartPerTickBaseSpace.title"
        ),
        type: "ratio"
      },
      ...
      "moonOutpost-unobtainiumPerTickSpace": {
        title: $I(
          "effectsMgr.statics.moonOutpost-unobtainiumPerTickSpace.title"
        ),
        type: "ratio"
      },
      ...
      "hydrofracturer-oilPerTickAutoprodSpace": {
        title: $I(
          "effectsMgr.statics.hydrofracturer-oilPerTickAutoprodSpace.title"
        ),
        type: "ratio"
      },
      ...
      "researchVessel-starchartPerTickBaseSpace": {
        title: $I(
          "effectsMgr.statics.researchVessel-starchartPerTickBaseSpace.title"
        ),
        type: "ratio"
      },
      ...
      "spaceBeacon-starchartPerTickBaseSpace": {
        title: $I(
          "effectsMgr.statics.spaceBeacon-starchartPerTickBaseSpace.title"
        ),
        type: "ratio"
      },
      ...
      manuscriptParchmentCost: {
        title: $I("effectsMgr.statics.manuscriptCost.title", [
          $I("resources.parchment.title")
        ]),
        type: "fixed"
      },
      manuscriptCultureCost: {
        title: $I("effectsMgr.statics.manuscriptCost.title", [
          $I("resources.culture.title")
        ]),
        type: "fixed"
      },
...
dojo.declare("com.nuclearunicorn.game.ui.GamePage", null, {
  ...
  constructor: function (containerId) {
    ...
    this.timer.addEvent(
      ...
      50
    ); //once per 50 ticks, we hardly need this
    this.timer.addEvent(
      dojo.hitch(this, function () {
        this.server.refresh();
      }),
      ONE_MIN * 10
    ); //reload MOTD and server info every 10 minutes
    this.timer.addEvent(
      dojo.hitch(this, function () {
        this.heartbeat();
      }),
      ONE_MIN * 10
    ); //send heartbeat every 10 min	//TODO: 30 min eventually
    this.timer.addEvent(
      dojo.hitch(this, function () {
        this.updateWinterCatnip();
      }),
      25
    ); //same as achievements, albeit a bit more frequient
    this.timer.addEvent(
      dojo.hitch(this, function () {
        this.ui.checkForUpdates();
      }),
      ONE_MIN * 5
    ); //check new version every 5 min
    this.timer.addEvent(
      dojo.hitch(this, function () {
        this.time.updateQueue();
      }),
      10
    );
  ...
  updateWinterCatnip: function () {
    this.winterCatnipPerTick = this.calcResourcePerTick("catnip", {
      modifiers: {
        catnip: 0.25
      }
  ...
  msg: function (message, type, tag, noBullet) {
    ...
    if (hasCalendarTech) {
      var currentDateMessage = $I("calendar.year.ext", [
        this.calendar.year.toLocaleString(),
        this.calendar.getCurSeasonTitle()
      ]);
  ...
  save: function () {
    ...
    var saveData = {
      ...
      resources: this.resPool.filterMetadata(this.resPool.resources, [
        "name",
        "value",
        "unlocked",
        "isHidden"
  ...
  wipe: function () {
    var game = this;
    this.ui.confirm(
      $I("wipe.confirmation.title"),
      $I("wipe.confirmation.msg1"),
      function () {
        game.ui.confirm(
          $I("wipe.confirmation.title"),
          $I("wipe.confirmation.msg2"),
          function () {
            game._wipe();
          }
        );
  ...
  load: function () {
    ...
    try {
      ...
      if (saveData) {
        console.log(
          "game#load - Successfully parsed local storage data, loading tab managers..."
        );
      ...
      this.msg(
        "Unable to load save data. Contact the devs and provide the faulty save file.",
        "important"
      );
      success = false;
    }
    ...
    if (saveData && saveData.game) {
      ...
      this.forceShowLimits = data.forceShowLimits
        ? data.forceShowLimits
        : false;
      this.colorScheme = data.colorScheme ? data.colorScheme : null;
      this.unlockedSchemes = data.unlockedSchemes
        ? data.unlockedSchemes
        : this.ui.defaultSchemes;

      this.karmaKittens =
        data.karmaKittens !== undefined ? data.karmaKittens : 0;
      ...
      this.isCMBREnabled =
        data.isCMBREnabled !== undefined ? data.isCMBREnabled : true; //true for all existing games
  ...
  saveToFile: function (withFullName) {
    ...
    if (withFullName) {
      filename +=
        " - Run " +
        (this.stats.getStat("totalResets").val + 1) +
        " - " +
        $I("calendar.year.full", [
          this.calendar.year,
          this.calendar.getCurSeasonTitle(),
          Math.floor(this.calendar.day)
        ]);
    }
  ...
  getDropboxAuthUrl: function () {
    ...
    var authUrl = this.dropBoxClient.getAuthenticationUrl(
      "https://" + window.location.host + redirectUrl
    );
    return authUrl;
  },
  ...
  importFromDropbox: function (callback) {
    ...
    var handler = function (e) {
      ...
      if (window.location.origin !== e.origin) {
        ...
        dbxt
          .filesDownload({ path: "/kittens.save" })
          .then(function (response) {
  ...
  saveImportDropboxText: function (lzdata, callback) {
    this.timer.scheduleEvent(
      dojo.hitch(this, this._loadSaveJson, lzdata, callback)
    );
  },
  ...
  migrateSave: function (save) {
    ...
    if (save.saveVersion == 2) {
      ...
      if (save.space && save.space.programs && save.space.planets) {
        ...
        for (var i = 0; i < save.space.programs.length; i++) {
          var program = save.space.programs[i];
          if (
            program.name == "spaceElevator" ||
            program.name == "sattelite" ||
            program.name == "spaceStation"
          ) {
            program.unlocked = true;
            buildings.push(program);
            save.space.programs.splice(i, 1);
      ...
      if (save.time) {
        if (save.time.usedCryochambers) {
          for (i = 0; i < save.time.usedCryochambers.length; i++) {
            save.time.usedCryochambers[i].on =
              save.time.usedCryochambers[i].val;
          }
        }
        if (save.time.cfu) {
    ...
    if (save.saveVersion == 9) {
      ...
      if (save.space && save.space.programs) {
        for (var i = 0; i < save.space.programs.length; i++) {
          if (
            save.space.programs[i].name == "rorschachMission" &&
            save.space.programs[i].on == 1
          ) {
    ...
    if (save.saveVersion == 11) {
      ...
      if (save.religion && save.religion.ru) {
        for (var i = 0; i < save.religion.ru.length; i++) {
          if (
            save.religion.ru[i].name == "transcendence" &&
            save.religion.ru[i].on == 1
          ) {
            var atheism = {
              name: "atheism",
              researched: false,
    ...
    if (save.saveVersion == 12) {
      if (save.religion && save.religion.tcratio && save.religion.tu) {
        var transcendenceLevel = Math.max(
          0,
          Math.round(
            Math.log(10 * this.game.getUnlimitedDR(save.religion.tcratio, 0.1))
          )
        );
        for (var i = 0; i < save.religion.tu.length; i++) {
          if (
            transcendenceLevel >=
            this.religion.getTU(save.religion.tu[i].name).tier
          ) {
          ...
          if (save.challenges.challenges[i].name == "atheism") {
            ...
            if (save.science && save.science.techs) {
              for (var j = 0; j < save.science.techs.length; j++) {
                if (
                  save.science.techs[j].name == "voidSpace" &&
                  save.science.techs[j].researched == true
                ) {
  ...
  calcResourcePerTick: function (resName, season) {
    ...
    if (
      this.workshop.get("spaceManufacturing").researched &&
      res.name != "uranium"
    ) {
    ...
    // +SPACE PerTickBase
    var perTickBaseSpace =
      this.getEffect(res.name + "PerTickBaseSpace") * spaceRatio;
    ...
    var resProduction = resMapProduction[res.name]
      ? resMapProduction[res.name]
      : 0;
    ...
    perTickAutoprod *=
      1 +
      this.getEffect("rankLeaderBonusConversion") *
        (this.village.leader ? this.village.leader.rank : 0);
    perTick += perTickAutoprod;

    // *MAGNETOS PRODUCTION BONUS
    if (
      !res.transient &&
      this.bld.get("magneto").on > 0 &&
      res.name != "catnip"
    ) {
      var steamworks = this.bld.get("steamworks");
      var swRatio =
        steamworks.on > 0
          ? 1 + steamworks.effects["magnetoBoostRatio"] * steamworks.on
          : 1;
      if (res.name != "oil") {
      ...
      paragonSpaceProductionRatio +=
        paragonSpaceProductionRatio * this.getEffect("magnetoRatio") * swRatio; //These special cases need to die in a hole
    }
    ...
    if (!res.transient && res.name != "uranium" && res.name != "catnip") {
      ...
      paragonSpaceProductionRatio +=
        paragonSpaceProductionRatio * this.getEffect("productionRatio");
    }

    // +*FAITH BONUS
    perTick *=
      1 +
      this.religion.getSolarRevolutionRatio() *
        (1 +
          (res.name == "wood" || res.name == "catnip"
            ? this.bld.pollutionEffects["solarRevolutionPollution"]
            : 0));
    ...
    perTick +=
      this.getEffect(res.name + "PerTickAutoprodSpace") *
      spaceRatio *
      (1 +
        (paragonSpaceProductionRatio - 1) *
          this.getEffect("prodTransferBonus"));
    ...
    perTick +=
      this.getEffect(res.name + "PerTick") *
      (1 + this.getEffect(res.name + "PerTickRatio"));
    ...
    if (
      res.name == "catnip" &&
      this.village.sim.kittens.length > 0 &&
      this.village.happiness > 1
    ) {
      var hapinnessConsumption = Math.max(
        this.village.happiness *
          (1 + this.getEffect("hapinnessConsumptionRatio")) -
          1,
        0
      );
      if (this.challenges.isActive("anarchy")) {
        resConsumption +=
          resConsumption *
          hapinnessConsumption *
          (1 + this.getEffect(res.name + "DemandWorkerRatioGlobal"));
      } else {
        resConsumption +=
          resConsumption *
          hapinnessConsumption *
          (1 + this.getEffect(res.name + "DemandWorkerRatioGlobal")) *
          (1 - this.village.getFreeKittens() / this.village.sim.kittens.length);
      }
    }
    // +POLICY EFFECTS
    ...
    perTick *=
      1 +
      this.resPool.get("sorrow").value * this.getEffect("blsProductionBonus");
    //pact Global Production Ratio- effect from pacts per pyramid
    var pyramidGlobalProductionRatio = this.getEffect(
      "pyramidGlobalProductionRatio"
    );
  ...
  addGlobalModToStack: function (array, resName) {
    ...
    if (game.science.getPolicy("necrocracy").researched) {
      array.push({
        ...
        type: "ratio",
        value:
          game.getEffect("blsProductionBonus") *
  ...
  getResourcePerTickStack: function (resName, calcAutomatedEffect, season) {
    ...
    if (
      (res.name == "wood" || res.name == "catnip") &&
      this.religion.getSolarRevolutionRatio() > 0
    ) {
      stack.push({
        name: $I("res.stack.pollution"),
        type: "ratioIndent",
    ...
    stack.push({
      // extra-compare with this.calcResourcePerTick
  ...
  getResourcePerDayStack: function (resName) {
    ...
    if (resName == "necrocorn") {
      stack.push({
        ...
        type: "perDay",
        value:
          this.getEffect("necrocornPerDay") +
    ...
    if (resName == "necrocorn") {
      ...
      corruptionStack.push({
        ...
        type: "perDay",
        value:
          this.religion.getCorruptionPerTickProduction() *
  ...
  getResourceOnYearStack: function (resName) {
    ...
    if (
      resName == "antimatter" &&
      this.resPool.energyProd < this.resPool.energyCons
    ) {
      stack.push({
  ...
  getCraftRatio: function (tag) {
    return (
      this.getEffect("craftRatio") +
      this.village.getEffectLeader("engineer", 0) +
      this.village.getEffectLeader(tag, 0)
    );
  },
  ...
  getResCraftRatio: function (craftedResName) {
    ...
    if (craftedResName == "blueprint") {
      ...
      var scienceBldAmt =
        this.bld.get("library").val +
        this.bld.get("academy").val +
        this.bld.get("observatory").val +
        this.bld.get("biolab").val;
  ...
  updateModel: function () {
    ...
    if (this.bld.cathPollution < 0) {
      this.bld.cathPollution = 0;
    }
  ...
  updateResources: function () {
    ...
    for (var i = 0; i < this.resPool.resources.length; i++) {
      ...
      if (res.calculatePerTick) {
        res.perTickCached = this.fixFloatPointNumber(
          this.calcResourcePerTick(res.name)
        );
      }
    }
  },
  ...
  getResourcePerTick: function (resName, withConversion) {
    ...
    if (res.calculatePerTick) {
      return withConversion
        ? res.perTickCached + this.getResourcePerTickConvertion(res.name)
        : res.perTickCached;
    } else {
      return 0;
    }
  ...
  getResourcePerDay: function (resName) {
    if (resName == "necrocorn") {
      return (
        this.religion.pactsManager.getNecrocornDeficitConsumptionModifier() *
          this.getEffect(resName + "PerDay") +
        this.religion.pactsManager.getSiphonedCorruption(1) +
  ...
  craftAll: function (resName) {
    ...
    if (resName != "wood" || this.getResourcePerTick("catnip", true) > 0) {
      ...
      var game = this;
      this.ui.confirm(
        $I("kittens.craft.confirmation.title"),
        $I("kittens.craft.confirmation.msg"),
        function () {
  ...
  attachResourceTooltip: function (container, resRef) {
    UIUtils.attachTooltip(this, container, 0, 100, function () {
      if (
        this.getResourcePerTick(resRef.name, false) != 0 ||
        ...
        resRef.name == "antimatter" ||
        (resRef.name == "kittens" &&
          this.village.sim.kittens.length < this.village.sim.maxKittens)
      ) {
  ...
  getDetailedResMap: function (res) {
    if (res.calculatePerDay) {
      ...
      if (this.opts.usePercentageResourceValues) {
        resString +=
          ...
          $I("res.netGain") +
          ": " +
          this.getDisplayValueExt(resPerDay, true, true);
      }
      if (resPerDay < 0) {
        var toZero =
          (this.calendar.ticksPerDay * res.value) /
          (-resPerDay *
            this.getTicksPerSecondUI() *
            (1 + this.timeAccelerationRatio()));
        resString +=
          "<br>" +
          $I("res.toZero") +
          ": " +
          this.toDisplaySeconds(toZero.toFixed());
      }
      if (res.name == "necrocorn") {
        var toNextNecrocorn =
          (1 - this.religion.corruption) /
          (this.religion.getCorruptionPerTick() *
            5 *
            (1 + this.timeAccelerationRatio()));
        if (toNextNecrocorn > 0) {
          resString +=
            "<br>" +
            $I("res.toNextNecrocorn") +
            ": " +
            this.toDisplaySeconds(toNextNecrocorn.toFixed());
        }
      }
      return resString;
      ...
      if (this.opts.usePercentageResourceValues) {
        resString +=
          ...
          $I("res.netGain") +
          ": " +
          this.getDisplayValueExt(resPerYear, true, true);
      }
      if (
        res.name == "antimatter" &&
        resPerYear &&
        this.resPool.energyWinterProd < this.resPool.energyCons
      ) {
    ...
    if (res.name == "kittens") {
      ...
      resString +=
        ...
        $I("res.toNextKitten") +
        " " +
        this.toDisplaySeconds((1 - nextKittenProgress) / kittensPerTick);
      return resString;
    }
    ...
    if (this.opts.usePercentageResourceValues) {
      resString +=
        ...
        $I("res.netGain") +
        ": " +
        this.getDisplayValueExt(resPerTick, true, true);
    }
    ...
    if (resPerTick < 0) {
      ...
      if (res.maxValue && res.value < res.maxValue) {
        var toCap =
          (res.maxValue - res.value) /
          (resPerTick * this.getTicksPerSecondUI());
        if (toCap) {
          resString +=
            "<br>" +
            $I("res.toCap") +
            ": " +
            this.toDisplaySeconds(toCap.toFixed());
        }
      }
    }
    ...
    if (res.calculatePerTickAndDay && res.name == "alicorn") {
      ...
      var totalPerDayDelta =
        "<br>" +
        $I("res.netGainPerDay") +
        ": " +
        this.getDisplayValueExt(
          resPerTick * this.calendar.ticksPerDay +
            this.getEffect("alicornPerDay")
        );
  ...
  processResourcePerTickStack: function (resStack, res, depth, hasFixed) {
    ...
    for (var i = 0; i < resStack.length; i++) {
      ...
      if (stackElem.length) {
        var subStack = this.processResourcePerTickStack(
          stackElem,
          res,
          depth + 1,
          hasFixed
      ...
      if (
        !stackElem.value ||
        (stackElem.type != "fixed" &&
          stackElem.type != "perDay" &&
          stackElem.type != "perYear" &&
          !hasFixed)
      ) {
        continue;
      }
  ...
  getStackElemString: function (stackElem, res) {
    ...
    if (stackElem.type == "fixed") {
      ...
      resString +=
        this.getDisplayValueExt((stackElem.value * 100).toFixed(), true) + "%";
    } else if (stackElem.type == "multiplier") {
      resString +=
        "×" + this.getDisplayValueExt((stackElem.value * 100).toFixed()) + "%";
      ...
      resString +=
        this.getDisplayValueExt(stackElem.value) + "/" + $I("res.per.day");
    } else if (stackElem.type == "perYear") {
      if (stackElem.value > 0) {
        resString += "+";
      }
      resString +=
        this.getDisplayValueExt(stackElem.value) + "/" + $I("res.per.year");
    }
  ...
  toDisplaySeconds: function (secondsRaw) {
    ...
    var minutes = Math.floor(
      (sec_num - years * year_secs - (days * 86400 + hours * 3600)) / 60
    );
    var seconds =
      sec_num - years * year_secs - days * 86400 - hours * 3600 - minutes * 60;
    ...
    var timeFormated = "";
    if (years) {
      timeFormated = years + $I("unit.y") + " ";
    }
    if (days) {
      timeFormated += days + $I("unit.d") + " ";
    }
    if (!years) {
      if (hours) {
        timeFormated += hours + $I("unit.h") + " ";
      }
      if (minutes) {
        timeFormated += minutes + $I("unit.m") + " ";
      }
      if (seconds) {
        timeFormated += seconds + $I("unit.s") + " ";
      }
    }
  ...
  toDisplayDays: function (daysRaw) {
    ...
    var daysPerYear =
      this.calendar.daysPerSeason * this.calendar.seasonsPerYear;
          ...
          if (percentage * 100 - Math.floor(percentage * 100) != 0) {
            ...
            if (percentage * 1000 - Math.floor(percentage * 1000) != 0) {
              ...
              if (percentage * 10000 - Math.floor(percentage * 10000) != 0) {
                precision = 5;
                if (
                  percentage * 100000 - Math.floor(percentage * 100000) !=
                  0
                ) {
                  precision = 6;
                  if (
                    percentage * 1000000 - Math.floor(percentage * 1000000) !=
                    0
                  ) {
                    precision = 7;
                    if (
                      percentage * 10000000 -
                        Math.floor(percentage * 10000000) !=
                      0
                    ) {
                      precision = 8;
                    }
  ...
  getEffectDisplayParams: function (effectName, effectValue, showIfZero) {
    ...
    if (effectName.substr(-3) === "Max") {
      ...
      if (res) {
        // If res is a resource and not just a variable
        effectValue = this.resPool.addResMaxRatios(res, effectValue);
      }
    }
    if (effectName.substr(-12) === "MaxChallenge") {
      var res = this.resPool.get(
        effectMeta.resName || effectName.slice(0, -12)
      );
      if (res) {
        // If res is a resource and not just a variable
        //This is the code used in resources.js to apply LDR to how Challenges affect resource storage.
        var maxValue = this.resPool.addResMaxRatios(
          res,
          this.getEffect(res.name + "Max") || 0
        );
        effectValue = this.getLimitedDR(
          this.resPool.addResMaxRatios(res, effectValue),
          maxValue - 1 - this.bld.effectsBase[res.name + "Max"] || 0
        );
      }
    }
    if (effectMeta.type === "perTick" && this.opts.usePerSecondValues) {
      ...
      var tempVal = Math.abs(effectValue * this.ticksPerSecond),
        precision;
      if (tempVal >= 0.001) {
        precision = tempVal < 0.01 ? 3 : 2;
        displayEffectValue =
          this.getDisplayValueExt(
            effectValue * this.ticksPerSecond,
            false,
            false,
            precision
          ) +
        ...
        displayEffectValue =
          this.getDisplayValueExt(
            effectValue * this.ticksPerSecond * 3600,
            false,
            false,
            2
          ) +
      ...
      displayEffectValue =
        this.getDisplayValueExt(effectValue) + "/" + $I("unit.day");
    } else if (effectMeta.type === "perYear") {
      displayEffectValue =
        this.getDisplayValueExt(effectValue) + "/" + $I("unit.year");
    } else if (effectMeta.type === "ratio") {
      displayEffectValue =
        this.toDisplayPercentage(effectValue, 2, false) + "%";
      ...
      displayEffectValue =
        this.getDisplayValueExt(effectValue) + $I("unit.watt");
    } else {
      displayEffectValue = this.getDisplayValueExt(effectValue);
    }
    return {
    ...
    switch (this.opts.notation) {
      ...
      default:
        ...
        for (var i = 0; i < this.postfixes.length; i++) {
          ...
          if (absValue >= p.limit) {
            if (usePerTickHack) {
              // Prevent recursive * this.ticksPerSecond;
              value = value / this.ticksPerSecond;
            }
            return this.getDisplayValueExt(
              value / p.divisor,
              prefix,
              usePerTickHack,
              precision,
  ...
  getDisplayValue: function (floatVal, plusPrefix, precision) {
    ...
    if (floatVal != 0) {
      ...
      if (
        (absVal < 0.01 && precision == 2) ||
        (absVal < 0.001 && precision == 3)
      ) {
        mantisa = "(...)";
      }
    }
  ...
  fixFloatPointNumber: function (number) {
    ...
    if (Math.round((number - numberAdjusted) * 10000000)) {
      numberAdjusted =
        Math.floor((number + 0.00000000000001) * 10000000) / 10000000;
    }
  ...
  start: function () {
    if (this.isWebWorkerSupported() && this.useWorkers) {
      //IE10 has a nasty security issue with running blob workers
      console.log("starting web worker...");
      var blob = new Blob([
        "onmessage = function(e) { setInterval(function(){ postMessage('tick'); }, 1000 / " +
          this.ticksPerSecond +
          "); }"
      ]); //no shitty external js
  ...
  tick: function () {
    ...
    this.totalUpdateTimeCurrent =
      this.totalUpdateTimeCurrent == 4 ? 0 : this.totalUpdateTimeCurrent + 1;

    var avg =
      (this.totalUpdateTime[0] +
        this.totalUpdateTime[1] +
        this.totalUpdateTime[2] +
        this.totalUpdateTime[3] +
        this.totalUpdateTime[4]) /
      this.totalUpdateTimeTicks;
    var avg0 =
      this.totalUpdateTime[0] / Math.floor((this.totalUpdateTimeTicks - 1) / 5);
    var avg1 =
      this.totalUpdateTime[1] / Math.floor((this.totalUpdateTimeTicks - 2) / 5);
    var avg2 =
      this.totalUpdateTime[2] / Math.floor((this.totalUpdateTimeTicks - 3) / 5);
    var avg3 =
      this.totalUpdateTime[3] / Math.floor((this.totalUpdateTimeTicks - 4) / 5);
    var avg4 =
      this.totalUpdateTime[4] / Math.floor((this.totalUpdateTimeTicks - 5) / 5);
  ...
  reset: function () {
    ...
    game.ui.confirm($I("reset.confirmation.title"), msg, function () {
      ...
      if (
        game.challenges.isActive("atheism") &&
  ...
  discardParagon: function () {
    ...
    this.ui.confirm("", $I("discardParagon.confirmation.msg1"), function () {
      if (game.resPool.get("paragon").value <= 100) {
        ...
        game.ui.confirm(
          "",
          $I("discardParagon.confirmation.msg2"),
          function () {
            if (
              !game.ironWill ||
              game.achievements.get("spaceOddity").starUnlocked
            ) {
              confirmDiscard = true;
            } else {
              game.ui.confirm(
                "",
                $I("discardParagon.confirmation.msgIW"),
                function () {
                  confirmDiscard = true;
                }
              );
    ...
    if (confirmDiscard) {
      this.resPool.get("burnedParagon").value +=
        this.resPool.get("paragon").value;
  ...
  getResetPrestige: function () {
    var kittens = Math.round(
      this.resPool.get("kittens").value *
        (1 + this.getEffect("simScalingRatio"))
  ...
  _resetInternal: function () {
    ...
    var saveRatio =
      this.bld.get("chronosphere").val > 0
        ? this.getEffect("resStasisRatio")
        : 0; // resStasisRatio excepted when challenge
    dojo.mixin(lsData.game, {
      karmaKittens: karmaKittens,
      karmaZebras: karmaZebras,
      ironWill:
        saveRatio > 0 || this.time.getVSU("cryochambers").on > 0 ? false : true, //chronospheres or cryochaimbers will disable IW. Post Apocalypse allows cryochaimbers to work without chronospheres working.
      deadKittens: 0,
      isCMBREnabled: false
    });
    ...
    if (cryochambers > 0) {
      ...
      var usedCryochambers_reset = this.time.filterMetadata(
        [this.time.getVSU("usedCryochambers")],
        ["name", "val", "on"]
      );
    ...
    if (this.challenges.getChallenge("postApocalypse").pending) {
      if (newKittens.length > 0) {
        ...
        catnip.value =
          newKittens.length *
          1000 *
          (1 + this.resPool.get("karma").value / 100);
        newResources.push(catnip);
      }
      cathPollution =
        (this.challenges.getChallenge("postApocalypse").on +
          newKittens.length) *
          1e8 +
        1e11;
    }
...
*/
    ...
    var saveData = {
      ...
      religion: {
        ...
        zu: [],
        ru: [],
        tu: this.religion.filterMetadata(this.religion.transcendenceUpgrades, [
          "name",
          "val",
          "on",
          "unlocked"
    ...
    for (var type in list) {
      ...
      for (var i = list[type].length - 1; i >= 0; i--) {
        ...
        if (!newUnlock) {
          console.trace();
          console.error(
            "unable to evaluate locks for unlockId",
            unlockId,
            "type",
            type
          );
  ...
  checkEldermass: function () {
    if (this.isEldermass()) {
      ...
      if (
        this.resPool.get("elderBox").value > 0 &&
  ...
  redeemGift: function () {
    ...
    if (
      this.resPool.get("timeCrystal").value &&
      this.prestige.getPerk("anachronomancy").researched &&
      this.workshop.get("stasisChambers").researched
    ) {
      gift = "TimeCrystal";
    }
    if (
      this.resPool.get("sorrow").value / this.resPool.get("sorrow").maxValue <
        0.25 &&
      this.prestige.getPerk("megalomania").researched &&
      this.religion.getZU("blackPyramid").val < 3
    ) {
      gift = "BLS";
    }
    if (this.religion.getRU("apocripha").on) {
    ...
    if (
      this.religion.getRU("transcendence").on &&
      this.religion.transcendenceTier <= 10
    ) {
      gift = "Transcendence";
    }
    if (
      this.prestige.getPerk("engeneering").researched &&
      !this.prestige.getPerk("renaissance").researched
    ) {
      gift = "Metaphysics";
    }
    ...
    if (
      this.diplomacy.get("leviathans").energy == this.diplomacy.getMarkerCap()
    ) {
      gift = "BurnedParagon";
    }
    ...
    switch (gift) {
      case "Karma":
        ...
        var karmaGained =
          this.getUnlimitedDR(this.karmaKittens + amt, 5) -
          this.getUnlimitedDR(this.karmaKittens, 5);
        var msg = $I("gift.resources", [
          this.getDisplayValueExt(karmaGained),
          ucfirst($I("resources.karma.title"))
        ]);
      ...
      case "Paragon":
        ...
        var msg = $I("gift.resources", [
          this.getDisplayValueExt(amt),
          ucfirst($I("resources.paragon.title"))
        ]);
      ...
      case "TimeCrystal":
        ...
        var msg = $I("gift.resources", [
          this.getDisplayValueExt(amt),
          ucfirst($I("resources.timeCrystal.title"))
        ]);
      ...
      case "BLS":
        amt =
          this.resPool.get("sorrow").maxValue -
          this.resPool.get("sorrow").value;
        var msg = $I("gift.resources", [
          this.getDisplayValueExt(amt),
          ucfirst($I("resources.sorrow.full"))
        ]);
      ...
      case "Apocrypha":
        ...
        var msg = $I("gift.apocrypha", [
          this.getDisplayValueExt(apocryphaGained)
        ]);
        break;
      ...
      case "Compendiums":
        ...
        var getSumOfPrices = function (item, resName) {
          //this function should return how much of one resource was spent on a stackable
          ...
          return (
            (resPrice * (Math.pow(priceRatio, num) - 1)) / (priceRatio - 1)
          );
        };
        var amt1 = Math.max(
          getSumOfPrices(unicornGraveyard, "necrocorn") +
            getSumOfPrices(unicornNecropolis, "necrocorn"),
          Math.max(this.resPool.get("necrocorn").value, 10)
        );
        var msg = $I("gift.resources", [
          this.getDisplayValueExt(amt),
          ucfirst($I("resources.compedium.title"))
        ]);
        msg += $I("gift.resources", [
          this.getDisplayValueExt(amt1),
          ucfirst($I("resources.necrocorn.title"))
        ]);
      ...
      case "BurnedParagon":
        ...
        var msg = $I("gift.resources", [
          this.getDisplayValueExt(amt),
          ucfirst($I("resources.burnedParagon.title"))
        ]);
  ...
  isEldermass: function () {
    var date = new Date();
    return (
      date.getMonth() == 11 && date.getDate() >= 15 && date.getDate() <= 31
    );
  },
  createRandomName: function (lenConst, charPool) {
    if (!charPool) {
      charPool =
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■◆◆åœ∑ß≈ç∂´®ƒ√∫©†¥˙˜µ∆¨ˆ˚µ≤¬øπ≥…π“æ÷æ«§¡™£¢∞§¶•ªº≠"; //◆
    }
